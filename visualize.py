...
from urm import TraceStep, instruction_to_string


def _trace_to_matrix(trace: List[TraceStep]) -> np.ndarray:
    """
    ...
    max_regs = max(len(step.registers) for step in trace)
    matrix = []

    for step in trace:
        row = step.registers + [0] * (max_regs - len(step.registers))
        matrix.append(row)
...
def _changed_registers(trace: List[TraceStep]) -> List[Optional[int]]:
    ...
    for i in range(1, len(trace)):
        prev = trace[i - 1].registers
        curr = trace[i].registers
...
def _jump_flow_suffix(trace: List[TraceStep], index: int) -> str:
    ...
    step = trace[index]
    if step.instruction is None or step.instruction[0] != "J":
        return ""

    executed_pc = trace[index - 1].pc
    next_pc = step.pc
...
def _row_labels(trace: List[TraceStep]) -> List[str]:
    ...
    for i, step in enumerate(trace):
        if step.instruction is None:
            ...
            instr = instruction_to_string(step.instruction)
...
def print_trace_table(trace: List[TraceStep]) -> None:
    ...
    for step in trace:
        instr = "None" if step.instruction is None else instruction_to_string(step.instruction)
        row = [str(step.step), str(step.pc), instr]
        row += [str(x) for x in step.registers + [0] * (max_regs - len(step.registers))]
        rows.append(row)
...
def pretty_trace(trace: List[TraceStep]) -> None:
    ...
    for step in trace:
        instr = "START" if step.instruction is None else instruction_to_string(step.instruction)

        changes = []
        if prev_regs is not None:
            max_len = max(len(prev_regs), len(step.registers))
            prev_pad = prev_regs + [0] * (max_len - len(prev_regs))
            curr_pad = step.registers + [0] * (max_len - len(step.registers))

            for i in range(max_len):
                if prev_pad[i] != curr_pad[i]:
                    changes.append(f"R{i}")
...
Registers:
{format_registers(step.registers, changes)}
