...
function buildGeneratedScanV2DfsGrammar({ analysis, runs = [], regions = [] }) {
  const programLength = analysis?.program?.length ?? 0;
  if (programLength <= 0) {
    return {
      enabled: false,
      skippedReason: "programUnavailable",
      rootInstructionIndex: null,
      discoveryOrder: [],
      discoveryIndexByInstruction: {},
      repeatedNodeClosures: [],
      setupPrefixRunIds: [],
      programCfgEdges: [],
      localMotifs: [],
      diamondPortAssignments: [],
      rootSubstructures: [],
      substructuresByParentRegionId: {},
    };
  }
  ...
  (analysis?.edges ?? []).forEach((edge) => {
    ...
    if (edge.targetIndex < 0 || edge.targetIndex >= programLength) return;
    if (!outgoingBySource.has(edge.sourceIndex)) outgoingBySource.set(edge.sourceIndex, []);
    outgoingBySource.get(edge.sourceIndex).push(edge);
  });
  outgoingBySource.forEach((outgoing) => {
    outgoing.sort((left, right) => (
      (left.targetIndex - right.targetIndex) ||
      (left.type === "fallthrough" ? -1 : 1) ||
  ...
  const repeatedNodeClosures = [];

  const visit = (instructionIndex) => {
    if (!Number.isInteger(instructionIndex)) return;
    if (instructionIndex < 0 || instructionIndex >= programLength) return;
    if (visited.has(instructionIndex)) return;

    visited.add(instructionIndex);
    activePath.push(instructionIndex);
    activePathSet.add(instructionIndex);
    discoveryIndexByInstruction.set(instructionIndex, discoveryOrder.length);
    discoveryOrder.push(instructionIndex);

    (outgoingBySource.get(instructionIndex) ?? []).forEach((edge) => {
      const targetIndex = edge.targetIndex;
      if (activePathSet.has(targetIndex)) {
        repeatedNodeClosures.push({
          edgeId: edge.id,
          sourceIndex: instructionIndex,
          targetIndex,
          closureKind: "currentPathRepeat",
          repeatedPath: activePath.slice(activePath.indexOf(targetIndex)),
        });
        return;
      }
      if (visited.has(targetIndex)) {
        repeatedNodeClosures.push({
          ...
          sourceIndex: instructionIndex,
          targetIndex,
          closureKind: "discoveredNodeRejoin",
          repeatedPath: [],
        });
        return;
      }
    ...
    activePath.pop();
    activePathSet.delete(instructionIndex);
  };

  visit(0);
  for (let instructionIndex = 0; instructionIndex < programLength; instructionIndex += 1) {
    visit(instructionIndex);
  }

  const getIntervalDiscoveryRank = (startIndex, endIndex) => {
  ...
  const compareSubstructures = (left, right) => (
    (left.discoveryRank - right.discoveryRank) ||
    (left.startIndex - right.startIndex) ||
    (left.kind === "region" ? -1 : 1) ||
    String(left.sourceId).localeCompare(String(right.sourceId))
  );
  const describePorts = (startIndex, endIndex) => {
    const entryInstructionIndexes = [];
    const exitInstructionIndexes = [];
    (analysis?.edges ?? []).forEach((edge) => {
      if (!Number.isInteger(edge.sourceIndex) || !Number.isInteger(edge.targetIndex)) return;
      const sourceInside = startIndex <= edge.sourceIndex && edge.sourceIndex <= endIndex;
      const targetInside = startIndex <= edge.targetIndex && edge.targetIndex <= endIndex;
      if (!sourceInside && targetInside) entryInstructionIndexes.push(edge.targetIndex);
      if (sourceInside && !targetInside) exitInstructionIndexes.push(edge.sourceIndex);
    });
    return {
      entrancePorts: Array.from(new Set(entryInstructionIndexes))
        .sort((left, right) => (
          (discoveryIndexByInstruction.get(left) ?? Number.MAX_SAFE_INTEGER) -
          (discoveryIndexByInstruction.get(right) ?? Number.MAX_SAFE_INTEGER)
        ))
        .map(formatInstructionNodeId),
      exitPorts: Array.from(new Set(exitInstructionIndexes))
        .sort((left, right) => (
          (discoveryIndexByInstruction.get(left) ?? Number.MAX_SAFE_INTEGER) -
          (discoveryIndexByInstruction.get(right) ?? Number.MAX_SAFE_INTEGER)
        ))
  ...
  const getRunSubstructureRole = (run) => {
    if (run.role === "setupRun") return "setup";
    if (run.role === "exitTailRun") return "exitHaltBranch";
    const hasConditional = rangeIndexes(run.startIndex, run.endIndex).some(
      (index) => analysis?.nodeRoles?.[index] === "conditionalJump",
    );
    if (hasConditional) return "localConditional";
    if (run.role === "mergeRun") return "mergeContinuation";
    return "mainBody";
  };
  ...
  const regionRows = regions.map((region) => ({
    ...
    endIndex: region.endIndex,
    parentRegionId: region.parentRegionId ?? null,
    discoveryRank: getIntervalDiscoveryRank(region.startIndex, region.endIndex),
    substructureRole: "loopFrame",
    ...describePorts(region.startIndex, region.endIndex),
  }));
  const runRows = runs.map((run) => ({
    ...
    startIndex: run.startIndex,
    endIndex: run.endIndex,
    parentRegionId: run.parentRegionId ?? null,
    discoveryRank: getIntervalDiscoveryRank(run.startIndex, run.endIndex),
    substructureRole: getRunSubstructureRole(run),
    ...describePorts(run.startIndex, run.endIndex),
  }));
  const substructuresByParentRegionId = {};
  [...regionRows, ...runRows].forEach((row) => {
    const parentKey = row.parentRegionId ?? "root";
    if (!substructuresByParentRegionId[parentKey]) {
      substructuresByParentRegionId[parentKey] = [];
    }
    substructuresByParentRegionId[parentKey].push(row);
  });
  Object.values(substructuresByParentRegionId).forEach((rows) => rows.sort(compareSubstructures));
  const localMotifs = buildGeneratedScanV2LocalMotifs({
    analysis,
    runs,
    discoveryIndexByInstruction,
  });
  const diamondPortAssignments = buildGeneratedScanV2DiamondPortAssignments({
    analysis,
    regions,
    localMotifs,
  });
  ...
  return {
    ...
    discoveryOrder,
    discoveryIndexByInstruction: Object.fromEntries(discoveryIndexByInstruction),
    repeatedNodeClosures,
    setupPrefixRunIds: runRows
      .filter((run) => run.substructureRole === "setup")
      .sort(compareSubstructures)
      .map((run) => run.sourceId),
    programCfgEdges: (analysis?.edges ?? [])
    ...
    localMotifs,
    diamondPortAssignments,
    rootSubstructures: substructuresByParentRegionId.root ?? [],
    substructuresByParentRegionId,
  };
}

function buildGeneratedScanV2DiamondPortAssignments({
  analysis,
  regions = [],
  localMotifs = [],
}) {
  const programLength = analysis?.program?.length ?? 0;
  const motifByDecisionIndex = new Map(
    localMotifs.map((motif) => [motif.decisionIndex, motif]),
  );
  const containingRegionsByInstruction = new Map();
  const getContainingRegions = (instructionIndex) => {
    if (!Number.isInteger(instructionIndex)) return [];
    if (!containingRegionsByInstruction.has(instructionIndex)) {
      containingRegionsByInstruction.set(
        instructionIndex,
        regions
          .filter((region) => (
            region.startIndex <= instructionIndex &&
            instructionIndex <= region.endIndex
          ))
          .sort((left, right) => (
            (left.endIndex - left.startIndex) - (right.endIndex - right.startIndex)
          )),
    ...
    return containingRegionsByInstruction.get(instructionIndex);
  };
  const targetsAncestorRegion = (sourceIndex, targetIndex) => {
    const sourceRegions = getContainingRegions(sourceIndex);
    if (sourceRegions.length < 2) return false;
    const innermost = sourceRegions[0];
    if (innermost.startIndex <= targetIndex && targetIndex <= innermost.endIndex) return false;
    return sourceRegions.slice(1).some((region) => (
      region.startIndex <= targetIndex && targetIndex <= region.endIndex
    ));
  };

  return (analysis?.nodeRoles ?? [])
    .map((role, decisionIndex) => ({ role, decisionIndex }))
    .filter(({ role }) => role === "conditionalJump")
    .map(({ decisionIndex }) => {
      const outgoingEdges = getOutgoingEdges(analysis.edges ?? [], decisionIndex);
      const adjacentIncidentEdgeCount = (analysis.edges ?? []).filter((edge) => (
        edge.sourceIndex === decisionIndex || edge.targetIndex === decisionIndex
      )).length;
      const fallthroughEdge = outgoingEdges.find(
        (edge) => edge.targetIndex === decisionIndex + 1,
      ) ?? null;
      const branchAssignments = outgoingEdges.map((edge) => {
        const side = edge.id === fallthroughEdge?.id ? "left" : "right";
        let overrideReason = null;
        if (isHaltExitEdge(edge, programLength)) {
          overrideReason = "immediateHaltOrOutputExit";
        } else if (!Number.isInteger(edge.targetIndex) || edge.targetIndex <= decisionIndex) {
          overrideReason = "loopBackOrFrameEdge";
        } else if (targetsAncestorRegion(decisionIndex, edge.targetIndex)) {
          overrideReason = "parentOrAncestorRegionTarget";
        }
        return {
          edgeId: edge.id,
          targetIndex: Number.isInteger(edge.targetIndex) ? edge.targetIndex : null,
          side,
          preferLowerDiagonal: overrideReason === null,
          overrideReason,
        };
      });
      const preferredDiagonalBranches = branchAssignments.filter(
        (assignment) => assignment.preferLowerDiagonal,
      );

      return {
        decisionIndex,
        decisionNodeId: formatInstructionNodeId(decisionIndex),
        adjacentIncidentEdgeCount,
        allowsDenseIncidentOverride: adjacentIncidentEdgeCount >= 4,
        branchClearance: motifByDecisionIndex.get(decisionIndex)?.branchClearance ?? 92,
        diagonalCorridorClearance: 54,
        branchAssignments,
        preferredDiagonalEdgeIds: preferredDiagonalBranches.map((assignment) => assignment.edgeId),
        branchAssignmentByEdgeId: Object.fromEntries(
          branchAssignments.map((assignment) => [assignment.edgeId, assignment]),
        ),
      };
    });
}

function buildGeneratedScanV2LocalMotifs({
  analysis,
  runs = [],
  discoveryIndexByInstruction = new Map(),
}) {
  const programLength = analysis?.program?.length ?? 0;
  const maxLocalSpan = 8;
  const branchClearance = 92;
  const frameClearance = 48;
  const branchMergeBySource = new Map(
    (analysis?.branchMergeRegions ?? []).map((region) => [region.sourceIndex, region]),
  );
  const runByInstruction = new Map();
  runs.forEach((run) => {
    for (let index = run.startIndex; index <= run.endIndex; index += 1) {
  ...
  const incomingCountByInstruction = new Map();
  (analysis?.edges ?? []).forEach((edge) => {
    if (!Number.isInteger(edge.targetIndex)) return;
    incomingCountByInstruction.set(
      edge.targetIndex,
      (incomingCountByInstruction.get(edge.targetIndex) ?? 0) + 1,
    );
  });
  const buildBranchLaneIndexes = ({
    decisionIndex,
    branchEdge,
    explicitArmIndexes = [],
    confirmedMergeIndexes = new Set(),
    allowContinuationPropagation = true,
  }) => {
    const branchTargetIndex = branchEdge?.targetIndex;
    if (
      !Number.isInteger(branchTargetIndex) ||
      branchTargetIndex <= decisionIndex ||
      branchTargetIndex >= programLength ||
      branchTargetIndex - decisionIndex > maxLocalSpan
    ) {
      return [];
    }

    const laneIndexes = new Set(explicitArmIndexes);
    if (!allowContinuationPropagation) {
      if (!confirmedMergeIndexes.has(branchTargetIndex)) laneIndexes.add(branchTargetIndex);
      return Array.from(laneIndexes).sort((left, right) => left - right);
    }

    let currentIndex = branchTargetIndex;
    const visited = new Set();
    while (
      Number.isInteger(currentIndex) &&
      currentIndex > decisionIndex &&
      currentIndex < programLength &&
      currentIndex - decisionIndex <= maxLocalSpan &&
      !visited.has(currentIndex)
    ) {
      visited.add(currentIndex);
      if (confirmedMergeIndexes.has(currentIndex)) break;
      laneIndexes.add(currentIndex);
      if (analysis.nodeRoles[currentIndex] === "conditionalJump") break;
      ...
      if (
        !Number.isInteger(nextEdge.targetIndex) ||
        nextEdge.targetIndex !== currentIndex + 1 ||
        confirmedMergeIndexes.has(nextEdge.targetIndex) ||
        (incomingCountByInstruction.get(nextEdge.targetIndex) ?? 0) > 1
      ) {
        break;
      }
      currentIndex = nextEdge.targetIndex;
  ...
  return (analysis?.nodeRoles ?? [])
    .map((role, index) => ({ role, index }))
    .filter(({ role }) => role === "conditionalJump")
    .map(({ index: decisionIndex }) => {
      const outgoingEdges = getOutgoingEdges(analysis.edges ?? [], decisionIndex);
      const forwardEdges = outgoingEdges
        .filter((edge) => (
          Number.isInteger(edge.targetIndex) &&
          edge.targetIndex > decisionIndex &&
          edge.targetIndex < programLength
        ));
      if (outgoingEdges.length < 2 || forwardEdges.length === 0) return null;

      const branchMerge = branchMergeBySource.get(decisionIndex) ?? null;
      const branchMergeIndexes = [
        ...(branchMerge?.fallthroughArm ?? []),
        ...(branchMerge?.jumpArm ?? []),
        branchMerge?.mergeIndex,
      ].filter(Number.isInteger);
      const branchMergeSpanEnd = branchMergeIndexes.length > 0
        ? Math.max(decisionIndex, ...branchMergeIndexes)
        : null;
      const canUseBranchMerge = (
        Number.isInteger(branchMergeSpanEnd) &&
        branchMergeSpanEnd - decisionIndex <= maxLocalSpan
      );

      const localForwardEdges = forwardEdges.filter((edge) => (
        edge.targetIndex - decisionIndex <= maxLocalSpan
      ));
      const localIndexes = new Set([decisionIndex]);
      const addLocalIndex = (instructionIndex) => {
        if (!Number.isInteger(instructionIndex)) return;
        if (instructionIndex <= decisionIndex || instructionIndex >= programLength) return;
        if (instructionIndex - decisionIndex > maxLocalSpan) return;
        localIndexes.add(instructionIndex);
      };

      if (canUseBranchMerge) {
        (branchMerge.fallthroughArm ?? []).forEach(addLocalIndex);
        (branchMerge.jumpArm ?? []).forEach(addLocalIndex);
        addLocalIndex(branchMerge.mergeIndex);
      } else {
        localForwardEdges.forEach((edge) => addLocalIndex(edge.targetIndex));
        addLocalIndex(decisionIndex + 1);
        addLocalIndex(decisionIndex + 2);
      }

      if (localIndexes.size < 3) return null;

      const fallthroughEdge = localForwardEdges.find(
        (edge) => edge.targetIndex === decisionIndex + 1,
      ) ?? null;
      const jumpEdge = localForwardEdges.find((edge) => edge.id !== fallthroughEdge?.id) ?? null;
      const fallthroughArmIndexes = canUseBranchMerge
        ? [...(branchMerge.fallthroughArm ?? [])].filter((index) => localIndexes.has(index))
        : (fallthroughEdge ? [fallthroughEdge.targetIndex] : []);
      const jumpArmIndexes = canUseBranchMerge
        ? [...(branchMerge.jumpArm ?? [])].filter((index) => localIndexes.has(index))
        : (jumpEdge ? [jumpEdge.targetIndex] : []);
      const localBranchEdges = [fallthroughEdge, jumpEdge].filter(Boolean);
      const branchSideByEdgeId = {};
      if (fallthroughEdge) branchSideByEdgeId[fallthroughEdge.id] = "left";
      if (jumpEdge) branchSideByEdgeId[jumpEdge.id] = "right";
      const structuralFallthroughPath = fallthroughEdge
        ? walkSimpleForwardPath(analysis, fallthroughEdge.targetIndex, decisionIndex)
        : [];
      const structuralJumpPath = jumpEdge
        ? walkSimpleForwardPath(analysis, jumpEdge.targetIndex, decisionIndex)
        : [];
      const jumpPathIndexes = new Set(structuralJumpPath);
      const firstSharedForwardIndex = structuralFallthroughPath.find(
        (instructionIndex) => jumpPathIndexes.has(instructionIndex),
      );
      const confirmedMergeIndexes = new Set(
        [
          branchMerge?.mergeIndex,
          firstSharedForwardIndex,
        ].filter(Number.isInteger),
      );
      const adjacentIncidentEdgeCount = (analysis?.edges ?? []).filter((edge) => (
        edge.sourceIndex === decisionIndex || edge.targetIndex === decisionIndex
      )).length;
      const allowContinuationPropagation = adjacentIncidentEdgeCount < 4;
      const fallthroughLaneIndexes = buildBranchLaneIndexes({
        decisionIndex,
        branchEdge: fallthroughEdge,
        explicitArmIndexes: fallthroughArmIndexes,
        confirmedMergeIndexes,
        allowContinuationPropagation,
      });
      const jumpLaneIndexes = buildBranchLaneIndexes({
        decisionIndex,
        branchEdge: jumpEdge,
        explicitArmIndexes: jumpArmIndexes,
        confirmedMergeIndexes,
        allowContinuationPropagation,
      });
      [...fallthroughLaneIndexes, ...jumpLaneIndexes].forEach(addLocalIndex);
      const memberInstructionIndexes = Array.from(localIndexes).sort((left, right) => left - right);
      const levelConsistencyHints = buildGeneratedScanV2LevelConsistencyHints({
        analysis,
        decisionIndex,
        memberInstructionIndexes,
        branchEdges: localBranchEdges,
      });

      return {
        motifId: `scan-local-motif:${decisionIndex}`,
        decisionIndex,
        decisionNodeId: formatInstructionNodeId(decisionIndex),
        discoveryRank:
          discoveryIndexByInstruction.get(decisionIndex) ?? Number.MAX_SAFE_INTEGER,
        ownerRunId: runByInstruction.get(decisionIndex)?.runId ?? null,
        memberInstructionIndexes,
        fallthroughArmIndexes,
        jumpArmIndexes,
        mergeIndex: canUseBranchMerge && Number.isInteger(branchMerge.mergeIndex)
          ? branchMerge.mergeIndex
          : null,
        localBranchEdgeIds: localBranchEdges.map((edge) => edge.id),
        branchSideByEdgeId,
        branchLanes: [
          {
            edgeId: fallthroughEdge?.id ?? null,
            side: "left",
            instructionIndexes: fallthroughLaneIndexes,
          },
          {
            edgeId: jumpEdge?.id ?? null,
            side: "right",
            instructionIndexes: jumpLaneIndexes,
          },
        ].filter((lane) => lane.edgeId && lane.instructionIndexes.length > 0),
        levelConsistencyHints,
        branchClearance,
        frameClearance,
      };
    })
...
function buildGeneratedScanV2LevelConsistencyHints({
  analysis,
  decisionIndex,
  memberInstructionIndexes = [],
  branchEdges = [],
  ...
  const programLength = analysis?.program?.length ?? 0;
  const memberIndexes = new Set(memberInstructionIndexes);
  const forwardAdjacencyBySource = new Map();
  (analysis?.edges ?? []).forEach((edge) => {
    if (!Number.isInteger(edge.sourceIndex) || !Number.isInteger(edge.targetIndex)) return;
    if (!memberIndexes.has(edge.sourceIndex) || !memberIndexes.has(edge.targetIndex)) return;
    if (edge.targetIndex <= edge.sourceIndex || edge.targetIndex >= programLength) return;
    if (!forwardAdjacencyBySource.has(edge.sourceIndex)) {
      forwardAdjacencyBySource.set(edge.sourceIndex, []);
    }
    forwardAdjacencyBySource.get(edge.sourceIndex).push(edge.targetIndex);
  });

  const branchDepthRows = branchEdges
    .filter((edge) => (
      Number.isInteger(edge.targetIndex) &&
      memberIndexes.has(edge.targetIndex) &&
      edge.targetIndex > decisionIndex
    ))
    .map((edge) => {
      const depthByInstruction = new Map([[edge.targetIndex, 1]]);
      const queue = [edge.targetIndex];
      for (let cursor = 0; cursor < queue.length; cursor += 1) {
        const sourceIndex = queue[cursor];
        const sourceDepth = depthByInstruction.get(sourceIndex);
        (forwardAdjacencyBySource.get(sourceIndex) ?? []).forEach((targetIndex) => {
          const nextDepth = sourceDepth + 1;
          if ((depthByInstruction.get(targetIndex) ?? Number.POSITIVE_INFINITY) <= nextDepth) return;
          depthByInstruction.set(targetIndex, nextDepth);
  ...
  if (branchDepthRows.length < 2) return [];

  const directBranchTargetIndexes = Array.from(
    new Set(branchDepthRows.map((row) => row.targetIndex)),
  );
  return directBranchTargetIndexes
    .map((targetIndex) => {
      const branchDepths = branchDepthRows
        .map((row) => ({
          edgeId: row.edgeId,
          branch: row.branch,
          branchTargetIndex: row.targetIndex,
          depth: row.depthByInstruction.get(targetIndex),
        }))
        .filter((row) => Number.isInteger(row.depth));
      if (branchDepths.length < 2) return null;

      const shortestDepth = Math.min(...branchDepths.map((row) => row.depth));
      const preferredDepth = Math.max(...branchDepths.map((row) => row.depth));
      if (preferredDepth <= shortestDepth) return null;

      const incomingSourceIndexes = Array.from(new Set(
        (analysis?.edges ?? [])
          .filter((edge) => (
            edge.targetIndex === targetIndex &&
            Number.isInteger(edge.sourceIndex) &&
            edge.sourceIndex !== decisionIndex &&
            memberIndexes.has(edge.sourceIndex) &&
            edge.sourceIndex < targetIndex
          ))
          .map((edge) => edge.sourceIndex),
      )).sort((left, right) => left - right);
      if (incomingSourceIndexes.length === 0) return null;

      return {
        targetIndex,
        shortestDepth,
        preferredDepth,
        incomingSourceIndexes,
        branchDepths,
      };
    })
    .filter(Boolean);
...
function applyGeneratedScanV2LevelMismatchCorrection({
  analysis,
  scanLayoutPlan,
  getNodeByIndex,
  stepY,
  ...
  const shiftedInstructionIndexes = new Set();
  const appliedRows = [];
  const evaluationRows = [];
  const setupRunIds = new Set(scanLayoutPlan?.dfsGrammar?.setupPrefixRunIds ?? []);
  const setupInstructionIndexes = new Set();
  (scanLayoutPlan?.runs ?? []).forEach((run) => {
    if (!setupRunIds.has(run.runId)) return;
    for (let index = run.startIndex; index <= run.endIndex; index += 1) {
      setupInstructionIndexes.add(index);
    }
  });

  const rawEdges = (analysis?.edges ?? []).filter((edge) => (
    Number.isInteger(edge.sourceIndex) &&
    Number.isInteger(edge.targetIndex)
  ));
  const scanEdgeById = new Map(
    (scanLayoutPlan?.edges ?? []).map((edge) => [edge.edgeId, edge]),
  );
  const assignmentByDecisionIndex = new Map(
    (scanLayoutPlan?.dfsGrammar?.diamondPortAssignments ?? [])
      .map((assignment) => [assignment.decisionIndex, assignment]),
  );
  const incidentEdgeCountByInstruction = new Map();
  rawEdges.forEach((edge) => {
    incidentEdgeCountByInstruction.set(
      edge.sourceIndex,
      (incidentEdgeCountByInstruction.get(edge.sourceIndex) ?? 0) + 1,
    );
    incidentEdgeCountByInstruction.set(
      edge.targetIndex,
      (incidentEdgeCountByInstruction.get(edge.targetIndex) ?? 0) + 1,
    );
  });
  const backwardTargetIndexes = new Set(
    rawEdges
      .filter((edge) => edge.targetIndex <= edge.sourceIndex)
      .map((edge) => edge.targetIndex),
  );

  const markNodeShift = (node, instructionIndex, nextY, reason) => {
    if (
      !node ||
    ...
    shiftedInstructionIndexes.add(instructionIndex);
    node.generatedScanV2LevelConsistency = {
      reason,
      previousY: Number(previousY.toFixed(2)),
      adjustedY: node.y,
    };
    return true;
  };

  (scanLayoutPlan?.dfsGrammar?.localMotifs ?? []).forEach((motif) => {
    const decisionIndex = motif.decisionIndex;
    const memberIndexes = new Set(motif.memberInstructionIndexes ?? []);
    if (!memberIndexes.has(decisionIndex)) return;

    const forwardEdges = rawEdges.filter((edge) => (
      memberIndexes.has(edge.sourceIndex) &&
      memberIndexes.has(edge.targetIndex) &&
      edge.targetIndex > edge.sourceIndex
    ));
    const forwardEdgesBySource = new Map();
    const incomingForwardEdgesByTarget = new Map();
    forwardEdges.forEach((edge) => {
      if (!forwardEdgesBySource.has(edge.sourceIndex)) {
        forwardEdgesBySource.set(edge.sourceIndex, []);
      }
      forwardEdgesBySource.get(edge.sourceIndex).push(edge);
      if (!incomingForwardEdgesByTarget.has(edge.targetIndex)) {
        incomingForwardEdgesByTarget.set(edge.targetIndex, []);
      }
      incomingForwardEdgesByTarget.get(edge.targetIndex).push(edge);
    });

    const branchEdges = (forwardEdgesBySource.get(decisionIndex) ?? [])
      .filter((edge) => memberIndexes.has(edge.targetIndex));
    if (branchEdges.length < 2) return;

    const branchDepthRows = branchEdges.map((branchEdge) => {
      const depthByInstruction = new Map([[branchEdge.targetIndex, 1]]);
      const queue = [branchEdge.targetIndex];
      for (let cursor = 0; cursor < queue.length; cursor += 1) {
        const sourceIndex = queue[cursor];
        const sourceDepth = depthByInstruction.get(sourceIndex);
        (forwardEdgesBySource.get(sourceIndex) ?? []).forEach((edge) => {
          const nextDepth = sourceDepth + 1;
          if ((depthByInstruction.get(edge.targetIndex) ?? Number.POSITIVE_INFINITY) <= nextDepth) {
            return;
          }
          depthByInstruction.set(edge.targetIndex, nextDepth);
          queue.push(edge.targetIndex);
        });
      }
      return {
        edgeId: branchEdge.id,
        branch: branchEdge.branch ?? null,
        branchTargetIndex: branchEdge.targetIndex,
        depthByInstruction,
      };
    });

    const sharedTargetIndexes = Array.from(memberIndexes)
      .filter((targetIndex) => (
        targetIndex !== decisionIndex &&
        branchDepthRows.every((row) => row.depthByInstruction.has(targetIndex))
      ));
    const firstSharedTargetIndexes = sharedTargetIndexes.filter((targetIndex) => (
      !(incomingForwardEdgesByTarget.get(targetIndex) ?? []).some((edge) => (
        edge.sourceIndex !== decisionIndex &&
        sharedTargetIndexes.includes(edge.sourceIndex)
      ))
    ));

    firstSharedTargetIndexes.forEach((targetIndex) => {
      const targetNode = getNodeByIndex(targetIndex);
      const branchDepths = branchDepthRows.map((row) => ({
        edgeId: row.edgeId,
        branch: row.branch,
        branchTargetIndex: row.branchTargetIndex,
        depth: row.depthByInstruction.get(targetIndex),
      }));
      const shortestDepth = Math.min(...branchDepths.map((row) => row.depth));
      const deepestDepth = Math.max(...branchDepths.map((row) => row.depth));
      const directAssignment = assignmentByDecisionIndex.get(decisionIndex);
      const directBranchAssignment = (directAssignment?.branchAssignments ?? [])
        .find((assignment) => assignment.targetIndex === targetIndex) ?? null;
      const targetIncomingRows = incomingForwardEdgesByTarget.get(targetIndex) ?? [];
      const deeperPredecessorRows = targetIncomingRows
        .filter((edge) => edge.sourceIndex !== decisionIndex)
        .map((edge) => {
          const sourceDepth = Math.max(
            ...branchDepthRows
              .map((row) => row.depthByInstruction.get(edge.sourceIndex))
              .filter(Number.isInteger),
          );
          return { edge, sourceDepth };
        })
        .filter((row) => Number.isFinite(row.sourceDepth))
        .sort((left, right) => right.sourceDepth - left.sourceDepth);
      const deeperPredecessorRow = deeperPredecessorRows[0] ?? null;
      const deeperPredecessorIndex = deeperPredecessorRow?.edge.sourceIndex ?? null;
      const deeperPredecessorNode = Number.isInteger(deeperPredecessorIndex)
        ? getNodeByIndex(deeperPredecessorIndex)
        : null;
      const targetScanEdges = targetIncomingRows
        .map((edge) => scanEdgeById.get(edge.id) ?? null)
        .filter(Boolean);

      let skippedReason = null;
      if (deepestDepth <= shortestDepth) {
        skippedReason = "noBranchDepthMismatch";
      } else if (!targetNode || !Number.isFinite(targetNode.y)) {
        skippedReason = "mergeTargetCoordinatesUnavailable";
      } else if (!deeperPredecessorNode || !Number.isFinite(deeperPredecessorNode.y)) {
        skippedReason = "deeperPredecessorCoordinatesUnavailable";
      } else if (setupInstructionIndexes.has(targetIndex)) {
        skippedReason = "setupPrefixTarget";
      } else if (backwardTargetIndexes.has(targetIndex)) {
        skippedReason = "loopBackOrAncestorTarget";
      } else if (
        directBranchAssignment?.overrideReason === "parentOrAncestorRegionTarget" ||
        targetScanEdges.some((edge) => (
          edge.edgeClass === "regionExit" ||
          edge.edgeClass === "exitTailTransfer" ||
          edge.proposedLaneType === "outerContinuation"
        ))
      ) {
        skippedReason = "parentFrameConnector";
      } else if (
        analysis?.nodeRoles?.[targetIndex] === "conditionalJump" &&
        (incidentEdgeCountByInstruction.get(targetIndex) ?? 0) >= 4
      ) {
        skippedReason = "highDegreeDiamondTarget";
      }

      const oldY = Number.isFinite(targetNode?.y) ? Number(targetNode.y.toFixed(2)) : null;
      const requiredMinY = Number.isFinite(deeperPredecessorNode?.y)
        ? Number((deeperPredecessorNode.y + stepY).toFixed(2))
        : null;
      const evaluation = {
        motifId: motif.motifId,
        diamondIndex: decisionIndex,
        mergeTargetIndex: targetIndex,
        deeperPredecessorIndex,
        shortestDepth,
        deepestDepth,
        oldY,
        requiredMinY,
      ...
      if (skippedReason || !markNodeShift(
        targetNode,
        targetIndex,
        requiredMinY,
        "deeperLocalBranchMerge",
      )) {
        if (!skippedReason) evaluation.skippedReason = "alreadyHasMergeLevelClearance";
        if (includeVerboseDiagnostics) evaluationRows.push(evaluation);
        return;
      }

      const propagatedIndexes = [];
      ...
      for (let cursor = 0; cursor < queue.length; cursor += 1) {
        ...
        if (analysis?.nodeRoles?.[sourceIndex] === "conditionalJump") continue;
        const outgoingRows = forwardEdgesBySource.get(sourceIndex) ?? [];
        if (outgoingRows.length !== 1) continue;
        const nextIndex = outgoingRows[0].targetIndex;
        if ((incomingForwardEdgesByTarget.get(nextIndex) ?? []).length !== 1) continue;
        if (analysis?.nodeRoles?.[nextIndex] !== "action") continue;
        const sourceNode = getNodeByIndex(sourceIndex);
        const nextNode = getNodeByIndex(nextIndex);
        if (!sourceNode || !nextNode) continue;
        if (markNodeShift(
          nextNode,
          nextIndex,
          sourceNode.y + stepY,
          "localMergeContinuationClearance",
        )) {
          propagatedIndexes.push(nextIndex);
        }
      ...
      if (includeVerboseDiagnostics) evaluationRows.push(appliedRow);
    });
  });

  return {
    shiftedInstructionIndexes,
    appliedRows,
    evaluationRows,
  };
}
...
function createGeneratedScanV2DiagnosticsDefaults() {
  return {
    enabled: false,
    skippedReason: null,
    placementDriver: "none",
    cellBoxesDrivingPlacement: false,
    nodesMissingCell: 0,
    fallbackReason: null,
    percentNodesOnMainSpine: 0,
    transferCellCollapsedOntoMainSpine: false,
    transferCellCenterX: null,
    transferCellWidth: null,
    transferCellOffsetFromMainSpine: null,
    transferCellBridgeBetweenSiblingIslands: false,
    transferCellWarnings: [],
    percentTransferNodesOnMainSpine: 0,
    siblingLoopIslandsShareSameGridColumn: false,
    exitOutsideOuter: false,
    exitCorridorGapFromOuter: null,
    totalDiagramHeight: 0,
    totalDiagramWidth: 0,
    cellPackingApplied: false,
    ...
    cellsExpandedOnlyForPaddingCount: 0,
    cellsWithExcessiveWhitespace: [],
    verticalNormalizationApplied: false,
    verticalNormalizationShiftY: 0,
    levelConsistencyAdjustmentsApplied: 0,
    levelConsistencyAdjustments: [],
    levelConsistencyEvaluations: [],
    localContinuationLaneAdjustmentsApplied: 0,
    localContinuationLaneAdjustments: [],
    localContinuationCenterPlacementRows: [],
    ownershipTokenPropagationRows: [],
    instructionIndexVerticalBiasCount: 0,
    neutralizedInstructionIndexVerticalBiasCount: 0,
    neutralizedInstructionIndexVerticalBiasRows: [],
    framesOrderedByInstructionIndexCount: 0,
    ...
    suspiciousLateInstructionDownshiftEdges: [],
    suspiciousLongEdgesCausedByIndexOrder: [],
    verticalStretchGapCount: 0,
    suspiciousLongEdgeCount: 0,
    overInflatedFrameCount: 0,
    maxReservedToNodeHeightRatio: 0,
    maxReservedToNodeWidthRatio: 0,
    maxLoopEnvelopeToBodyHeightRatio: 0,
    frameEnvelopeInflationRows: [],
    verticalGapConstraintRows: [],
    safetyOverrideGeometryRows: [],
    rowPackingRows: [],
    localFrameLayoutAvailable: false,
    localFrameCount: 0,
    localFrameCommitCount: 0,
    localFrameLayout: null,
    branchChildFrameCandidateCount: 0,
    branchChildFrameMaterializedCount: 0,
    branchChildFrameSkippedCount: 0,
    mergeChildFrameCandidateCount: 0,
    mergeChildFrameMaterializedCount: 0,
    exitTailFrameCandidateCount: 0,
    exitTailFrameMaterializedCount: 0,
    branchChildFrameCandidates: [],
    centerSpinePlacementCount: 0,
    centerFallbackCount: 0,
    unknownCenterFallbackCount: 0,
    centerSpinePlacementRows: [],
    runSegmentationSummary: {},
    runsSplitCount: 0,
    coarseRunWarningCount: 0,
    failedContainmentCheckCount: 0,
    failedContainmentChecks: [],
    regionBoxSourceCounts: {},
    parentBoxExpandedForChildren: false,
    parentRegionExpansionStats: [],
    regionBoxes: [],
    nullRegionBoxCount: 0,
    regionContainmentChecks: [],
    hierarchyCoherent: false,
    runBoxes: [],
    nodeCoordinates: [],
    edgesUsingSimpleLocalRouting: 0,
    edgesUsingOuterCorridor: 0,
    edgesDeferredOrUnresolved: [],
    suggestedMajorLayoutSkeleton: null,
  };
}

function resolveGeneratedScanV2ComponentLane({
  component,
  assignedLaneX = null,
  parentRegionLaneX = null,
  branchLaneX = null,
  globalCenterX = 0,
  exitCorridorX = null,
  islandX = null,
  ...
  const placementIntent = String(component?.placementIntent ?? "").trim();
  const isOffGlobalCenter = (value) => (
    Number.isFinite(value) &&
    Math.abs(value - globalCenterX) > 0.5
  );
  const selectLane = (laneX, reason, centerReason = null) => ({
    laneX: Number.isFinite(laneX) ? Number(laneX.toFixed(2)) : globalCenterX,
    reason,
    centerReason,
  });

  if (Number.isFinite(branchLaneX)) {
    return selectLane(branchLaneX, "branchLane");
  }
  if (role === "setupRun") {
    return selectLane(globalCenterX, "setupPrefix", "setupPrefix");
  }
  if (placementIntent === "outer exit corridor" || role === "exitTailRun") {
    return selectLane(
      isOffGlobalCenter(assignedLaneX) ? assignedLaneX : exitCorridorX,
      "sideOrExitLane",
    );
  }
  if (placementIntent === "right island") {
    return selectLane(
      isOffGlobalCenter(assignedLaneX) ? assignedLaneX : continuationX,
      "rightIslandLane",
    );
  }
  if (placementIntent === "left island") {
    return selectLane(
      isOffGlobalCenter(assignedLaneX) ? assignedLaneX : -Math.abs(islandX ?? 0),
      "leftIslandLane",
    );
  }
  if (placementIntent === "inner island") {
    return selectLane(
      isOffGlobalCenter(assignedLaneX) ? assignedLaneX : parentRegionLaneX,
      "innerRegionLane",
    );
  }
  if (placementIntent === "loop return lane") {
    return selectLane(
      isOffGlobalCenter(assignedLaneX) ? assignedLaneX : -Math.abs(islandX ?? 0),
      "loopReturnLane",
    );
  }
  if (Number.isFinite(parentRegionLaneX)) {
    return selectLane(parentRegionLaneX, "parentRegionLocalFrame");
  }
  if (isOffGlobalCenter(assignedLaneX)) {
    return selectLane(assignedLaneX, "assignedStructuralLane");
  }
  if (role === "mergeRun") {
    return selectLane(globalCenterX, "confirmedMerge", "trueMerge");
  }
  if (role === "continuationRun" || role === "loopBodyRun") {
    return selectLane(globalCenterX, "mainSpineContinuation", "mainContinuation");
  }
  if (Number.isFinite(assignedLaneX)) {
    return selectLane(assignedLaneX, "noLaneAssigned", "noLaneAssigned");
  }
  return selectLane(globalCenterX, "unknownFallback", "unknownFallback");
}

function buildGeneratedScanV2LocalFrameLayout({
  layoutPlan,
  scanLayoutPlan,
  regionBoxes = [],
  getNodeByIndex,
  ...
  const programLength = layoutPlan.analysis?.program?.length ?? 0;
  const instructionIndexes = rangeIndexes(0, Math.max(-1, programLength - 1))
    .filter((instructionIndex) => Boolean(getNodeByIndex(instructionIndex)));
  if (instructionIndexes.length === 0) {
    return {
      available: false,
      skippedReason: "instructionNodesUnavailable",
      frameCount: 0,
      rootFrame: null,
      frameRows: [],
    };
  ...
  const rawEdges = (layoutPlan.analysis?.edges ?? []).filter(
    (edge) => Number.isInteger(edge.sourceIndex),
  );
  const descriptors = [{
    frameId: "scan-frame:root",
    kind: "root",
    sourceId: "root",
    startIndex: instructionIndexes[0],
    endIndex: instructionIndexes.at(-1),
    parentFrameId: null,
    closureEdgeId: null,
    substructureRole: "root",
  }];
  (scanLayoutPlan?.regions ?? []).forEach((region) => {
    descriptors.push({
      frameId: `scan-frame:region:${region.regionId}`,
      kind: "region",
      sourceId: region.regionId,
      startIndex: region.startIndex,
      endIndex: region.endIndex,
      parentFrameId: region.parentRegionId
        ? `scan-frame:region:${region.parentRegionId}`
        : "scan-frame:root",
      closureEdgeId: region.closureEdgeId ?? null,
      substructureRole: region.suggestedVisualRole ?? "loopFrame",
    });
  });
  (scanLayoutPlan?.runs ?? []).forEach((run) => {
    descriptors.push({
      frameId: `scan-frame:run:${run.runId}`,
      kind: "run",
      sourceId: run.runId,
      startIndex: run.startIndex,
      endIndex: run.endIndex,
      parentFrameId: run.parentRegionId
        ? `scan-frame:region:${run.parentRegionId}`
        : "scan-frame:root",
      closureEdgeId: run.closureEdgeId ?? null,
      substructureRole: run.role ?? "run",
    });
  });

  const descriptorById = new Map(descriptors.map((descriptor) => [descriptor.frameId, descriptor]));
  const childrenByParentId = new Map();
  const rebuildChildrenByParentId = () => {
    childrenByParentId.clear();
    descriptors.forEach((descriptor) => {
      if (!descriptor.parentFrameId || !descriptorById.has(descriptor.parentFrameId)) return;
      if (!childrenByParentId.has(descriptor.parentFrameId)) {
        childrenByParentId.set(descriptor.parentFrameId, []);
      }
      childrenByParentId.get(descriptor.parentFrameId).push(descriptor);
    });
  ...
  const isInside = (descriptor, instructionIndex) => (
    Number.isInteger(instructionIndex) &&
    descriptor.startIndex <= instructionIndex &&
    instructionIndex <= descriptor.endIndex
  );
  const getInternalIndexes = (descriptor) => (
    instructionIndexes.filter((instructionIndex) => isInside(descriptor, instructionIndex))
  );
  const ownerFrameIdByInstruction = new Map();
  instructionIndexes.forEach((instructionIndex) => {
    const owner = descriptors
      .filter((descriptor) => (
        descriptor.kind !== "root" &&
        isInside(descriptor, instructionIndex)
      ))
      .sort((left, right) => (
        (left.kind === "run" ? -1 : 1) ||
        ((left.endIndex - left.startIndex) - (right.endIndex - right.startIndex))
      ))[0] ?? descriptors[0];
    ownerFrameIdByInstruction.set(instructionIndex, owner.frameId);
  });
  const getOwnedIndexes = (descriptor) => (
    instructionIndexes.filter(
      (instructionIndex) => ownerFrameIdByInstruction.get(instructionIndex) === descriptor.frameId,
    )
  );
  const getBounds = (internalIndexes, frameKind, resolveNodeX = null, resolveNodeY = null) => {
    const nodes = internalIndexes.map((instructionIndex) => getNodeByIndex(instructionIndex)).filter(Boolean);
    if (nodes.length === 0) return null;
    const horizontalPadding = frameKind === "run" ? 0 : layout.sideRouteGap;
    const verticalPadding = frameKind === "run" ? 0 : layout.verticalGap * 0.5;
    return {
      left: Number((Math.min(...nodes.map((node) => {
        const { width } = getNodeSize(node, layout);
        const x = resolveNodeX ? resolveNodeX(node.instructionIndex) : node.x;
        return x - width / 2;
      })) - horizontalPadding).toFixed(2)),
      right: Number((Math.max(...nodes.map((node) => {
        const { width } = getNodeSize(node, layout);
        const x = resolveNodeX ? resolveNodeX(node.instructionIndex) : node.x;
        return x + width / 2;
      })) + horizontalPadding).toFixed(2)),
      top: Number((Math.min(...nodes.map((node) => {
        const { height } = getNodeSize(node, layout);
        const y = resolveNodeY ? resolveNodeY(node.instructionIndex) : node.y;
        return y - height / 2;
      })) - verticalPadding).toFixed(2)),
      bottom: Number((Math.max(...nodes.map((node) => {
        const { height } = getNodeSize(node, layout);
        const y = resolveNodeY ? resolveNodeY(node.instructionIndex) : node.y;
        return y + height / 2;
      })) + verticalPadding).toFixed(2)),
  ...
  const plannedFrameOffsetXById = new Map();
  const plannedFrameOffsetYById = new Map();
  const branchFramePlacementRows = [];
  const branchChildFrameCandidates = [];
  const getEffectiveNodeX = (instructionIndex, extraFrameId = null, extraOffsetX = 0) => {
    const node = getNodeByIndex(instructionIndex);
    if (!node) return Number.NaN;
    const extraFrame = extraFrameId ? descriptorById.get(extraFrameId) ?? null : null;
    const plannedOffsetX = descriptors.reduce((sum, descriptor) => (
      isInside(descriptor, instructionIndex)
        ? sum + (plannedFrameOffsetXById.get(descriptor.frameId) ?? 0)
        : sum
    ), 0);
    return node.x + plannedOffsetX + (
      extraFrame && isInside(extraFrame, instructionIndex)
        ? extraOffsetX
        : 0
    );
  };
  const getEffectiveNodeY = (instructionIndex, extraFrameId = null, extraOffsetY = 0) => {
    const node = getNodeByIndex(instructionIndex);
    if (!node) return Number.NaN;
    const extraFrame = extraFrameId ? descriptorById.get(extraFrameId) ?? null : null;
    const plannedOffsetY = descriptors.reduce((sum, descriptor) => (
      isInside(descriptor, instructionIndex)
        ? sum + (plannedFrameOffsetYById.get(descriptor.frameId) ?? 0)
        : sum
    ), 0);
    return node.y + plannedOffsetY + (
      extraFrame && isInside(extraFrame, instructionIndex)
        ? extraOffsetY
        : 0
    );
  };
  const activeRegionBoxById = new Map(
    (regionBoxes ?? [])
      .filter((box) => (
        box?.materialized &&
      ...
      .map((box) => [box.regionId, box]),
  );
  const getContainingActiveRegionBoxes = (descriptor) => (
    descriptors
      .filter((candidate) => (
        candidate.kind === "region" &&
        candidate.startIndex <= descriptor.startIndex &&
        descriptor.endIndex <= candidate.endIndex
      ))
      .map((candidate) => activeRegionBoxById.get(candidate.sourceId) ?? null)
      .filter(Boolean)
  );
  ...
  const getDescriptorDepth = (descriptor) => {
    let depth = 0;
    let cursor = descriptor;
    const guard = new Set();
    while (cursor?.parentFrameId && !guard.has(cursor.parentFrameId)) {
      guard.add(cursor.parentFrameId);
      cursor = descriptorById.get(cursor.parentFrameId) ?? null;
      if (cursor) depth += 1;
    }
    return depth;
  };
  ...
  const regionDescriptors = descriptors.filter((descriptor) => descriptor.kind === "region");
  const getFrameAncestry = (frameId) => {
    const ancestry = [];
    const guard = new Set();
    let cursor = descriptorById.get(frameId) ?? null;
    while (cursor && !guard.has(cursor.frameId)) {
      guard.add(cursor.frameId);
      ancestry.push(cursor);
      cursor = cursor.parentFrameId
        ? descriptorById.get(cursor.parentFrameId) ?? null
        : null;
    }
    return ancestry;
  };
  const getNearestCommonAncestorFrameId = (leftFrameId, rightFrameId) => {
    const rightAncestorIds = new Set(
      getFrameAncestry(rightFrameId).map((descriptor) => descriptor.frameId),
    );
    return getFrameAncestry(leftFrameId)
      .find((descriptor) => rightAncestorIds.has(descriptor.frameId))
      ?.frameId ?? null;
  };
  const analyzeBranchChildScope = ({
    assignment,
    targetDescriptor,
    decisionOwnerFrameId,
    previousGeneratedOwner,
    candidateKind,
  }) => {
    const decisionIndex = assignment.decisionIndex;
    const targetIndex = targetDescriptor.startIndex;
    const lowerIndex = Math.min(decisionIndex, targetIndex);
    const upperIndex = Math.max(decisionIndex, targetIndex);
    const regionsCrossedBetweenEndpoints = regionDescriptors.filter((descriptor) => {
      const containsDecision = isInside(descriptor, decisionIndex);
      const containsTarget = isInside(descriptor, targetIndex);
      const liesStrictlyBetween = (
        lowerIndex < descriptor.startIndex &&
        descriptor.endIndex < upperIndex
      );
      return liesStrictlyBetween || containsDecision !== containsTarget;
    });
    const interveningLoopRegionCount = regionsCrossedBetweenEndpoints
      .filter((descriptor) => Boolean(descriptor.closureEdgeId))
      .length;
    const interveningRegionBoundaryCount = regionsCrossedBetweenEndpoints.length;
    const structuralDistanceFromDecision = Math.abs(targetIndex - decisionIndex);
    const nearestCommonAncestorFrameId = getNearestCommonAncestorFrameId(
      decisionOwnerFrameId,
      previousGeneratedOwner,
    );
    // One or two crossed scopes can be the local branch body itself. More than that
    // indicates that an early decision is reaching across an enclosing computation phase.
    const maxLocalInterveningRegionBoundaries = 2;
    const crossesLoopScopeBoundary = (
      interveningLoopRegionCount > maxLocalInterveningRegionBoundaries
    );
    const crossesAncestorTransition = regionsCrossedBetweenEndpoints
      .filter((descriptor) => (
        isInside(descriptor, decisionIndex) &&
        !isInside(descriptor, targetIndex)
      ))
      .length > 1;
    const ownerScopeMismatch = (
      nearestCommonAncestorFrameId === "scan-frame:root" &&
      interveningRegionBoundaryCount > maxLocalInterveningRegionBoundaries
    );
    const crossesTerminalPhaseBoundary = (
      candidateKind === "exitTail" &&
      (
        interveningRegionBoundaryCount > maxLocalInterveningRegionBoundaries ||
        (
          ownerScopeMismatch &&
          structuralDistanceFromDecision > Math.max(12, Math.ceil(programLength / 3))
        )
      )
    );
    const scopeBoundarySkipReason = crossesTerminalPhaseBoundary
      ? "terminalOutputTailNotLocal"
      : crossesLoopScopeBoundary
        ? "crossesLoopScopeBoundary"
        : crossesAncestorTransition
          ? "crossesAncestorTransition"
          : ownerScopeMismatch
            ? "ownerScopeMismatch"
            : null;
    return {
      nearestCommonAncestorFrameId,
      interveningLoopRegionCount,
      interveningRegionBoundaryCount,
      crossesAncestorTransition,
      crossesLoopScopeBoundary,
      crossesTerminalPhaseBoundary,
      structuralDistanceFromDecision,
      ownerScopeMismatch,
      scopeBoundarySkipReason,
    };
  };
  const attachedBranchDescriptorIds = new Set();
  const branchChildAssignments = (scanLayoutPlan?.dfsGrammar?.diamondPortAssignments ?? [])
    .flatMap((assignment) => (
      (assignment.branchAssignments ?? []).map((branch) => ({ assignment, branch }))
    ));
  branchChildAssignments.forEach(({ assignment, branch }) => {
    if (!Number.isInteger(branch.targetIndex)) return;
    const targetDescriptor = descriptors
      .filter((descriptor) => (
        descriptor.kind === "run" &&
        descriptor.startIndex === branch.targetIndex &&
        !isInside(descriptor, assignment.decisionIndex)
      ))
      .sort((left, right) => (
        (left.endIndex - left.startIndex) - (right.endIndex - right.startIndex)
      ))[0] ?? null;
    const decisionOwnerFrameId = ownerFrameIdByInstruction.get(assignment.decisionIndex) ?? null;
    const previousGeneratedOwner = targetDescriptor?.parentFrameId ?? null;
    const incomingEdges = incomingEdgesByTarget.get(branch.targetIndex) ?? [];
    const candidateKind = targetDescriptor?.substructureRole === "exitTailRun"
      ? "exitTail"
      : (
        targetDescriptor?.substructureRole === "mergeRun" || incomingEdges.length > 1
          ? "mergeContinuation"
          : "branchBody"
      );
    const scopeAnalysis = targetDescriptor && decisionOwnerFrameId && previousGeneratedOwner
      ? analyzeBranchChildScope({
        assignment,
        targetDescriptor,
        decisionOwnerFrameId,
        previousGeneratedOwner,
      ...
      : {
        nearestCommonAncestorFrameId: null,
        interveningLoopRegionCount: 0,
        interveningRegionBoundaryCount: 0,
        crossesAncestorTransition: false,
        crossesLoopScopeBoundary: false,
        crossesTerminalPhaseBoundary: false,
        structuralDistanceFromDecision: Number.isInteger(branch.targetIndex)
          ? Math.abs(branch.targetIndex - assignment.decisionIndex)
          : null,
        ownerScopeMismatch: false,
        scopeBoundarySkipReason: null,
      };
    const reportedCandidateKind = scopeAnalysis.crossesTerminalPhaseBoundary
      ? "terminalOutputTail"
      : candidateKind;
    const row = {
      decisionInstruction: assignment.decisionIndex,
      branchTargetInstruction: branch.targetIndex,
      branchDirection: branch.side,
      discoveredInstructionRange: targetDescriptor
        ? `${formatInstructionNodeId(targetDescriptor.startIndex)}-${formatInstructionNodeId(targetDescriptor.endIndex)}`
        : null,
      candidateKind: reportedCandidateKind,
      decisionFrameId: decisionOwnerFrameId,
      currentOwnerFrameId: previousGeneratedOwner,
      proposedOwnerFrameId: decisionOwnerFrameId,
      materializedFrameId: null,
      skipReason: null,
      previousGeneratedOwner,
      newGeneratedOwner: previousGeneratedOwner,
      intervalRunOwnershipWasPrevented: false,
      placedBeforeAbsoluteYOrdering: false,
      ...scopeAnalysis,
    };
    if (!targetDescriptor) {
      row.skipReason = "existingOwnerConflict";
    } else if (!decisionOwnerFrameId || !descriptorById.has(decisionOwnerFrameId)) {
      row.skipReason = "mergeOwnershipAmbiguous";
    } else if (attachedBranchDescriptorIds.has(targetDescriptor.frameId)) {
      row.skipReason = candidateKind === "mergeContinuation"
        ? "siblingBranchConflict"
        : "existingOwnerConflict";
    } else if (decisionOwnerFrameId === targetDescriptor.frameId) {
      row.skipReason = "existingOwnerConflict";
    } else if (row.scopeBoundarySkipReason) {
      row.skipReason = row.scopeBoundarySkipReason;
    } else {
      const decisionNode = getNodeByIndex(assignment.decisionIndex);
      const targetNode = getNodeByIndex(branch.targetIndex);
      const direction = branch.side === "left" ? -1 : 1;
      const preferredTargetX = decisionNode
        ? getEffectiveNodeX(assignment.decisionIndex) + direction * assignment.branchClearance
        : Number.NaN;
      const incomingPredecessorYValues = incomingEdges
        .map((edge) => getEffectiveNodeY(edge.sourceIndex))
        .filter(Number.isFinite);
      const preferredTargetY = decisionNode
        ? (
          candidateKind === "mergeContinuation" && incomingPredecessorYValues.length > 0
            ? Math.max(
              getEffectiveNodeY(assignment.decisionIndex) + layout.actionNodeHeight + layout.verticalGap,
              ...incomingPredecessorYValues,
            )
            : getEffectiveNodeY(assignment.decisionIndex) + layout.actionNodeHeight + layout.verticalGap
        )
        : Number.NaN;
      const offsetX = targetNode
        ? Number((preferredTargetX - getEffectiveNodeX(branch.targetIndex)).toFixed(2))
        : Number.NaN;
      const offsetY = targetNode
        ? Number((preferredTargetY - getEffectiveNodeY(branch.targetIndex)).toFixed(2))
        : Number.NaN;
      const activeRegionBoxes = getContainingActiveRegionBoxes(targetDescriptor);
      const proposedBounds = getBounds(
        getInternalIndexes(targetDescriptor),
        targetDescriptor.kind,
        (instructionIndex) => getEffectiveNodeX(instructionIndex, targetDescriptor.frameId, offsetX),
        (instructionIndex) => getEffectiveNodeY(instructionIndex, targetDescriptor.frameId, offsetY),
      );
      if (
        !decisionNode ||
        !targetNode ||
        ...
        row.skipReason = "unknown";
      } else if (
        activeRegionBoxes.length > 0 &&
        !activeRegionBoxes.every((box) => containsBox(box, proposedBounds))
      ) {
        row.skipReason = "routingEnvelopeWouldBeViolated";
      } else {
        targetDescriptor.parentFrameId = decisionOwnerFrameId;
        targetDescriptor.branchDirection = branch.side;
        targetDescriptor.branchChildKind = candidateKind;
        plannedFrameOffsetXById.set(targetDescriptor.frameId, offsetX);
        plannedFrameOffsetYById.set(targetDescriptor.frameId, offsetY);
        attachedBranchDescriptorIds.add(targetDescriptor.frameId);
        row.materializedFrameId = targetDescriptor.frameId;
        row.newGeneratedOwner = decisionOwnerFrameId;
        row.intervalRunOwnershipWasPrevented = previousGeneratedOwner !== decisionOwnerFrameId;
        row.placedBeforeAbsoluteYOrdering = true;
      }
    }
    branchChildFrameCandidates.push(row);
  });
  rebuildChildrenByParentId();
  const branchFrameCandidates = (scanLayoutPlan?.dfsGrammar?.diamondPortAssignments ?? [])
    .flatMap((assignment) => (
      (assignment.branchAssignments ?? [])
        .filter((branch) => branch.preferLowerDiagonal && Number.isInteger(branch.targetIndex))
        .map((branch) => ({ assignment, branch }))
    ))
    .map(({ assignment, branch }) => {
      const descriptorsAtTarget = descriptors
        .filter((descriptor) => (
          descriptor.kind !== "root" &&
          descriptor.startIndex === branch.targetIndex &&
          !isInside(descriptor, assignment.decisionIndex)
      ...
      return {
        assignment,
        branch,
        descriptor: descriptorsAtTarget[0] ?? null,
      };
    })
    .filter(({ descriptor }) => descriptor)
    .sort((left, right) => (
      (getDescriptorDepth(left.descriptor) - getDescriptorDepth(right.descriptor)) ||
      (left.descriptor.startIndex - right.descriptor.startIndex)
    ));
  const branchDirectionByFrameId = new Map();
  branchFrameCandidates.forEach(({ branch, descriptor }) => {
    const currentDirection = branchDirectionByFrameId.get(descriptor.frameId) ?? null;
    if (!currentDirection) {
      branchDirectionByFrameId.set(descriptor.frameId, branch.side);
    } else if (currentDirection !== branch.side) {
      branchDirectionByFrameId.set(descriptor.frameId, "ambiguous");
    }
  });

  branchFrameCandidates.forEach(({ assignment, branch, descriptor }) => {
    const decisionNode = getNodeByIndex(assignment.decisionIndex);
    const targetNode = getNodeByIndex(branch.targetIndex);
    const incomingFrameEdges = rawEdges.filter((edge) => (
      !isInside(descriptor, edge.sourceIndex) &&
      isInside(descriptor, edge.targetIndex)
    ));
    const row = {
      frameId: descriptor.frameId,
      decisionIndex: assignment.decisionIndex,
    ...
    if (!decisionNode || !targetNode) {
      row.reason = "branchEndpointUnavailable";
    } else if (
      incomingFrameEdges.length !== 1 ||
      incomingFrameEdges[0].id !== branch.edgeId
    ) {
      row.reason = "ambiguousFrameEntry";
    } else {
      const direction = branch.side === "left" ? -1 : 1;
      const decisionX = getEffectiveNodeX(assignment.decisionIndex);
      const targetX = getEffectiveNodeX(branch.targetIndex);
      const preferredTargetX = decisionX + direction * assignment.branchClearance;
      const offsetX = Number((preferredTargetX - targetX).toFixed(2));
      const currentLaneDistance = direction * (targetX - decisionX);
      const activeRegionBoxes = getContainingActiveRegionBoxes(descriptor);
      const proposedBounds = getBounds(
        getInternalIndexes(descriptor),
        descriptor.kind,
        (instructionIndex) => getEffectiveNodeX(instructionIndex, descriptor.frameId, offsetX),
        getEffectiveNodeY,
      );
      if (currentLaneDistance >= assignment.branchClearance - 0.5) {
        row.reason = "branchLaneAlreadyPreserved";
      } else if (!Number.isFinite(offsetX) || !proposedBounds) {
        row.reason = "invalidProposedFrameGeometry";
      } else if (activeRegionBoxes.length === 0) {
        row.reason = "routingEnvelopeUnavailable";
      } else if (!activeRegionBoxes.every((box) => containsBox(box, proposedBounds))) {
        row.reason = "routingEnvelopeWouldBeViolated";
      } else {
        plannedFrameOffsetXById.set(descriptor.frameId, offsetX);
        row.status = "applied";
        row.reason = "decisionRelativeDiagonalBranchLane";
        row.offsetX = offsetX;
      }
    }
    branchFramePlacementRows.push(row);
  });

  const toRelativePoint = (node, origin, side = "center") => {
    const { width, height } = getNodeSize(node, layout);
    const nodeX = getEffectiveNodeX(node.instructionIndex);
    const nodeY = getEffectiveNodeY(node.instructionIndex);
    const x = side === "left"
      ? nodeX - width / 2
      : side === "right"
        ? nodeX + width / 2
    ...
    const y = side === "top"
      ...
      : side === "bottom"
        ...
        : nodeY;
    return [
      Number((x - origin.x).toFixed(2)),
      Number((y - origin.y).toFixed(2)),
    ];
  };

  let frameCount = 0;
  const frameRows = [];
  const buildFrame = (descriptor, guard = new Set()) => {
    if (guard.has(descriptor.frameId)) {
      throw new Error(`generatedScanV2LocalFrameCycle:${descriptor.frameId}`);
    }
    const nextGuard = new Set(guard);
    nextGuard.add(descriptor.frameId);
    const internalInstructionIndexes = getInternalIndexes(descriptor);
    const ownedInstructionIndexes = getOwnedIndexes(descriptor);
    const box = getBounds(internalInstructionIndexes, descriptor.kind, getEffectiveNodeX, getEffectiveNodeY);
    if (!box) {
      throw new Error(`generatedScanV2LocalFrameMissingBox:${descriptor.frameId}`);
    }
    const origin = { x: box.left, y: box.top };
    const entryInstructionIndexes = Array.from(new Set(
      rawEdges
        .filter((edge) => (
          !isInside(descriptor, edge.sourceIndex) &&
          isInside(descriptor, edge.targetIndex)
    ...
    if (descriptor.kind === "root" && entryInstructionIndexes.length === 0) {
      entryInstructionIndexes.push(instructionIndexes[0]);
    }
    const exitEdges = rawEdges.filter((edge) => (
      isInside(descriptor, edge.sourceIndex) &&
      !isInside(descriptor, edge.targetIndex)
    ));
    const loopBackEdges = rawEdges
      .filter((edge) => (
        isInside(descriptor, edge.sourceIndex) &&
        isInside(descriptor, edge.targetIndex) &&
        edge.targetIndex <= edge.sourceIndex
      ...
      .map((edge) => ({
        ...
        sourceIndex: edge.sourceIndex,
        targetIndex: edge.targetIndex,
        inheritedSide: getGeneratedScanV2NodeBranchSide(getNodeByIndex(edge.sourceIndex)),
      }));
    const ownedLoopBackEdges = descriptor.closureEdgeId
      ? loopBackEdges.filter((edge) => edge.edgeId === descriptor.closureEdgeId)
      : descriptor.kind === "root"
        ? loopBackEdges.filter((edge) => !descriptors.some((candidate) => (
          candidate.kind === "region" && candidate.closureEdgeId === edge.edgeId
        )))
        : [];
    const childFrames = (childrenByParentId.get(descriptor.frameId) ?? [])
      .map((childDescriptor) => buildFrame(childDescriptor, nextGuard));
    frameCount += 1;
    const frame = {
      frameId: descriptor.frameId,
      kind: descriptor.kind,
      sourceId: descriptor.sourceId,
      substructureRole: descriptor.substructureRole,
      parentFrameId: descriptor.parentFrameId,
      branchDirection: ["left", "right"].includes(descriptor.branchDirection)
        ? descriptor.branchDirection
        : (
          ["left", "right"].includes(branchDirectionByFrameId.get(descriptor.frameId))
            ? branchDirectionByFrameId.get(descriptor.frameId)
            : null
        ),
      origin,
      offsetFromParent: null,
      box,
      ownedNodePositions: ownedInstructionIndexes.map((instructionIndex) => {
        const node = getNodeByIndex(instructionIndex);
        return {
          instructionIndex,
          x: Number((getEffectiveNodeX(instructionIndex) - origin.x).toFixed(2)),
          y: Number((getEffectiveNodeY(instructionIndex) - origin.y).toFixed(2)),
        };
      }),
      entryPort: (() => {
        const entryNode = getNodeByIndex(entryInstructionIndexes[0]);
        return entryNode
          ? { instructionIndex: entryInstructionIndexes[0], point: toRelativePoint(entryNode, origin, "top") }
          : null;
      })(),
      exitPorts: exitEdges.map((edge) => {
        const sourceNode = getNodeByIndex(edge.sourceIndex);
        return {
          edgeId: edge.id,
          sourceIndex: edge.sourceIndex,
          targetIndex: Number.isInteger(edge.targetIndex) ? edge.targetIndex : null,
          point: sourceNode ? toRelativePoint(sourceNode, origin, "bottom") : null,
        };
      }),
      loopBackEdges,
      // Frame-level reservations are scaffolding for future local routing, not active routed corridors.
      reservedLoopCorridors: ownedLoopBackEdges.map((edge) => ({
        ...edge,
        status: "reservedForLocalFrameRouting",
      })),
      children: childFrames,
      ...(includeVerboseDiagnostics ? {
        interval: `${formatInstructionNodeId(descriptor.startIndex)}-${formatInstructionNodeId(descriptor.endIndex)}`,
        internalInstructionIndexes,
        ownedInstructionIndexes,
        nodePositions: internalInstructionIndexes.map((instructionIndex) => {
          const node = getNodeByIndex(instructionIndex);
          return {
            instructionIndex,
            x: Number((getEffectiveNodeX(instructionIndex) - origin.x).toFixed(2)),
        ...
        branchPlacement: branchFramePlacementRows.find((row) => row.frameId === descriptor.frameId) ?? null,
      } : {}),
    };
    frame.children.forEach((childFrame) => {
      childFrame.offsetFromParent = {
        x: Number((childFrame.origin.x - frame.origin.x).toFixed(2)),
        y: Number((childFrame.origin.y - frame.origin.y).toFixed(2)),
      };
    });
    if (includeVerboseDiagnostics) {
      frameRows.push({
        ...
        box: frame.box,
        entryPort: frame.entryPort,
        exitPortCount: frame.exitPorts.length,
        loopBackEdgeIds: frame.loopBackEdges.map((edge) => edge.edgeId),
        reservedLoopCorridorEdgeIds: frame.reservedLoopCorridors.map((corridor) => corridor.edgeId),
        childFrameIds: frame.children.map((child) => child.frameId),
        ownedInstructionIndexes,
        branchPlacement: frame.branchPlacement,
      });
    }
    return frame;
  };

  const rootFrame = buildFrame(descriptors[0]);
  return {
    available: true,
    skippedReason: null,
    frameCount,
    rootFrame,
    frameRows,
    branchChildFrameCandidateCount: branchChildFrameCandidates.length,
    branchChildFrameMaterializedCount:
      branchChildFrameCandidates.filter((row) => row.materializedFrameId).length,
    branchChildFrameSkippedCount:
      branchChildFrameCandidates.filter((row) => !row.materializedFrameId).length,
    mergeChildFrameCandidateCount:
      branchChildFrameCandidates.filter((row) => row.candidateKind === "mergeContinuation").length,
    mergeChildFrameMaterializedCount:
      branchChildFrameCandidates.filter((row) => (
        row.candidateKind === "mergeContinuation" && row.materializedFrameId
      )).length,
    exitTailFrameCandidateCount:
      branchChildFrameCandidates.filter((row) => row.candidateKind === "exitTail").length,
    exitTailFrameMaterializedCount:
      branchChildFrameCandidates.filter((row) => (
        row.candidateKind === "exitTail" && row.materializedFrameId
      )).length,
    ...(includeVerboseDiagnostics ? { branchFramePlacementRows, branchChildFrameCandidates } : {}),
  };
}

function commitGeneratedScanV2LocalFrameLayout(localFrameLayout, getNodeByIndex) {
  if (!localFrameLayout?.available || !localFrameLayout.rootFrame) return 0;
  const committedInstructionIndexes = new Set();
  const commitFrame = (frame, absoluteOrigin = frame.origin, parentAncestry = []) => {
    if (!Number.isFinite(absoluteOrigin?.x) || !Number.isFinite(absoluteOrigin?.y)) {
      throw new Error(`generatedScanV2LocalFrameInvalidOrigin:${frame.frameId}`);
    }
    const ancestry = [
      ...parentAncestry,
      {
        ...
        branchDirection: frame.branchDirection ?? null,
      },
    ];
    (frame.ownedNodePositions ?? []).forEach((position) => {
      const node = getNodeByIndex(position.instructionIndex);
      const x = absoluteOrigin.x + position.x;
      const y = absoluteOrigin.y + position.y;
      if (!node || !Number.isFinite(x) || !Number.isFinite(y)) {
        throw new Error(`generatedScanV2LocalFrameInvalidNodePosition:${frame.frameId}:${position.instructionIndex}`);
      }
      node.x = Number(x.toFixed(2));
      node.y = Number(y.toFixed(2));
      node.generatedScanV2LocalFrameId = frame.frameId;
      node.generatedScanV2LocalFrameAncestry = ancestry;
      committedInstructionIndexes.add(position.instructionIndex);
    });
    (frame.children ?? []).forEach((childFrame) => {
      const offset = childFrame.offsetFromParent;
      if (!Number.isFinite(offset?.x) || !Number.isFinite(offset?.y)) {
        throw new Error(`generatedScanV2LocalFrameInvalidChildOffset:${childFrame.frameId}`);
      }
      commitFrame(childFrame, {
        x: absoluteOrigin.x + offset.x,
        y: absoluteOrigin.y + offset.y,
      }, ancestry);
    });
  };
  commitFrame(localFrameLayout.rootFrame);
  return committedInstructionIndexes.size;
}

function applyGeneratedScanV2LocalContinuationLanePersistence({
  analysis,
  scanLayoutPlan,
  getNodeByIndex,
  globalCenterX = 0,
  ...
  rawEdges.forEach((edge) => {
    if (!incomingEdgesByTarget.has(edge.targetIndex)) {
      incomingEdgesByTarget.set(edge.targetIndex, []);
    }
    incomingEdgesByTarget.get(edge.targetIndex).push(edge);
    if (!outgoingEdgesBySource.has(edge.sourceIndex)) {
      outgoingEdgesBySource.set(edge.sourceIndex, []);
    }
    outgoingEdgesBySource.get(edge.sourceIndex).push(edge);
  });
  ...
  const confirmedMergeIndexes = new Set(
    (scanLayoutPlan?.dfsGrammar?.localMotifs ?? [])
      .map((motif) => motif.mergeIndex)
      .filter(Number.isInteger),
  );
  const backwardTargetIndexes = new Set(
    rawEdges
  ...
  const innermostRegionByInstruction = new Map();
  (scanLayoutPlan?.regions ?? []).forEach((region) => {
    if (!Number.isInteger(region.startIndex) || !Number.isInteger(region.endIndex)) return;
    for (let index = region.startIndex; index <= region.endIndex; index += 1) {
      const previous = innermostRegionByInstruction.get(index) ?? null;
      if (!previous || (region.nestingDepth ?? region.depth ?? 0) > (previous.nestingDepth ?? previous.depth ?? 0)) {
        innermostRegionByInstruction.set(index, region);
      }
    }
  });

  const getLocalLane = (node) => {
    if (!node || !Number.isFinite(node.x)) return null;
    const branchLane = node.generatedScanV2BranchLane ?? null;
    if (branchLane?.side === "left" || branchLane?.side === "right") {
      return {
        laneX: node.x,
        side: branchLane.side,
        source: "diamondBranchLane",
        decisionIndex: branchLane.decisionIndex ?? null,
      };
    }
    const motifRole = node.generatedScanV2LocalMotifMember?.role ?? null;
    if (motifRole === "fallthroughArm" || motifRole === "jumpArm") {
      return {
        laneX: node.x,
        side: motifRole === "fallthroughArm" ? "left" : "right",
        source: "localMotifArm",
        decisionIndex: null,
      };
    }
    const continuationLane = node.generatedScanV2ContinuationLane ?? null;
    if (Number.isFinite(continuationLane?.laneX)) {
      return {
        laneX: continuationLane.laneX,
        side: continuationLane.side ?? null,
        source: "predecessorContinuationLane",
        decisionIndex: continuationLane.decisionIndex ?? null,
      };
    }
    return null;
  };
  const targetHasExplicitLane = (node) => (
    Boolean(node?.generatedScanV2BranchLane) ||
    node?.generatedScanV2LocalMotifMember?.role === "fallthroughArm" ||
    node?.generatedScanV2LocalMotifMember?.role === "jumpArm"
  );
  const getOwnershipToken = (node) => {
    if (node?.generatedScanV2OwnershipToken?.side === "left" ||
      node?.generatedScanV2OwnershipToken?.side === "right") {
      return node.generatedScanV2OwnershipToken;
    }
    const localLane = getLocalLane(node);
    if (localLane?.side !== "left" && localLane?.side !== "right") return null;
    return {
      originDecisionInstruction: localLane.decisionIndex ?? null,
      side: localLane.side,
      branchTargetInstruction: node?.instructionIndex ?? null,
      tokenKind: "branch",
    };
  };
  ...
  let appliedCount = 0;
  const appliedRows = [];
  const centerPlacementRows = [];
  const ownershipTokenRows = [];
  rawEdges.forEach((edge) => {
    const sourceNode = getNodeByIndex(edge.sourceIndex);
    const targetNode = getNodeByIndex(edge.targetIndex);
    const inheritedLane = getLocalLane(sourceNode);
    const ownershipTokenBefore = getOwnershipToken(sourceNode);
    if (!inheritedLane && !ownershipTokenBefore) return;

    const scanEdge = scanEdgeById.get(edge.id) ?? null;
    const sourceRegion = innermostRegionByInstruction.get(edge.sourceIndex) ?? null;
    const targetRegion = innermostRegionByInstruction.get(edge.targetIndex) ?? null;
    const sourceRegionId = sourceRegion?.regionId ?? sourceRegion?.id ?? null;
    const targetRegionId = targetRegion?.regionId ?? targetRegion?.id ?? null;
    let skippedReason = null;
    if (!sourceNode || !targetNode || !Number.isFinite(targetNode.x)) {
      skippedReason = "continuationCoordinatesUnavailable";
    } else if (analysis?.nodeRoles?.[edge.sourceIndex] === "conditionalJump") {
      skippedReason = "sourceIsDiamondSplit";
    } else if ((outgoingEdgesBySource.get(edge.sourceIndex) ?? []).length !== 1) {
      skippedReason = "sourceHasMultipleOutgoingEdges";
    } else if (edge.type !== "fallthrough" || edge.targetIndex !== edge.sourceIndex + 1) {
      skippedReason = "notOrdinaryLinearFallthrough";
    } else if ((incomingEdgesByTarget.get(edge.targetIndex) ?? []).length !== 1) {
      skippedReason = "confirmedMergeTarget";
    } else if (confirmedMergeIndexes.has(edge.targetIndex)) {
      skippedReason = "confirmedLocalMotifMerge";
    } else if (backwardTargetIndexes.has(edge.targetIndex)) {
      skippedReason = "loopBackOrAncestorTarget";
    } else if (setupInstructionIndexes.has(edge.targetIndex)) {
      skippedReason = "setupPrefixTarget";
    } else if (sourceRegionId !== targetRegionId) {
      skippedReason = "parentOrAncestorRegionTransition";
    } else if (
      scanEdge?.edgeClass === "regionExit" ||
      scanEdge?.edgeClass === "exitTailTransfer" ||
      scanEdge?.edgeClass === "haltExit" ||
      scanEdge?.edgeClass === "pendingForwardMerge"
    ) {
      skippedReason = "structuralTransition";
    } else if (targetHasExplicitLane(targetNode)) {
      skippedReason = "targetAlreadyHasExplicitLocalLane";
    }

    const oldX = Number.isFinite(targetNode?.x) ? Number(targetNode.x.toFixed(2)) : null;
    const laneX = Number.isFinite(inheritedLane?.laneX)
      ? Number(inheritedLane.laneX.toFixed(2))
      : null;
    const centerwardBeforeCorrection = (
      Number.isFinite(oldX) &&
      Number.isFinite(laneX) &&
      Math.abs(oldX - globalCenterX) + 0.5 < Math.abs(laneX - globalCenterX)
    );
    const row = {
      nodeId: targetNode?.id ?? formatInstructionNodeId(edge.targetIndex),
      instructionIndex: edge.targetIndex,
      previousNodeId: sourceNode?.id ?? formatInstructionNodeId(edge.sourceIndex),
      previousInstructionIndex: edge.sourceIndex,
      assignedRegionId: targetRegionId,
      assignedLane: inheritedLane?.side ?? ownershipTokenBefore?.side ?? null,
      inheritedLaneSource: inheritedLane?.source ?? "ownershipTokenOnly",
      oldX,
      laneX,
      centerPlacementReason: skippedReason,
      centerwardBeforeCorrection,
      ownershipTokenBefore,
      ownershipTokenAfter: null,
      tokenPropagationReason: null,
      tokenResetReason: null,
    };
    const isOrdinarySequentialContinuation = (
      sourceNode &&
      targetNode &&
      edge.type === "fallthrough" &&
      edge.targetIndex === edge.sourceIndex + 1 &&
      (outgoingEdgesBySource.get(edge.sourceIndex) ?? []).length === 1
    );
    const tokenResetReason = !isOrdinarySequentialContinuation
      ? "notOrdinaryLinearFallthrough"
      : (incomingEdgesByTarget.get(edge.targetIndex) ?? []).length !== 1
        ? "ambiguousMultiEntryOrMerge"
        : setupInstructionIndexes.has(edge.targetIndex)
          ? "setupPrefixTarget"
          : sourceRegionId !== targetRegionId
            ? "ownerScopeBoundary"
            : null;
    if (ownershipTokenBefore && !tokenResetReason) {
      targetNode.generatedScanV2OwnershipToken = {
        ...ownershipTokenBefore,
        tokenKind: "branch",
      };
      row.ownershipTokenAfter = targetNode.generatedScanV2OwnershipToken;
      row.tokenPropagationReason = "ordinarySequentialContinuation";
    } else if (ownershipTokenBefore) {
      row.tokenResetReason = tokenResetReason;
    }
    if (includeVerboseDiagnostics && ownershipTokenBefore) ownershipTokenRows.push(row);
    if (includeVerboseDiagnostics && centerwardBeforeCorrection) {
      centerPlacementRows.push(row);
    }
    if (skippedReason || !Number.isFinite(laneX)) return;

    targetNode.x = laneX;
    targetNode.generatedScanV2ContinuationLane = {
      predecessorIndex: edge.sourceIndex,
      laneX,
      side: inheritedLane.side,
      decisionIndex: inheritedLane.decisionIndex,
      reason: "ordinaryPredecessorRelativeContinuation",
    };
    if (Math.abs((oldX ?? laneX) - laneX) <= 0.5) return;
    appliedCount += 1;
    if (includeVerboseDiagnostics) {
      appliedRows.push({
        ...row,
        newX: laneX,
        centerPlacementReason: "correctedToPredecessorLocalLane",
      });
    }
  });
...
function applyGeneratedScanV2Placement(layoutPlan, scanLayoutPlan, scanCellLayout = null) {
  if (layoutPlan.layoutMode !== "generatedScanV2") {
    return {
      ...createGeneratedScanV2DiagnosticsDefaults(),
      skippedReason: "layoutModeNotGeneratedScanV2",
      fallbackReason: "layoutModeNotGeneratedScanV2",
    };
  }
  if (!scanLayoutPlan?.enabled) {
    return {
      ...createGeneratedScanV2DiagnosticsDefaults(),
      skippedReason: "scanLayoutPlanUnavailable",
      fallbackReason: "scanLayoutPlanUnavailable",
    };
  }
  ...
  const includeVerboseDiagnostics = isBetaFlowAuditDebugEnabled();
  const stepY = layoutPlan.layout.actionNodeHeight + layoutPlan.layout.verticalGap;
  const exitCorridorX = Math.max(layoutPlan.layout.localBranchTargetOffset, 120) +
    Math.max(layoutPlan.layout.sideHaltDistance * 0.75, 140);

  const discoveryIndexByInstruction = new Map(
    Object.entries(scanLayoutPlan.dfsGrammar?.discoveryIndexByInstruction ?? {})
      .map(([index, rank]) => [Number(index), Number(rank)]),
  );
  const getRunDiscoveryRank = (run) => {
    let rank = Number.POSITIVE_INFINITY;
    for (let index = run.startIndex; index <= run.endIndex; index += 1) {
      rank = Math.min(rank, discoveryIndexByInstruction.get(index) ?? Number.POSITIVE_INFINITY);
    }
  ...
  const runById = new Map((scanLayoutPlan.runs ?? []).map((run) => [run.runId, run]));
  const sortRuns = [...(scanLayoutPlan.runs ?? [])].sort((left, right) => (
    (getRunDiscoveryRank(left) - getRunDiscoveryRank(right)) ||
    (left.startIndex - right.startIndex) ||
    (left.endIndex - right.endIndex)
  ));

  const getNodeByIndex = (index) => instructionNodeLookup.get(index) ?? null;
  ...
  const computeRegionNodeBounds = (region) => {
    if (!Number.isInteger(region?.startIndex) || !Number.isInteger(region?.endIndex)) return null;
    const regionNodes = rangeIndexes(region.startIndex, region.endIndex)
      .map((instructionIndex) => getNodeByIndex(instructionIndex))
      .filter(Boolean);
    if (regionNodes.length === 0) return null;
    const left = Math.min(...regionNodes.map((node) => {
      const { width } = getNodeSize(node, layoutPlan.layout);
      return node.x - width / 2;
    })) - layoutPlan.layout.sideRouteGap;
    const right = Math.max(...regionNodes.map((node) => {
      const { width } = getNodeSize(node, layoutPlan.layout);
      return node.x + width / 2;
    })) + layoutPlan.layout.sideRouteGap;
    const top = Math.min(...regionNodes.map((node) => {
      const { height } = getNodeSize(node, layoutPlan.layout);
      return node.y - height / 2;
    })) - layoutPlan.layout.verticalGap * 0.5;
    const bottom = Math.max(...regionNodes.map((node) => {
      const { height } = getNodeSize(node, layoutPlan.layout);
      return node.y + height / 2;
    })) + layoutPlan.layout.verticalGap * 0.5;
  ...
  const buildNodeCoordinates = () => (
    includeVerboseDiagnostics ? sortRuns.flatMap((run) => (
      rangeIndexes(run.startIndex, run.endIndex)
        .map((index) => {
          const node = getNodeByIndex(index);
          if (!node) return null;
          const runBox = runBoxes.find((candidate) => candidate.runId === run.runId) ?? null;
          return {
            nodeId: node.id,
            instructionIndex: index,
            ...
            runId: run.runId,
            placementParentFrameId: runBox?.placementParentFrameId ?? run.parentRegionId ?? "root",
            placementAnchorInstruction: runBox?.placementAnchorInstruction ?? null,
            placementAnchorEdge: runBox?.placementAnchorEdge ?? null,
            placementReason: runBox?.placementReason ?? "localFramePacking",
            usesInstructionIndexForY: runBox?.usesInstructionIndexForY ?? false,
            instructionIndexUseReason:
              runBox?.instructionIndexUseReason ?? "runMembershipAndStableTieBreakerOnly",
            placementOrderSource: runBox?.placementOrderSource ?? "localFramePacking",
          };
        })
        .filter(Boolean)
    )) : []
  ...
  const runBoxes = [];
  let nodeCoordinates = [];
  let placementDriver = "legacyRunStacking";
  let cellBoxesDrivingPlacement = false;
  let fallbackReason = null;
  let nodesPlacedFromCells = 0;
  let nodesMissingCell = 0;
  let transferCellCollapsedOntoMainSpine = false;
  let transferCellCenterX = null;
  let transferCellWidth = null;
  let transferCellOffsetFromMainSpine = null;
  let transferCellBridgeBetweenSiblingIslands = false;
  let transferCellWarnings = [];
  let percentTransferNodesOnMainSpine = 0;
  let siblingLoopIslandsShareSameGridColumn = false;
  let exitCorridorGapFromOuter = null;
  let totalDiagramHeight = 0;
  let totalDiagramWidth = 0;
  let cellPackingApplied = false;
  let averageVerticalGapBetweenSequentialCells = 0;
  let maxVerticalGapBetweenSequentialCells = 0;
  let compactnessRatio = 0;
  let cellsExpandedOnlyForPaddingCount = 0;
  let cellsWithExcessiveWhitespace = [];
  let verticalNormalizationApplied = false;
  let verticalNormalizationShiftY = 0;
  let levelConsistencyAdjustmentsApplied = 0;
  const levelConsistencyAdjustments = [];
  const levelConsistencyEvaluations = [];
  let localContinuationLaneAdjustmentsApplied = 0;
  const localContinuationLaneAdjustments = [];
  const localContinuationCenterPlacementRows = [];
  const ownershipTokenPropagationRows = [];
  let instructionIndexVerticalBiasCount = 0;
  let neutralizedInstructionIndexVerticalBiasCount = 0;
  let neutralizedInstructionIndexVerticalBiasRows = [];
  let framesOrderedByInstructionIndexCount = 0;
  let framesOrderedByDiscoveryCount = 0;
  let nodesWithYFromInstructionIndex = [];
  let nodesWithYFromControlFlowAdjacency = [];
  let suspiciousLateInstructionDownshiftEdges = [];
  let suspiciousLongEdgesCausedByIndexOrder = [];
  let verticalStretchGapCount = 0;
  let overInflatedFrameCount = 0;
  let maxReservedToNodeHeightRatio = 0;
  let maxReservedToNodeWidthRatio = 0;
  let maxLoopEnvelopeToBodyHeightRatio = 0;
  let frameEnvelopeInflationRows = [];
  let verticalGapConstraintRows = [];
  let rowPackingRows = [];
  let localFrameLayout = null;
  let localFrameLayoutAvailable = false;
  let localFrameCount = 0;
  let localFrameCommitCount = 0;
  let centerSpinePlacementCount = 0;
  let centerFallbackCount = 0;
  let unknownCenterFallbackCount = 0;
  const centerSpinePlacementRows = [];

  const tryCellDrivenPlacement = () => {
    if (!scanCellLayout?.enabled || !Array.isArray(scanCellLayout.cells)) {
      return { applied: false, reason: "generatedScanCellLayoutUnavailable" };
    }

    const runCellByRunId = new Map(
      scanCellLayout.cells
        .filter((cell) => cell?.sourceKind === "run" && typeof cell?.sourceId === "string")
        .map((cell) => [cell.sourceId, cell]),
    );
    const regionCellByRegionId = new Map(
      scanCellLayout.cells
        .filter((cell) => cell?.sourceKind === "region" && typeof cell?.sourceId === "string")
        .map((cell) => [cell.sourceId, cell]),
    );

    const missingRunCells = sortRuns.filter((run) => {
      const cell = runCellByRunId.get(run.runId);
      return !cell || !isFiniteBox(cell.box);
    });
    if (missingRunCells.length > 0) {
      return {
        applied: false,
        reason: `missingRunCellBox:${missingRunCells.map((run) => run.runId).join(",")}`,
      };
    }

    const placedNodeIds = new Set();
    sortRuns.forEach((run) => {
      const runNodes = rangeIndexes(run.startIndex, run.endIndex)
        .map((index) => getNodeByIndex(index))
        .filter(Boolean);
      const runCell = runCellByRunId.get(run.runId);
      const box = runCell?.box;
      if (!isFiniteBox(box) || runNodes.length === 0) return;

      const centerX = (box.left + box.right) / 2;
      const parentRegionCell = run.parentRegionId
        ? regionCellByRegionId.get(run.parentRegionId) ?? null
        : null;
      const parentRegionLaneX = isFiniteBox(parentRegionCell?.box)
        ? (parentRegionCell.box.left + parentRegionCell.box.right) / 2
        : null;
      const laneResolution = resolveGeneratedScanV2ComponentLane({
        component: run,
        assignedLaneX: centerX,
        parentRegionLaneX,
        globalCenterX: 0,
        exitCorridorX,
      });
      const padding = Number.isFinite(runCell.padding) ? Math.max(6, Math.min(28, runCell.padding)) : 14;
      const topY = box.top + layoutPlan.layout.actionNodeHeight / 2 + padding;
      const bottomY = box.bottom - layoutPlan.layout.actionNodeHeight / 2 - padding;
      const usableHeight = Math.max(0, bottomY - topY);
      const perStep = runNodes.length > 1
        ? usableHeight / (runNodes.length - 1)
        : 0;

      runNodes.forEach((node, index) => {
        const y = runNodes.length === 1
          ? (topY + bottomY) / 2
          : topY + perStep * index;
        node.x = laneResolution.laneX;
        node.y = Number(y.toFixed(2));
      ...
      runBoxes.push({
        ...
        interval: run.interval,
        role: run.role,
        placementIntent: run.placementIntent,
        segmentationSource: run.segmentationSource ?? "unknown",
        splitReasons: run.splitReasons ?? [],
        sourceRunId: run.sourceRunId ?? null,
        sourceRunInterval: run.sourceRunInterval ?? null,
        coarseRunWarnings: run.coarseRunWarnings ?? [],
        x: laneResolution.laneX,
        topY: Number(box.top.toFixed(2)),
        bottomY: Number(box.bottom.toFixed(2)),
        width: Number((box.right - box.left).toFixed(2)),
        height: Number((box.bottom - box.top).toFixed(2)),
        parentRegionId: run.parentRegionId ?? null,
        nestingDepth: run.nestingDepth ?? 0,
        sourceCellId: runCell.cellId,
        laneResolution,
        placementParentFrameId: runCell.placementParentFrameId ?? run.parentRegionId ?? "root",
        placementAnchorInstruction: runCell.placementAnchorInstruction ?? null,
        placementAnchorEdge: runCell.placementAnchorEdge ?? null,
        placementReason: runCell.placementReason ?? "localFramePacking",
        usesInstructionIndexForY: runCell.usesInstructionIndexForY ?? false,
        instructionIndexUseReason:
          runCell.instructionIndexUseReason ?? "runMembershipAndStableTieBreakerOnly",
        placementOrderSource: runCell.placementOrderSource ?? "localFramePacking",
      });
    });

    const diamondPortAssignments = scanLayoutPlan.dfsGrammar?.diamondPortAssignments ?? [];
    const incomingEdgeCountByTarget = new Map();
    const analysisEdgesBySource = new Map();
    (layoutPlan.analysis?.edges ?? []).forEach((edge) => {
      if (Number.isInteger(edge.sourceIndex)) {
        if (!analysisEdgesBySource.has(edge.sourceIndex)) {
          analysisEdgesBySource.set(edge.sourceIndex, []);
        }
        analysisEdgesBySource.get(edge.sourceIndex).push(edge);
      }
      if (!Number.isInteger(edge.targetIndex)) return;
    ...
    diamondPortAssignments.forEach((assignment) => {
      const decisionNode = getNodeByIndex(assignment.decisionIndex);
      if (!decisionNode) return;
      decisionNode.generatedScanV2DiamondPorts = assignment;

      (assignment.branchAssignments ?? []).forEach((branch) => {
        if (!branch.preferLowerDiagonal || !Number.isInteger(branch.targetIndex)) return;
        const targetNode = getNodeByIndex(branch.targetIndex);
        if (!targetNode || targetNode.y <= decisionNode.y) return;
        if ((incomingEdgeCountByTarget.get(branch.targetIndex) ?? 0) > 1) return;
        const direction = branch.side === "left" ? -1 : 1;
        const laneX = Number((
          decisionNode.x + direction * assignment.branchClearance
        ).toFixed(2));
        let currentIndex = branch.targetIndex;
        const visited = new Set();
        while (
          Number.isInteger(currentIndex) &&
          !visited.has(currentIndex)
          ...
          visited.add(currentIndex);
          const currentNode = getNodeByIndex(currentIndex);
          if (!currentNode || currentNode.y <= decisionNode.y) break;
          if ((incomingEdgeCountByTarget.get(currentIndex) ?? 0) > 1) break;
          currentNode.x = laneX;
          currentNode.generatedScanV2BranchLane = {
            decisionIndex: assignment.decisionIndex,
            side: branch.side,
          };
          currentNode.generatedScanV2OwnershipToken = {
            originDecisionInstruction: assignment.decisionIndex,
            side: branch.side,
            branchTargetInstruction: branch.targetIndex,
            tokenKind: "branch",
          };
          if (
            assignment.allowsDenseIncidentOverride ||
            layoutPlan.analysis?.nodeRoles?.[currentIndex] === "conditionalJump"
          ) {
            break;
          }

          const outgoingEdges = analysisEdgesBySource.get(currentIndex) ?? [];
          if (outgoingEdges.length !== 1) break;
          const [nextEdge] = outgoingEdges;
          const scanEdge = scanEdgeById.get(nextEdge.id) ?? null;
          if (
            nextEdge.type !== "fallthrough" ||
            nextEdge.targetIndex !== currentIndex + 1 ||
            scanEdge?.proposedLaneType === "outerContinuation" ||
            (incomingEdgeCountByTarget.get(nextEdge.targetIndex) ?? 0) > 1
          ) {
            break;
          }
          currentIndex = nextEdge.targetIndex;
    ...
    const localMotifs = scanLayoutPlan.dfsGrammar?.localMotifs ?? [];
    localMotifs.forEach((motif) => {
      const decisionNode = getNodeByIndex(motif.decisionIndex);
      if (!decisionNode) return;
      const centerX = decisionNode.x;
      const branchClearance = Number.isFinite(motif.branchClearance)
        ? motif.branchClearance
        : Math.max(layoutPlan.layout.localBranchTargetOffset * 0.75, 84);
      const positionArm = (instructionIndexes, direction) => {
        instructionIndexes.forEach((instructionIndex) => {
          const node = getNodeByIndex(instructionIndex);
          if (!node || instructionIndex === motif.mergeIndex) return;
          node.x = Number((centerX + direction * branchClearance).toFixed(2));
          node.generatedScanV2LocalMotifMember = {
            motifId: motif.motifId,
            role: direction < 0 ? "fallthroughArm" : "jumpArm",
          };
          node.generatedScanV2OwnershipToken = {
            originDecisionInstruction: motif.decisionIndex,
            side: direction < 0 ? "left" : "right",
            branchTargetInstruction: instructionIndexes[0] ?? instructionIndex,
            tokenKind: "branch",
          };
        });
      };
      const branchLanes = motif.branchLanes ?? [];
      if (branchLanes.length > 0) {
        branchLanes.forEach((lane) => {
          positionArm(lane.instructionIndexes ?? [], lane.side === "left" ? -1 : 1);
        });
      } else {
        positionArm(motif.fallthroughArmIndexes ?? [], -1);
        positionArm(motif.jumpArmIndexes ?? [], 1);
      }

      const mergeNode = Number.isInteger(motif.mergeIndex)
        ? getNodeByIndex(motif.mergeIndex)
        : null;
      if (mergeNode) {
        mergeNode.x = Number(centerX.toFixed(2));
        mergeNode.generatedScanV2LocalMotifMember = {
          motifId: motif.motifId,
          role: "merge",
        };
      }
    ...
    const localContinuationLanePersistence = applyGeneratedScanV2LocalContinuationLanePersistence({
      analysis: layoutPlan.analysis,
      scanLayoutPlan,
      getNodeByIndex,
      globalCenterX: 0,
      includeVerboseDiagnostics,
    });
    localContinuationLaneAdjustmentsApplied += localContinuationLanePersistence.appliedCount;
    if (includeVerboseDiagnostics) {
      localContinuationLaneAdjustments.push(...localContinuationLanePersistence.appliedRows);
      localContinuationCenterPlacementRows.push(...localContinuationLanePersistence.centerPlacementRows);
      ownershipTokenPropagationRows.push(...localContinuationLanePersistence.ownershipTokenRows);
    }

    const levelMismatchCorrection = applyGeneratedScanV2LevelMismatchCorrection({
      analysis: layoutPlan.analysis,
      scanLayoutPlan,
      getNodeByIndex,
      stepY,
    ...
    const shiftedInstructionIndexes = levelMismatchCorrection.shiftedInstructionIndexes;
    levelConsistencyAdjustmentsApplied += levelMismatchCorrection.appliedRows.length;
    if (includeVerboseDiagnostics) {
      levelConsistencyAdjustments.push(...levelMismatchCorrection.appliedRows);
      levelConsistencyEvaluations.push(...levelMismatchCorrection.evaluationRows);
    }

    if (shiftedInstructionIndexes.size > 0) {
      const shiftedNodes = Array.from(shiftedInstructionIndexes)
        .map((instructionIndex) => ({ instructionIndex, node: getNodeByIndex(instructionIndex) }))
        .filter(({ node }) => node);
      runBoxes.forEach((runBox) => {
        const matchingNodes = shiftedNodes.filter(({ instructionIndex }) => {
          const run = runById.get(runBox.runId);
          return run && run.startIndex <= instructionIndex && instructionIndex <= run.endIndex;
        });
        if (matchingNodes.length === 0) return;
        const bottomY = Math.max(
          runBox.bottomY,
          ...matchingNodes.map(({ node }) => node.y + layoutPlan.layout.actionNodeHeight / 2),
        );
        runBox.bottomY = Number(bottomY.toFixed(2));
        runBox.height = Number((runBox.bottomY - runBox.topY).toFixed(2));
      });
      (scanCellLayout.cells ?? []).forEach((cell) => {
        if (!isFiniteBox(cell?.box)) return;
        const matchingNodes = shiftedNodes.filter(({ instructionIndex }) => (
          cell.startIndex <= instructionIndex && instructionIndex <= cell.endIndex
        ));
        if (matchingNodes.length === 0) return;
        const padding = Number.isFinite(cell.padding) ? cell.padding : 0;
        const bottom = Math.max(
          cell.box.bottom,
          ...matchingNodes.map(({ node }) => (
            node.y + layoutPlan.layout.actionNodeHeight / 2 + padding
          )),
        );
        cell.box.bottom = Number(bottom.toFixed(2));
      });
    }
    ...
    localMotifs.forEach((motif) => {
      const decisionNode = getNodeByIndex(motif.decisionIndex);
      if (!decisionNode) return;
      const motifNodes = (motif.memberInstructionIndexes ?? [])
        .map((instructionIndex) => getNodeByIndex(instructionIndex))
        .filter(Boolean);
      const frameClearance = Number.isFinite(motif.frameClearance)
        ? motif.frameClearance
        : 48;
      const motifBox = motifNodes.length > 0
        ? {
            left: Number((Math.min(...motifNodes.map((node) => {
              const { width } = getNodeSize(node, layoutPlan.layout);
              return node.x - width / 2;
            })) - frameClearance).toFixed(2)),
            right: Number((Math.max(...motifNodes.map((node) => {
              const { width } = getNodeSize(node, layoutPlan.layout);
              return node.x + width / 2;
            })) + frameClearance).toFixed(2)),
            top: Number((Math.min(...motifNodes.map((node) => {
              const { height } = getNodeSize(node, layoutPlan.layout);
              return node.y - height / 2;
            })) - frameClearance).toFixed(2)),
            bottom: Number((Math.max(...motifNodes.map((node) => {
              const { height } = getNodeSize(node, layoutPlan.layout);
              return node.y + height / 2;
            })) + frameClearance).toFixed(2)),
          }
        : null;
      motif.box = motifBox;
      decisionNode.generatedScanV2LocalMotif = {
        motifId: motif.motifId,
        localBranchEdgeIds: motif.localBranchEdgeIds ?? [],
        branchSideByEdgeId: motif.branchSideByEdgeId ?? {},
        levelConsistencyHints: motif.levelConsistencyHints ?? [],
        box: motifBox,
      };
    });

    nodesPlacedFromCells = placedNodeIds.size;
    nodesMissingCell = Math.max(0, instructionNodes.length - nodesPlacedFromCells);
    nodeCoordinates = buildNodeCoordinates();

    const loopIslandCellsByGroup = new Map();
    scanCellLayout.cells
      .filter((cell) => cell?.sourceKind === "region" && cell.type === "loopIslandCell")
      .forEach((cell) => {
        const region = scanLayoutPlan.regions?.find((candidate) => candidate.regionId === cell.sourceId) ?? null;
        const group = region?.parentRegionId ? `region-children:${region.parentRegionId}` : "region-roots";
        if (!loopIslandCellsByGroup.has(group)) loopIslandCellsByGroup.set(group, []);
        loopIslandCellsByGroup.get(group).push(cell);
      });
    siblingLoopIslandsShareSameGridColumn = Array.from(loopIslandCellsByGroup.values()).some((group) => {
      if (group.length < 2) return false;
      const columns = group.map((cell) => cell.suggestedGridColumn);
      return new Set(columns).size < columns.length;
    });

    const outerRegionCells = scanCellLayout.cells.filter((cell) => (
      cell?.sourceKind === "region" &&
      cell?.type === "outerLoopCell" &&
      isFiniteBox(cell?.box)
    ));
    const exitCells = scanCellLayout.cells.filter((cell) => (
      cell?.type === "exitCorridorCell" &&
      isFiniteBox(cell?.box)
    ));
    if (outerRegionCells.length > 0 && exitCells.length > 0) {
      const maxOuterRight = Math.max(...outerRegionCells.map((cell) => cell.box.right));
      const minExitLeft = Math.min(...exitCells.map((cell) => cell.box.left));
      exitCorridorGapFromOuter = Number((minExitLeft - maxOuterRight).toFixed(2));
    }

    return {
      applied: true,
      regionCellByRegionId,
      runCellByRunId,
    };
  };

  const cellPlacement = tryCellDrivenPlacement();
  if (cellPlacement.applied) {
    placementDriver = "cellLayout";
    cellBoxesDrivingPlacement = true;
    fallbackReason = null;
  } else {
    fallbackReason = cellPlacement.reason;

    const firstInstructionY = instructionNodes.length > 0
      ? Math.min(...instructionNodes.map((node) => node.y))
      : 0;
    const runGap = Math.max(Math.round(stepY * 0.45), 24);
    const regionGap = Math.max(Math.round(stepY * 0.75), 40);
    const mainX = 0;
    const islandX = Math.max(layoutPlan.layout.localBranchTargetOffset, 120);
    const continuationX = islandX + Math.max(layoutPlan.layout.sideRouteGap * 2, 36);
    let cursorY = firstInstructionY;

    const fallbackRegionLaneXById = new Map();
    const getFallbackRegionLaneX = (regionId, guard = new Set()) => {
      if (!regionId) return null;
      if (fallbackRegionLaneXById.has(regionId)) return fallbackRegionLaneXById.get(regionId);
      if (guard.has(regionId)) return null;
      guard.add(regionId);
      const region = (scanLayoutPlan.regions ?? []).find(
        (candidate) => candidate.regionId === regionId,
      ) ?? null;
      if (!region) return null;
      if (!region.parentRegionId) {
        fallbackRegionLaneXById.set(regionId, mainX);
        return mainX;
      }
      const parentLaneX = getFallbackRegionLaneX(region.parentRegionId, guard) ?? mainX;
      const siblings = (scanLayoutPlan.regions ?? [])
        .filter((candidate) => candidate.parentRegionId === region.parentRegionId)
        .sort((left, right) => left.startIndex - right.startIndex);
      const siblingIndex = Math.max(0, siblings.findIndex((candidate) => candidate.regionId === regionId));
      const direction = siblingIndex % 2 === 0 ? -1 : 1;
      const laneX = parentLaneX + direction * islandX;
      fallbackRegionLaneXById.set(regionId, laneX);
      return laneX;
    };

    sortRuns.forEach((run, runIndex) => {
      const runNodes = rangeIndexes(run.startIndex, run.endIndex)
        .map((index) => getNodeByIndex(index))
        .filter(Boolean);
      ...
      const laneResolution = resolveGeneratedScanV2ComponentLane({
        component: run,
        parentRegionLaneX: getFallbackRegionLaneX(run.parentRegionId),
        globalCenterX: mainX,
        exitCorridorX,
        islandX,
        continuationX,
      });
      const x = laneResolution.laneX;
      const height = (runNodes.length - 1) * stepY + layoutPlan.layout.actionNodeHeight;
      const topY = cursorY - layoutPlan.layout.actionNodeHeight / 2;

      runNodes.forEach((node, index) => {
        node.x = x;
        node.y = cursorY + index * stepY;
      });
      ...
      runBoxes.push({
        ...
        width: layoutPlan.layout.actionNodeWidth,
        height: Number(height.toFixed(2)),
        parentRegionId: run.parentRegionId ?? null,
        nestingDepth: run.nestingDepth ?? 0,
        sourceCellId: null,
        laneResolution,
        placementParentFrameId: run.parentRegionId ?? "root",
        placementAnchorInstruction: null,
        placementAnchorEdge: null,
        placementReason: "fallbackStableOrder",
        usesInstructionIndexForY: false,
        instructionIndexUseReason: "dfsDiscoveryWithInstructionIndexStableTieBreakerOnly",
        placementOrderSource: "controlFlowDiscovery",
      });

      cursorY = bottomY + runGap + layoutPlan.layout.actionNodeHeight / 2;
      const nextRun = sortRuns[runIndex + 1] ?? null;
      if (nextRun && (nextRun.nestingDepth ?? 0) > (run.nestingDepth ?? 0)) {
        cursorY += Math.round(regionGap * 0.35);
      }
    });

    nodeCoordinates = buildNodeCoordinates();
    nodesPlacedFromCells = 0;
    nodesMissingCell = instructionNodes.length;
    const centerTolerance = Math.max(8, Math.round(layoutPlan.layout.actionNodeWidth * 0.2));
    const centerCount = instructionNodes.filter((node) => Math.abs(node.x) <= centerTolerance).length;
    const percent = instructionNodes.length > 0 ? centerCount / instructionNodes.length : 0;
    transferCellCollapsedOntoMainSpine = percent > 0.6;
    siblingLoopIslandsShareSameGridColumn = true;
    exitCorridorGapFromOuter = null;
  }

  (scanLayoutPlan.dfsGrammar?.diamondPortAssignments ?? []).forEach((assignment) => {
    const decisionNode = getNodeByIndex(assignment.decisionIndex);
    if (decisionNode) {
      decisionNode.generatedScanV2DiamondPorts = assignment;
    }
  ...
  const regionCellsByRegionId = new Map(
    (scanCellLayout?.cells ?? [])
      .filter((cell) => cell?.sourceKind === "region" && typeof cell?.sourceId === "string" && isFiniteBox(cell?.box))
      .map((cell) => [cell.sourceId, cell]),
  );

  const regionBoxes = (scanLayoutPlan.regions ?? []).map((region) => {
    const regionCell = regionCellsByRegionId.get(region.regionId) ?? null;
    const regionRuns = runBoxes.filter((runBox) => {
      const run = runById.get(runBox.runId);
      if (!run) return false;
      return (
        Number.isInteger(region.startIndex) &&
    ...
    const runBounds = regionRuns.length > 0
      ? {
        left: Math.min(...regionRuns.map((run) => run.x - run.width / 2)) - layoutPlan.layout.sideRouteGap,
        right: Math.max(...regionRuns.map((run) => run.x + run.width / 2)) + layoutPlan.layout.sideRouteGap,
        top: Math.min(...regionRuns.map((run) => run.topY)) - layoutPlan.layout.verticalGap * 0.5,
        bottom: Math.max(...regionRuns.map((run) => run.bottomY)) + layoutPlan.layout.verticalGap * 0.5,
      }
      : null;
    const nodeBounds = computeRegionNodeBounds(region);
    const cellBounds = regionCell && isFiniteBox(regionCell.box)
      ? {
        left: regionCell.box.left,
        right: regionCell.box.right,
        top: regionCell.box.top,
        bottom: regionCell.box.bottom,
      }
      : null;
    const bounds = cellBounds ?? runBounds ?? nodeBounds;
    ...
    if (!bounds) {
      return {
        ...
        parentRegionId: region.parentRegionId ?? null,
        childrenRegionIds: region.childrenRegionIds ?? [],
      };
    }
    ...
    return {
      ...
      materialized: true,
      materializationSource: cellBounds
        ? "cellLayoutRegionCell"
        : runBounds
          ? "runBounds"
          : "nodeBoundsFallback",
      parentRegionId: region.parentRegionId ?? null,
  ...
  const parentRegionExpansionStats = [];
  let parentBoxExpandedForChildren = false;
  if (placementDriver === "cellLayout") {
    const byId = new Map(regionBoxes.map((region) => [region.regionId, region]));
    const depthByRegionId = new Map(
      (scanLayoutPlan.regions ?? []).map((region) => {
        let depth = 0;
        let cursor = region;
        const guard = new Set();
        while (cursor?.parentRegionId && !guard.has(cursor.parentRegionId)) {
          guard.add(cursor.parentRegionId);
          const parent = (scanLayoutPlan.regions ?? []).find(
            (candidate) => candidate.regionId === cursor.parentRegionId,
          ) ?? null;
          if (!parent) break;
          depth += 1;
          cursor = parent;
        }
        return [region.regionId, depth];
      }),
    );
    const sortedParents = [...regionBoxes]
      .sort((left, right) => (depthByRegionId.get(right.regionId) ?? 0) - (depthByRegionId.get(left.regionId) ?? 0));

    sortedParents.forEach((parentRegion) => {
      if (!parentRegion.materialized) return;
      const childIds = Array.isArray(parentRegion.childrenRegionIds)
        ? parentRegion.childrenRegionIds
        : [];
      const childBoxes = childIds
        .map((childId) => byId.get(childId))
        .filter((child) => child?.materialized);
      if (childBoxes.length === 0) return;

      const padding = Math.max(8, Math.round(layoutPlan.layout.sideRouteGap * 0.4));
      const before = {
        left: parentRegion.left,
        right: parentRegion.right,
        top: parentRegion.top,
        bottom: parentRegion.bottom,
      };
      const after = {
        left: Number(Math.min(parentRegion.left, ...childBoxes.map((child) => child.left - padding)).toFixed(2)),
        right: Number(Math.max(parentRegion.right, ...childBoxes.map((child) => child.right + padding)).toFixed(2)),
        top: Number(Math.min(parentRegion.top, ...childBoxes.map((child) => child.top - padding)).toFixed(2)),
        bottom: Number(Math.max(parentRegion.bottom, ...childBoxes.map((child) => child.bottom + padding)).toFixed(2)),
      };
      parentRegion.left = after.left;
      parentRegion.right = after.right;
      parentRegion.top = after.top;
      parentRegion.bottom = after.bottom;
      const expanded = (
      ...
      if (expanded) {
        parentBoxExpandedForChildren = true;
      }
      parentRegion.materializationSource = "cellLayoutRegionCellWithChildEnvelope";
      if (includeVerboseDiagnostics) {
        parentRegionExpansionStats.push({
          parentRegionId: parentRegion.regionId,
          parentInterval: parentRegion.interval ?? null,
          childRegionIds: childBoxes.map((child) => child.regionId),
          padding,
          expanded,
          before,
          after,
          expansion: {
            left: Number((before.left - after.left).toFixed(2)),
            right: Number((after.right - before.right).toFixed(2)),
            top: Number((before.top - after.top).toFixed(2)),
            bottom: Number((after.bottom - before.bottom).toFixed(2)),
          },
        });
      }
    });
  ...
  const regionBoxById = new Map(regionBoxes.map((region) => [region.regionId, region]));
  const containmentTolerance = Math.max(8, Math.round(layoutPlan.layout.sideRouteGap * 0.5));
  const regionContainmentChecks = regionBoxes.flatMap((parentRegion) => {
    if (!parentRegion.materialized || !Array.isArray(parentRegion.childrenRegionIds)) return [];
    return parentRegion.childrenRegionIds.map((childRegionId) => {
      const childRegion = regionBoxById.get(childRegionId) ?? null;
      if (!childRegion || !childRegion.materialized) {
        return {
          parentRegionId: parentRegion.regionId,
          parentInterval: parentRegion.interval ?? null,
          childRegionId,
          childInterval: childRegion?.interval ?? null,
          containmentStatus: "childNotMaterialized",
          containsChild: false,
          ...
          childBox: null,
          failedHorizontal: true,
          failedVertical: true,
        };
      }
      const leftOk = childRegion.left >= parentRegion.left - containmentTolerance;
      const rightOk = childRegion.right <= parentRegion.right + containmentTolerance;
      const topOk = childRegion.top >= parentRegion.top - containmentTolerance;
      const bottomOk = childRegion.bottom <= parentRegion.bottom + containmentTolerance;
      const containsChild = leftOk && rightOk && topOk && bottomOk;
      return {
        parentRegionId: parentRegion.regionId,
        parentInterval: parentRegion.interval ?? null,
        childRegionId,
        childInterval: childRegion.interval ?? null,
        containmentStatus: containsChild ? "containsChild" : "outsideParentBounds",
        containsChild,
        tolerance: containmentTolerance,
        parentBox: {
          left: parentRegion.left,
        ...
        childBox: {
          left: childRegion.left,
          right: childRegion.right,
          top: childRegion.top,
          bottom: childRegion.bottom,
        },
        failedHorizontal: !(leftOk && rightOk),
        failedVertical: !(topOk && bottomOk),
      };
    });
  });
  const failedContainmentChecks = regionContainmentChecks.filter((check) => !check.containsChild);
  const failedContainmentCheckCount = failedContainmentChecks.length;
  const regionBoxSourceCounts = regionBoxes.reduce((counts, region) => {
    const key = String(region.materializationSource ?? "unknown");
    counts[key] = (counts[key] ?? 0) + 1;
    return counts;
  }, {});
  const nullRegionBoxCount = regionBoxes.filter((region) => !region.materialized).length;
  const hierarchyCoherent = regionContainmentChecks.every((check) => check.containsChild);
  const runSegmentationSourceCounts = (scanLayoutPlan.summary?.runSegmentationSourceCounts) ?? (
    runBoxes.reduce((acc, runBox) => {
      const key = String(runBox.segmentationSource ?? "unknown");
      acc[key] = (acc[key] ?? 0) + 1;
      return acc;
    }, {})
  );
  const coarseRunWarningCount = Number.isInteger(scanLayoutPlan.summary?.coarseRunWarningCount)
    ? scanLayoutPlan.summary.coarseRunWarningCount
    : runBoxes.filter((runBox) => Array.isArray(runBox.coarseRunWarnings) && runBox.coarseRunWarnings.length > 0).length;
  const runsSplitCount = Number.isInteger(scanLayoutPlan.summary?.runsSplitCount)
    ? scanLayoutPlan.summary.runsSplitCount
    : Math.max(0, runBoxes.length - ((scanLayoutPlan.runs ?? []).length));

  const haltNodes = layoutPlan.nodes.filter((node) => node.kind === "halt" || node.id === "halt");
  const exitCorridorCells = (scanCellLayout?.cells ?? [])
    .filter((cell) => cell?.type === "exitCorridorCell" && isFiniteBox(cell?.box));
  const exitCorridorRight = exitCorridorCells.length > 0
    ? Math.max(...exitCorridorCells.map((cell) => cell.box.right))
    : null;
  ...
  haltNodes.forEach((haltNode) => {
    const incoming = edgesByTo.get(haltNode.id) ?? [];
    const sourceY = incoming
      .map((edge) => Number.isInteger(edge.sourceIndex) ? instructionNodeLookup.get(edge.sourceIndex)?.y : null)
      .filter((value) => Number.isFinite(value));
    if (sourceY.length > 0) {
      haltNode.y = Number((sourceY.reduce((sum, value) => sum + value, 0) / sourceY.length).toFixed(2));
    }
    haltNode.x = Number((
      (Number.isFinite(exitCorridorRight)
        ? exitCorridorRight + Math.max(layoutPlan.layout.sideRouteGap * 2, 30)
        : exitCorridorX + Math.max(layoutPlan.layout.sideRouteGap * 2, 30))
    ).toFixed(2));
  });

  const edgeRows = scanLayoutPlan.edges ?? [];
  const edgesUsingSimpleLocalRouting = edgeRows.filter((edge) => (
    edge.proposedLaneType === "localFallthrough" ||
    edge.proposedLaneType === "localBranch" ||
    edge.proposedLaneType === "loopReturn" ||
    edge.proposedLaneType === "nestedLoopReturn"
  )).length;
  const edgesUsingOuterCorridor = edgeRows.filter((edge) => (
    edge.proposedLaneType === "outerContinuation" ||
    edge.proposedLaneType === "haltExitCorridor"
  )).length;
  const edgesDeferredOrUnresolved = edgeRows
    .filter((edge) => edge.edgeClass === "pendingForwardMerge" || edge.edgeClass === "interRegionTransfer")
    .map((edge) => edge.edgeId);

  if (placementDriver === "cellLayout" && includeVerboseDiagnostics) {
    const centerTolerance = Math.max(8, Math.round(layoutPlan.layout.actionNodeWidth * 0.2));
    const centeredCount = instructionNodes.filter((node) => Math.abs(node.x) <= centerTolerance).length;
    const centeredPercent = instructionNodes.length > 0 ? centeredCount / instructionNodes.length : 0;
    const allCells = scanCellLayout?.cells ?? [];
    const cellById = new Map(allCells.map((cell) => [cell.cellId, cell]));
    const hasOuterLoopParent = (cell) => {
      if (!cell?.parentCellId) return false;
      const parent = cellById.get(cell.parentCellId) ?? null;
      return parent?.type === "outerLoopCell";
    };
    const transferAggregateRanges = allCells
      .filter((cell) => (
        cell?.sourceKind === "syntheticPhase" &&
        String(cell?.sourceId ?? "").includes("transferAggregate")
      ))
      .map((cell) => ({ startIndex: cell.startIndex, endIndex: cell.endIndex }));
    const transferRunCells = allCells.filter((cell) => (
      cell?.sourceKind === "run" &&
      isFiniteBox(cell?.box) &&
      (
        String(cell?.placementIntent ?? "").includes("bridgeTransfer") ||
    ...
    const transferNodeIndices = new Set();
    transferRunCells.forEach((cell) => {
      for (let index = cell.startIndex; index <= cell.endIndex; index += 1) {
        if (instructionNodeLookup.has(index)) {
          transferNodeIndices.add(index);
        }
      }
    });
    const transferNodeCount = transferNodeIndices.size;
    const transferNodeOnSpineCount = Array.from(transferNodeIndices).reduce((count, instructionIndex) => {
      const node = instructionNodeLookup.get(instructionIndex);
      if (!node) return count;
      return Math.abs(node.x) <= centerTolerance ? count + 1 : count;
    }, 0);
    percentTransferNodesOnMainSpine = transferNodeCount > 0
      ? Number((transferNodeOnSpineCount / transferNodeCount).toFixed(3))
      : 0;
    transferCellCollapsedOntoMainSpine = transferNodeCount > 0
      ? percentTransferNodesOnMainSpine > 0.35
      : false;
    if (transferRunCells.length > 0) {
      const centers = transferRunCells.map((cell) => (cell.box.left + cell.box.right) / 2);
      const left = Math.min(...transferRunCells.map((cell) => cell.box.left));
      const right = Math.max(...transferRunCells.map((cell) => cell.box.right));
      transferCellCenterX = Number((centers.reduce((sum, value) => sum + value, 0) / centers.length).toFixed(2));
      transferCellWidth = Number((right - left).toFixed(2));
      transferCellOffsetFromMainSpine = Number(Math.abs(transferCellCenterX).toFixed(2));
    }
    if (transferRunCells.length === 0) {
      transferCellWarnings.push("noTransferRunCellsDetected");
    }
    if (transferCellCollapsedOntoMainSpine) {
      transferCellWarnings.push("transferNodesRemainOnMainSpine");
    }

    const findOuterAncestorCell = (cell) => {
      let cursor = cell;
      const guard = new Set();
      while (cursor?.parentCellId && !guard.has(cursor.parentCellId)) {
        guard.add(cursor.parentCellId);
        ...
        if (parent.type === "outerLoopCell") return parent;
        cursor = parent;
      }
      return null;
    };
    transferCellBridgeBetweenSiblingIslands = transferRunCells.some((cell) => {
      const outerAncestor = findOuterAncestorCell(cell);
      if (!outerAncestor) return false;
      const siblingLoopCells = allCells.filter((candidate) => (
        candidate?.sourceKind === "region" &&
        candidate?.type === "loopIslandCell" &&
        candidate?.parentCellId === outerAncestor.cellId &&
        isFiniteBox(candidate?.box)
      ));
      if (siblingLoopCells.length < 2 || !isFiniteBox(cell.box)) return false;
      const siblingCenters = siblingLoopCells.map((loopCell) => (loopCell.box.left + loopCell.box.right) / 2);
      const minCenter = Math.min(...siblingCenters);
      const maxCenter = Math.max(...siblingCenters);
      const transferCenter = (cell.box.left + cell.box.right) / 2;
      return transferCenter >= minCenter - centerTolerance && transferCenter <= maxCenter + centerTolerance;
    });
    if (!transferCellBridgeBetweenSiblingIslands && transferRunCells.length > 0) {
      transferCellWarnings.push("transferCellsNotBridgingSiblingIslands");
    }

    const loopIslandCells = (scanCellLayout?.cells ?? []).filter((cell) => (
      cell?.sourceKind === "region" && cell?.type === "loopIslandCell"
    ));
    const groupedByParent = new Map();
    loopIslandCells.forEach((cell) => {
      const region = scanLayoutPlan.regions?.find((candidate) => candidate.regionId === cell.sourceId) ?? null;
      const key = region?.parentRegionId ?? "root";
      if (!groupedByParent.has(key)) groupedByParent.set(key, []);
      groupedByParent.get(key).push(cell);
    });
    siblingLoopIslandsShareSameGridColumn = Array.from(groupedByParent.values()).some((group) => {
      if (group.length < 2) return false;
      const cols = group.map((cell) => cell.suggestedGridColumn);
      return new Set(cols).size < cols.length;
    });

    const outerRegionBoxes = regionBoxes
      .filter((region) => region.materialized && region.parentRegionId === null)
      .map((region) => ({ left: region.left, right: region.right }));
    const maxOuterRight = outerRegionBoxes.length > 0
      ? Math.max(...outerRegionBoxes.map((box) => box.right))
      : null;
    const minExitLeft = exitCorridorCells.length > 0
      ? Math.min(...exitCorridorCells.map((cell) => cell.box.left))
      : null;
    exitCorridorGapFromOuter = (Number.isFinite(maxOuterRight) && Number.isFinite(minExitLeft))
      ? Number((minExitLeft - maxOuterRight).toFixed(2))
      : null;

    nodeCoordinates = buildNodeCoordinates();
    if (!Number.isFinite(centeredPercent)) {
      fallbackReason = fallbackReason ?? "invalidCenteredPercent";
    }
  } else {
    transferCellCenterX = null;
    transferCellWidth = null;
  ...
  if (placementDriver === "cellLayout" && instructionNodes.length > 0) {
    const instructionTop = Math.min(
      ...instructionNodes.map((node) => node.y - layoutPlan.layout.actionNodeHeight / 2),
    );
    const targetTop = Math.max(
      NODE_TOP_PADDING + layoutPlan.layout.actionNodeHeight * 0.5,
      layoutPlan.layout.actionNodeHeight * 1.35,
    );
    const shiftY = Number((targetTop - instructionTop).toFixed(2));
    if (Number.isFinite(shiftY) && Math.abs(shiftY) > 0.5) {
      instructionNodes.forEach((node) => {
        node.y = Number((node.y + shiftY).toFixed(2));
      });
      layoutPlan.nodes
        .filter((node) => node.kind === "halt")
        .forEach((node) => {
          node.y = Number((node.y + shiftY).toFixed(2));
        });
      runBoxes.forEach((runBox) => {
        runBox.topY = Number((runBox.topY + shiftY).toFixed(2));
        runBox.bottomY = Number((runBox.bottomY + shiftY).toFixed(2));
      });
      regionBoxes.forEach((regionBox) => {
        if (!regionBox?.materialized) return;
        regionBox.top = Number((regionBox.top + shiftY).toFixed(2));
        regionBox.bottom = Number((regionBox.bottom + shiftY).toFixed(2));
      });
      (scanCellLayout?.cells ?? []).forEach((cell) => {
        if (!isFiniteBox(cell?.box)) return;
        cell.box.top = Number((cell.box.top + shiftY).toFixed(2));
        cell.box.bottom = Number((cell.box.bottom + shiftY).toFixed(2));
      });
      (scanLayoutPlan.dfsGrammar?.localMotifs ?? []).forEach((motif) => {
        if (!isFiniteBox(motif?.box)) return;
        motif.box.top = Number((motif.box.top + shiftY).toFixed(2));
        motif.box.bottom = Number((motif.box.bottom + shiftY).toFixed(2));
        const decisionNode = getNodeByIndex(motif.decisionIndex);
        if (decisionNode?.generatedScanV2LocalMotif) {
          decisionNode.generatedScanV2LocalMotif.box = motif.box;
        }
      });
      nodeCoordinates = buildNodeCoordinates();
      verticalNormalizationApplied = true;
      verticalNormalizationShiftY = shiftY;
    }
  }

  localFrameLayout = buildGeneratedScanV2LocalFrameLayout({
    layoutPlan,
    scanLayoutPlan,
    regionBoxes,
    getNodeByIndex,
  ...
  localFrameLayoutAvailable = Boolean(localFrameLayout.available);
  localFrameCount = localFrameLayout.frameCount ?? 0;
  localFrameCommitCount = commitGeneratedScanV2LocalFrameLayout(
    localFrameLayout,
    getNodeByIndex,
  );
  haltNodes.forEach((haltNode) => {
  ...
  const percentNodesOnMainSpine = instructionNodes.length > 0
    ? Number((
      instructionNodes.filter((node) => Math.abs(node.x) <= Math.max(8, Math.round(layoutPlan.layout.actionNodeWidth * 0.2))).length /
      instructionNodes.length
    ).toFixed(3))
    : 0;
  const centerTolerance = Math.max(8, Math.round(layoutPlan.layout.actionNodeWidth * 0.2));
  const runBoxByRunId = new Map(runBoxes.map((runBox) => [runBox.runId, runBox]));
  sortRuns.forEach((run) => {
    if (run.role === "setupRun") return;
    const centeredInstructionIndexes = rangeIndexes(run.startIndex, run.endIndex)
      .filter((instructionIndex) => {
        const node = getNodeByIndex(instructionIndex);
        return node && Math.abs(node.x) <= centerTolerance;
      });
    if (centeredInstructionIndexes.length === 0) return;

    const centeredNodes = centeredInstructionIndexes
      .map((instructionIndex) => getNodeByIndex(instructionIndex))
      .filter(Boolean);
    const initialCenterReason = runBoxByRunId.get(run.runId)?.laneResolution?.centerReason ?? null;
    let reason = initialCenterReason;
    if (centeredNodes.some((node) => node.generatedScanV2BranchLane)) {
      reason = "branchLaneAlignedWithGlobalCenter";
    } else if (centeredNodes.every((node) => node.generatedScanV2LocalMotifMember?.role === "merge")) {
      reason = "trueMerge";
    } else if (run.parentRegionId) {
      reason = "localRegionAlignedWithGlobalCenter";
    } else if (run.role === "mergeRun") {
      reason = "trueMerge";
    } else if (run.role === "continuationRun" || run.role === "loopBodyRun") {
      reason = "mainContinuation";
    } else if (!reason) {
      reason = "unknownFallback";
    }

    const isCenterFallback = (
      reason === "noLaneAssigned" ||
      reason === "collisionFallback" ||
      reason === "unknownFallback"
    );
    centerSpinePlacementCount += 1;
    if (isCenterFallback) centerFallbackCount += 1;
    if (reason === "unknownFallback") unknownCenterFallbackCount += 1;
    if (includeVerboseDiagnostics) {
      centerSpinePlacementRows.push({
        runId: run.runId,
        interval: run.interval,
        role: run.role,
        placementIntent: run.placementIntent ?? null,
        parentRegionId: run.parentRegionId ?? null,
        centeredInstructionIndexes,
        reason,
        layoutSmell: isCenterFallback,
      });
    }
  });
  const exitOutsideOuter = Number.isFinite(exitCorridorGapFromOuter)
    ? exitCorridorGapFromOuter >= 0
    : false;

  if (placementDriver === "cellLayout") {
    const cellSummary = scanCellLayout?.summary ?? {};
    cellPackingApplied = Boolean(cellSummary.cellPackingApplied);
    cellsExpandedOnlyForPaddingCount = Number.isInteger(cellSummary.cellsExpandedOnlyForPaddingCount)
      ? cellSummary.cellsExpandedOnlyForPaddingCount
      : 0;
    cellsWithExcessiveWhitespace = Array.isArray(cellSummary.cellsWithExcessiveWhitespace)
      ? cellSummary.cellsWithExcessiveWhitespace
      : [];
    instructionIndexVerticalBiasCount =
      Number.isInteger(cellSummary.instructionIndexVerticalBiasCount)
        ? cellSummary.instructionIndexVerticalBiasCount
        : 0;
    neutralizedInstructionIndexVerticalBiasCount =
      Number.isInteger(cellSummary.neutralizedInstructionIndexVerticalBiasCount)
        ? cellSummary.neutralizedInstructionIndexVerticalBiasCount
        : 0;
    neutralizedInstructionIndexVerticalBiasRows =
      Array.isArray(cellSummary.neutralizedInstructionIndexVerticalBiasRows)
        ? cellSummary.neutralizedInstructionIndexVerticalBiasRows
        : [];
    framesOrderedByInstructionIndexCount =
      Number.isInteger(cellSummary.framesOrderedByInstructionIndexCount)
        ? cellSummary.framesOrderedByInstructionIndexCount
        : 0;
    framesOrderedByDiscoveryCount =
      Number.isInteger(cellSummary.framesOrderedByDiscoveryCount)
        ? cellSummary.framesOrderedByDiscoveryCount
        : 0;
    nodesWithYFromInstructionIndex = Array.isArray(cellSummary.nodesWithYFromInstructionIndex)
      ? cellSummary.nodesWithYFromInstructionIndex
      : [];
    nodesWithYFromControlFlowAdjacency =
      Array.isArray(cellSummary.nodesWithYFromControlFlowAdjacency)
        ? cellSummary.nodesWithYFromControlFlowAdjacency
        : [];
    suspiciousLateInstructionDownshiftEdges =
      Array.isArray(cellSummary.suspiciousLateInstructionDownshiftEdges)
        ? cellSummary.suspiciousLateInstructionDownshiftEdges
        : [];
    suspiciousLongEdgesCausedByIndexOrder =
      Array.isArray(cellSummary.suspiciousLongEdgesCausedByIndexOrder)
        ? cellSummary.suspiciousLongEdgesCausedByIndexOrder
        : [];
    verticalStretchGapCount = cellSummary.verticalStretchGapCount ?? 0;
    overInflatedFrameCount = cellSummary.overInflatedFrameCount ?? 0;
    maxReservedToNodeHeightRatio = cellSummary.maxReservedToNodeHeightRatio ?? 0;
    maxReservedToNodeWidthRatio = cellSummary.maxReservedToNodeWidthRatio ?? 0;
    maxLoopEnvelopeToBodyHeightRatio = cellSummary.maxLoopEnvelopeToBodyHeightRatio ?? 0;
    frameEnvelopeInflationRows = cellSummary.frameEnvelopeInflationRows ?? [];
    verticalGapConstraintRows = cellSummary.verticalGapConstraintRows ?? [];
    rowPackingRows = cellSummary.rowPackingRows ?? [];

    const instructionBounds = instructionNodes.length > 0
      ? {
        left: Math.min(...instructionNodes.map((node) => node.x - layoutPlan.layout.actionNodeWidth / 2)),
        right: Math.max(...instructionNodes.map((node) => node.x + layoutPlan.layout.actionNodeWidth / 2)),
        top: Math.min(...instructionNodes.map((node) => node.y - layoutPlan.layout.actionNodeHeight / 2)),
        bottom: Math.max(...instructionNodes.map((node) => node.y + layoutPlan.layout.actionNodeHeight / 2)),
      }
      : null;
    if (instructionBounds) {
      totalDiagramWidth = Number((instructionBounds.right - instructionBounds.left).toFixed(2));
      totalDiagramHeight = Number((instructionBounds.bottom - instructionBounds.top).toFixed(2));
      const area = totalDiagramWidth * totalDiagramHeight;
      const runBoxArea = runBoxes.reduce((sum, runBox) => sum + ((runBox.width ?? 0) * (runBox.height ?? 0)), 0);
      compactnessRatio = area > 0 ? Number((runBoxArea / area).toFixed(4)) : 0;
    }

    const orderedRunBoxes = [...runBoxes]
      .sort((left, right) => {
        const leftRun = runById.get(left.runId) ?? null;
        const rightRun = runById.get(right.runId) ?? null;
        return (
          (getRunDiscoveryRank(leftRun ?? left) - getRunDiscoveryRank(rightRun ?? right)) ||
          ((leftRun?.startIndex ?? 0) - (rightRun?.startIndex ?? 0))
        );
      });
    const gapValues = [];
    for (let index = 0; index + 1 < orderedRunBoxes.length; index += 1) {
      const current = orderedRunBoxes[index];
      const next = orderedRunBoxes[index + 1];
      const gap = Number((next.topY - current.bottomY).toFixed(2));
      if (Number.isFinite(gap) && gap >= 0) {
        gapValues.push(gap);
      }
    }
    averageVerticalGapBetweenSequentialCells = gapValues.length > 0
      ? Number((gapValues.reduce((sum, value) => sum + value, 0) / gapValues.length).toFixed(2))
      : 0;
    maxVerticalGapBetweenSequentialCells = gapValues.length > 0
      ? Number(Math.max(...gapValues).toFixed(2))
      : 0;
  }
  ...
  return {
    ...
    skippedReason: null,
    placementDriver,
    cellBoxesDrivingPlacement,
    nodesMissingCell,
    fallbackReason,
    percentNodesOnMainSpine,
    exitOutsideOuter,
    exitCorridorGapFromOuter,
    totalDiagramHeight,
    totalDiagramWidth,
    maxVerticalGapBetweenSequentialCells,
    failedContainmentCheckCount,
    regionBoxes,
    nullRegionBoxCount,
    hierarchyCoherent,
    levelConsistencyAdjustmentsApplied,
    localContinuationLaneAdjustmentsApplied,
    localFrameLayoutAvailable,
    localFrameCount,
    localFrameCommitCount,
    branchChildFrameCandidateCount: localFrameLayout.branchChildFrameCandidateCount ?? 0,
    branchChildFrameMaterializedCount: localFrameLayout.branchChildFrameMaterializedCount ?? 0,
    branchChildFrameSkippedCount: localFrameLayout.branchChildFrameSkippedCount ?? 0,
    mergeChildFrameCandidateCount: localFrameLayout.mergeChildFrameCandidateCount ?? 0,
    mergeChildFrameMaterializedCount: localFrameLayout.mergeChildFrameMaterializedCount ?? 0,
    exitTailFrameCandidateCount: localFrameLayout.exitTailFrameCandidateCount ?? 0,
    exitTailFrameMaterializedCount: localFrameLayout.exitTailFrameMaterializedCount ?? 0,
    centerSpinePlacementCount,
    centerFallbackCount,
    unknownCenterFallbackCount,
    ...(includeVerboseDiagnostics ? {
      transferCellCollapsedOntoMainSpine,
      transferCellCenterX,
      transferCellWidth,
      transferCellOffsetFromMainSpine,
      transferCellBridgeBetweenSiblingIslands,
      transferCellWarnings,
      percentTransferNodesOnMainSpine,
      siblingLoopIslandsShareSameGridColumn,
      cellPackingApplied,
      averageVerticalGapBetweenSequentialCells,
      compactnessRatio,
      cellsExpandedOnlyForPaddingCount,
      cellsWithExcessiveWhitespace,
      verticalNormalizationApplied,
      verticalNormalizationShiftY,
      levelConsistencyAdjustmentsApplied,
      levelConsistencyAdjustments,
      levelConsistencyEvaluations,
      localContinuationLaneAdjustments,
      ...
      suspiciousLateInstructionDownshiftEdges,
      suspiciousLongEdgesCausedByIndexOrder,
      verticalStretchGapCount,
      suspiciousLongEdgeCount: 0,
      overInflatedFrameCount,
      maxReservedToNodeHeightRatio,
      maxReservedToNodeWidthRatio,
      maxLoopEnvelopeToBodyHeightRatio,
      frameEnvelopeInflationRows,
      verticalGapConstraintRows,
      rowPackingRows,
      localFrameLayout,
      branchChildFrameCandidates: localFrameLayout.branchChildFrameCandidates ?? [],
      centerSpinePlacementRows,
      runSegmentationSummary: runSegmentationSourceCounts,
      runsSplitCount,
      coarseRunWarningCount,
      failedContainmentChecks,
      regionBoxSourceCounts,
      parentBoxExpandedForChildren,
      parentRegionExpansionStats,
      regionContainmentChecks,
      runBoxes,
      nodeCoordinates,
      edgesUsingSimpleLocalRouting,
      edgesUsingOuterCorridor,
      edgesDeferredOrUnresolved,
      suggestedMajorLayoutSkeleton: scanLayoutPlan.suggestedMajorLayoutSkeleton ?? null,
    } : {}),
  };
}
...
function buildScanV2LoopClosureRoute({
  sourceNode,
  targetNode,
  layout,
  laneX,
  ...
  const sourceBounds = getNodeBounds(sourceNode, layout);
  const targetBounds = getNodeBounds(targetNode, layout);
  const start = laneSide === "left"
    ? getLeftPort(sourceBounds)
    : getRightPort(sourceBounds);
  const end = laneSide === "left"
    ? getLeftPort(targetBounds)
    : getRightPort(targetBounds);
...
function getBoundsEnvelope(boundsRows = []) {
  const finiteBounds = boundsRows.filter((bounds) => (
    bounds &&
  ...
  return {
    left: Math.min(...finiteBounds.map((bounds) => bounds.left)),
    right: Math.max(...finiteBounds.map((bounds) => bounds.right)),
    top: Math.min(...finiteBounds.map((bounds) => bounds.top)),
    bottom: Math.max(...finiteBounds.map((bounds) => bounds.bottom)),
  };
}

function getLoopFrameCanvasExpansion(points, contentEnvelope) {
  if (!contentEnvelope || !Array.isArray(points) || points.length === 0) return 0;
  const minX = Math.min(...points.map(([x]) => x));
  const maxX = Math.max(...points.map(([x]) => x));
  return Math.max(0, contentEnvelope.left - minX) + Math.max(0, maxX - contentEnvelope.right);
}

function getLoopFrameConnectorLength(points = []) {
  if (!Array.isArray(points) || points.length < 2) return 0;
  return points.slice(1).reduce((total, point, index) => (
    total +
    Math.abs(point[0] - points[index][0]) +
    Math.abs(point[1] - points[index][1])
  ), 0);
}

function getGeneratedScanV2RouteBBox(points = []) {
  if (
...
function classifyGeneratedScanV2LoopRouteSideRelativeToOwner({
  routeBBox,
  ownerFrameBBox,
  sideThreshold,
}) {
  if (!routeBBox || !ownerFrameBBox) return "unknown";
  const leftOverflow = ownerFrameBBox.left - routeBBox.left;
  const rightOverflow = routeBBox.right - ownerFrameBBox.right;
  const exitsLeft = leftOverflow >= sideThreshold;
  const exitsRight = rightOverflow >= sideThreshold;
  if (exitsLeft && exitsRight) return "mixed";
  if (exitsLeft) return "left";
  if (exitsRight) return "right";
  return "inside";
}

function buildGeneratedScanV2LoopSideGeometryDiagnostics({
  points,
  ownerFrameBBox,
  returnPointFrameBBox,
  finalClosureSide,
  sideThreshold,
}) {
  const edgeRouteBBox = getGeneratedScanV2RouteBBox(points);
  const actualRenderedSideRelativeToOwner = classifyGeneratedScanV2LoopRouteSideRelativeToOwner({
    routeBBox: edgeRouteBBox,
    ownerFrameBBox,
    sideThreshold,
  });
  const finalSideHonoredByRoute =
    (finalClosureSide === "left" || finalClosureSide === "right") &&
    actualRenderedSideRelativeToOwner === finalClosureSide;
  let sideGeometryMismatchReason = null;
  if (!finalSideHonoredByRoute) {
    sideGeometryMismatchReason = actualRenderedSideRelativeToOwner === "mixed"
      ? "routeCrossesBothSidesOfOwnerFrame"
      : actualRenderedSideRelativeToOwner === "inside"
        ? "routeDoesNotLeaveOwnerFrame"
        : actualRenderedSideRelativeToOwner === "unknown"
          ? "routeGeometryUnavailable"
          : "renderedRouteUsesOppositeSide";
  }
  return {
    ownerFrameBBox,
    returnPointFrameBBox,
    edgeRouteBBox,
    edgeRouteCenterX: edgeRouteBBox
      ? Number(((edgeRouteBBox.left + edgeRouteBBox.right) / 2).toFixed(2))
      : null,
    edgeRouteMinX: edgeRouteBBox?.left ?? null,
    edgeRouteMaxX: edgeRouteBBox?.right ?? null,
    actualRenderedSideRelativeToOwner,
    sideGeometryMatchesFinalSide: finalSideHonoredByRoute,
    sideGeometryMismatchReason,
    routeSideOverrideReason: null,
    finalSideHonoredByRoute,
  };
}

function isGeneratedScanV2RegionAncestor(regionById, ancestorRegionId, descendantRegionId) {
  if (!ancestorRegionId || !descendantRegionId || ancestorRegionId === descendantRegionId) return false;
  let cursor = regionById.get(descendantRegionId) ?? null;
  const seenRegionIds = new Set();
  while (cursor?.parentRegionId && !seenRegionIds.has(cursor.parentRegionId)) {
    if (cursor.parentRegionId === ancestorRegionId) return true;
    seenRegionIds.add(cursor.parentRegionId);
    cursor = regionById.get(cursor.parentRegionId) ?? null;
  }
  return false;
}

function classifyGeneratedScanV2LoopCorridorScopeRelationship({
  regionById,
  ownerRegionId,
  reservedOwnerRegionId,
}) {
  if (!ownerRegionId || !reservedOwnerRegionId) return "unknown";
  if (ownerRegionId === reservedOwnerRegionId) return "sameScope";
  if (isGeneratedScanV2RegionAncestor(regionById, reservedOwnerRegionId, ownerRegionId)) {
    return "ancestor";
  }
  if (isGeneratedScanV2RegionAncestor(regionById, ownerRegionId, reservedOwnerRegionId)) {
    return "descendant";
  }
  const ownerRegion = regionById.get(ownerRegionId) ?? null;
  const reservedRegion = regionById.get(reservedOwnerRegionId) ?? null;
  if (
    ownerRegion?.parentRegionId &&
    ownerRegion.parentRegionId === reservedRegion?.parentRegionId
  ) {
    return "sibling";
  }
  return "unrelated";
...
function isGeneratedScanV2NestedCorridorOrderingValid({
  laneSide,
  laneX,
  reservedLaneX,
  relationship,
}) {
  if (relationship !== "ancestor" && relationship !== "descendant") return true;
  const candidateShouldBeTighter = relationship === "ancestor";
  if (laneSide === "left") {
    return candidateShouldBeTighter ? laneX > reservedLaneX : laneX < reservedLaneX;
  }
  return candidateShouldBeTighter ? laneX < reservedLaneX : laneX > reservedLaneX;
}

function summarizeGeneratedScanV2ReservedLoopCorridorRelationships({
  ...
  const comparedCorridors = reservedLoopCorridors.filter((corridor) => (
    corridor.laneSide === laneSide &&
    Math.min(bottom, corridor.bottom) - Math.max(top, corridor.top) > 8
  )).map((corridor) => {
    const relationship = classifyGeneratedScanV2LoopCorridorScopeRelationship({
      regionById,
      ownerRegionId,
      reservedOwnerRegionId: corridor.ownerRegionId,
    });
    return {
      edgeId: corridor.edgeId,
      ownerRegionId: corridor.ownerRegionId,
      relationship,
      nearbyAbsoluteCorridor: Math.abs(corridor.laneX - laneX) <= laneStep * 1.5,
      nestedOrderingValid: isGeneratedScanV2NestedCorridorOrderingValid({
        laneSide,
        laneX,
        reservedLaneX: corridor.laneX,
        relationship,
      }),
    };
  });
  const countRelationship = (relationship) => (
    comparedCorridors.filter((corridor) => corridor.relationship === relationship).length
  );
  return {
    comparedReservedCorridorCount: comparedCorridors.length,
    sameScopeConflictCount: countRelationship("sameScope"),
    ancestorCorridorCount: countRelationship("ancestor"),
    descendantCorridorCount: countRelationship("descendant"),
    siblingScopeConflictCount: countRelationship("sibling"),
    unrelatedAbsoluteConflictCount: comparedCorridors.filter((corridor) => (
      corridor.relationship === "unrelated" || corridor.relationship === "unknown"
    )).length,
    nearbyAbsoluteConflictCount: comparedCorridors.filter((corridor) => (
      corridor.nearbyAbsoluteCorridor
    )).length,
    nestedOrderingViolationCount: comparedCorridors.filter((corridor) => (
      !corridor.nestedOrderingValid
    )).length,
    ...(includeVerboseDiagnostics ? { corridorComparisons: comparedCorridors } : {}),
  };
}

function evaluateGeneratedScanV2LoopFrameCandidateCost({
  candidatePoints,
  laneSide,
  laneX,
  ownerEnvelope,
  ...
  newFindingSummary,
  newEvaluation,
  childIntersectionCount,
  includeVerboseDiagnostics,
}) {
  const pointYs = candidatePoints.map(([, y]) => y);
  const top = Math.min(...pointYs);
  const bottom = Math.max(...pointYs);
  const branchInteriorPressure = countEdgeRegionBoxIntersections(candidatePoints, branchInteriorBoxes);
  const corridorRelationships = summarizeGeneratedScanV2ReservedLoopCorridorRelationships({
    reservedLoopCorridors,
    regionById,
    ownerRegionId,
    laneSide,
  ...
  const wrapsOwnerRegion = laneSide === "left"
    ? laneX < ownerEnvelope.left
    : laneX > ownerEnvelope.right;
  const canvasExpansion = getLoopFrameCanvasExpansion(candidatePoints, contentEnvelope);
  const connectorLength = getLoopFrameConnectorLength(candidatePoints);
  const nodeIntersectionCount = newEvaluation.intersectingNodeIds.length;
  const foreignRegionCount = newEvaluation.foreignRegionIds.length;
  const highFindingCount = newFindingSummary.highCount ?? 0;
  const totalFindingCount = newFindingSummary.totalCount ?? 0;
  const factors = {
    nodeIntersectionCount,
    foreignRegionCount,
    childIntersectionCount,
    branchInteriorPressure,
    ...corridorRelationships,
    hardGeometryCollisionCount: nodeIntersectionCount,
    wrapsOwnerRegion,
    canvasExpansion: Number(canvasExpansion.toFixed(2)),
    connectorLength: Number(connectorLength.toFixed(2)),
    highFindingCount,
    totalFindingCount,
  };
  const weightedCosts = {
    nodeIntersections: nodeIntersectionCount * 1200,
    foreignRegions: foreignRegionCount * 900,
    childRegionIntersections: childIntersectionCount * 700,
    branchInteriorPressure: branchInteriorPressure * 420,
    sameScopeRankPressure: corridorRelationships.sameScopeConflictCount * 54,
    siblingScopeCollisions: corridorRelationships.siblingScopeConflictCount * 220,
    unrelatedAbsolutePressure: corridorRelationships.unrelatedAbsoluteConflictCount * 8,
    nestedOrderingViolations: corridorRelationships.nestedOrderingViolationCount * 900,
    ownerWrapFailure: wrapsOwnerRegion ? 0 : 1400,
    canvasExpansion: canvasExpansion * 0.7,
    connectorLength: connectorLength * 0.025,
    highFindings: highFindingCount * 220,
    totalFindings: totalFindingCount * 8,
  };
  const totalCost = Object.values(weightedCosts).reduce((total, cost) => total + cost, 0);
  const [mainCostReason] = Object.entries(weightedCosts)
...
function getGeneratedScanV2NodeBranchSide(node) {
  if (node?.generatedScanV2OwnershipToken?.side === "left" ||
    node?.generatedScanV2OwnershipToken?.side === "right") {
    return node.generatedScanV2OwnershipToken.side;
  }
  if (node?.generatedScanV2BranchLane?.side === "left" || node?.generatedScanV2BranchLane?.side === "right") {
    return node.generatedScanV2BranchLane.side;
  }
  if (node?.generatedScanV2LocalMotifMember?.role === "fallthroughArm") return "left";
  if (node?.generatedScanV2LocalMotifMember?.role === "jumpArm") return "right";
  return null;
}

function resolveGeneratedScanV2LoopTargetReturnPort(sourceNode, targetNode) {
  const ownershipTokenSide = sourceNode?.generatedScanV2OwnershipToken?.side;
  const sourceTokenSide = ownershipTokenSide === "left" || ownershipTokenSide === "right"
    ? ownershipTokenSide
    : getGeneratedScanV2NodeBranchSide(sourceNode);
  const isSplitTarget = targetNode?.kind === "conditionalJump";
  if (!isSplitTarget) {
    return {
      targetReturnPort: "none",
      targetReturnPortReason: "returnTargetIsNotSplit",
      sourceTokenSide,
    };
  }
  if (sourceTokenSide === "left" || sourceTokenSide === "right") {
    return {
      targetReturnPort: sourceTokenSide,
      targetReturnPortReason: ownershipTokenSide
        ? "sourceOwnershipTokenMatchesSplitPort"
        : "sourceBranchLaneFallbackMatchesSplitPort",
      sourceTokenSide,
    };
  }
  return {
    targetReturnPort: "ambiguous",
    targetReturnPortReason: "sourceOwnershipTokenUnavailable",
    sourceTokenSide: null,
  };
}

function classifyGeneratedScanV2LoopReturn({
  edge,
  row,
  sourceNode,
  targetNode,
  ...
  const sourceFrameId = sourceNode?.generatedScanV2LocalFrameId ?? null;
  const targetFrameId = targetNode?.generatedScanV2LocalFrameId ?? null;
  const instructionDistance = Number.isInteger(row?.sourceIndex) && Number.isInteger(row?.targetIndex)
    ? row.sourceIndex - row.targetIndex
    : null;
  const oldRouteBBox = getGeneratedScanV2RouteBBox(edge?.points ?? []);
  const oldRouteSideRelativeToInterval = classifyGeneratedScanV2LoopRouteSideRelativeToOwner({
    routeBBox: oldRouteBBox,
    ownerFrameBBox: intervalEnvelope,
    sideThreshold: Math.max(layout.sideRouteGap * 0.5, 4),
  });
  const compactClosureCandidate = Boolean(
    sourceFrameId &&
    sourceFrameId === targetFrameId &&
    Number.isInteger(instructionDistance) &&
    instructionDistance <= 4 &&
    oldRouteSideRelativeToInterval === "inside"
  );
  const hasTargetSplitPort = splitReturnPort.targetReturnPort === "left" ||
    splitReturnPort.targetReturnPort === "right";
  const hasDirectionalTargetAncestry = returnPointScope.inheritedClosureSide === "left" ||
    returnPointScope.inheritedClosureSide === "right";
  const isPreSplitAncestor = !hasTargetSplitPort &&
    !hasDirectionalTargetAncestry &&
    ownership.ownershipKind === "outerGlobalFrame";

  if (compactClosureCandidate) {
    return {
      returnClassification: "compactLocalClosure",
      returnClassificationReason: "sameLocalFrameShortReturnPreservesExistingInsideRoute",
      sideAuthority: "compactLocalClosure",
      preferredClosureSide: null,
      enclosingBodyFrameId: targetFrameId,
      enclosingBodyFrameSide: null,
      compactClosureCandidate,
      compactClosureAccepted: true,
      targetSplitPortUsedFor: hasTargetSplitPort ? "attachment" : "unavailable",
      oldRouteSideRelativeToInterval,
    };
  }

  if (isPreSplitAncestor) {
    return {
      returnClassification: "preSplitAncestorFrameClosure",
      returnClassificationReason: "returnTargetHasNoDirectionalSplitPortOrFrameAncestry",
      sideAuthority: "preSplitOuterContainmentCost",
      preferredClosureSide: null,
      enclosingBodyFrameId: null,
      enclosingBodyFrameSide: null,
      compactClosureCandidate,
      compactClosureAccepted: false,
      targetSplitPortUsedFor: "unavailable",
      oldRouteSideRelativeToInterval,
    };
  }

  const hasExistingBodyEnvelopeSide = oldRouteSideRelativeToInterval === "left" ||
    oldRouteSideRelativeToInterval === "right";
  const enclosingBodyFrameSide = hasExistingBodyEnvelopeSide
    ? oldRouteSideRelativeToInterval
    : hasDirectionalTargetAncestry
      ? returnPointScope.inheritedClosureSide
      : ownership.inheritedPreferredSide;
  if (enclosingBodyFrameSide === "left" || enclosingBodyFrameSide === "right") {
    return {
      returnClassification: "enclosingBodyFrameClosure",
      returnClassificationReason: hasExistingBodyEnvelopeSide
        ? "existingLoopBodyEnvelopeOwnsExteriorCorridor"
        : hasDirectionalTargetAncestry
          ? "nearestDirectionalReturnPointFrameOwnsExteriorCorridor"
          : "loopBodyOwnershipOwnsExteriorCorridor",
      sideAuthority: "enclosingBodyFrameSide",
      preferredClosureSide: enclosingBodyFrameSide,
      enclosingBodyFrameId: hasExistingBodyEnvelopeSide
        ? row?.ownerRegionId ?? null
        : hasDirectionalTargetAncestry
        ? returnPointScope.inheritedFromFrameId
        : null,
      enclosingBodyFrameSide,
      compactClosureCandidate,
      compactClosureAccepted: false,
      targetSplitPortUsedFor: hasTargetSplitPort ? "attachment" : "unavailable",
      oldRouteSideRelativeToInterval,
  ...
  return {
    returnClassification: "unknown",
    returnClassificationReason: "noCompactRouteOrDirectionalEnclosingFrameAvailable",
    sideAuthority: hasTargetSplitPort ? "targetFrameAncestryFallback" : "unknownFallback",
    preferredClosureSide: hasTargetSplitPort ? splitReturnPort.targetReturnPort : null,
    enclosingBodyFrameId: null,
    enclosingBodyFrameSide: null,
    compactClosureCandidate,
    compactClosureAccepted: false,
    targetSplitPortUsedFor: hasTargetSplitPort ? "corridorSide" : "unavailable",
    oldRouteSideRelativeToInterval,
  };
}

function resolveGeneratedScanV2LoopClosureSideFromReturnPointFrameAncestry(targetNode) {
  const ancestry = Array.isArray(targetNode?.generatedScanV2LocalFrameAncestry)
    ? targetNode.generatedScanV2LocalFrameAncestry
    : [];
  const directionalFrame = [...ancestry].reverse().find((frame) => (
    frame?.branchDirection === "left" || frame?.branchDirection === "right"
  )) ?? null;

  return {
    returnPointFrameId: targetNode?.generatedScanV2LocalFrameId ?? null,
    inheritedClosureSide: directionalFrame?.branchDirection ?? "none",
    inheritedFromFrameId: directionalFrame?.frameId ?? null,
    inheritedFromBranchDirection: directionalFrame?.branchDirection ?? null,
    ancestry,
  };
}

function resolveGeneratedScanV2LoopFrameOwnership({
  sourceNode,
  targetNode,
  intervalNodes = [],
  ownerRegion,
  ownerRegionBox,
  parentRegionBox,
  isOutermostLoopClosure,
}) {
  if (isOutermostLoopClosure) {
    return {
      ownershipKind: "outerGlobalFrame",
      owningBranchLane: null,
      inheritedPreferredSide: null,
      evidence: "outermostLoopClosure",
    };
  }

  const sourceSide = getGeneratedScanV2NodeBranchSide(sourceNode);
  if (sourceSide) {
    return {
      ownershipKind: `${sourceSide}BranchLane`,
      owningBranchLane: sourceSide,
      inheritedPreferredSide: sourceSide,
      evidence: "sourceNodeBranchLane",
    };
  }

  const targetSide = getGeneratedScanV2NodeBranchSide(targetNode);
  const intervalSideCounts = intervalNodes.reduce((counts, node) => {
    const side = getGeneratedScanV2NodeBranchSide(node);
    if (side) counts[side] += 1;
    return counts;
  }, { left: 0, right: 0 });
  if (intervalSideCounts.left !== intervalSideCounts.right) {
    const dominantSide = intervalSideCounts.left > intervalSideCounts.right ? "left" : "right";
    return {
      ownershipKind: `${dominantSide}BranchLane`,
      owningBranchLane: dominantSide,
      inheritedPreferredSide: dominantSide,
      evidence: "repeatedIntervalBranchLaneMajority",
      intervalSideCounts,
    };
  }
  if (targetSide) {
    return {
      ownershipKind: `${targetSide}BranchLane`,
      owningBranchLane: targetSide,
      inheritedPreferredSide: targetSide,
      evidence: "targetNodeBranchLane",
      intervalSideCounts,
    };
  }

  if (ownerRegion?.parentRegionId && ownerRegionBox && parentRegionBox) {
    const ownerCenterX = (ownerRegionBox.left + ownerRegionBox.right) / 2;
    const parentCenterX = (parentRegionBox.left + parentRegionBox.right) / 2;
    const offset = ownerCenterX - parentCenterX;
    if (Math.abs(offset) > 1) {
      const side = offset < 0 ? "left" : "right";
      return {
        ownershipKind: `${side}LocalRegion`,
        owningBranchLane: side,
        inheritedPreferredSide: side,
        evidence: "ownerRegionOffsetFromParent",
        intervalSideCounts,
      };
    }
  }
  ...
  return {
    ownershipKind: ownerRegion ? "centralMainRegion" : "unknown",
    owningBranchLane: null,
    inheritedPreferredSide: null,
    evidence: ownerRegion ? "ownerRegionCenteredOrAmbiguous" : "ownerRegionUnavailable",
    intervalSideCounts,
  };
}

function getGeneratedScanV2LoopFrameOverrideReason(inheritedCandidate, oppositeCandidate) {
  if (!inheritedCandidate || !oppositeCandidate) return null;
  const inherited = inheritedCandidate.cost.factors;
  const opposite = oppositeCandidate.cost.factors;

  if (opposite.nodeIntersectionCount < inherited.nodeIntersectionCount) {
    return "realGeometryCollision:nodeIntersection";
  }
  if (opposite.foreignRegionCount < inherited.foreignRegionCount) {
    return "parentBoundaryViolation:foreignRegionIntersection";
  }
  if (opposite.childIntersectionCount < inherited.childIntersectionCount) {
    return "parentBoundaryViolation:childRegionIntersection";
  }
  if (opposite.branchInteriorPressure < inherited.branchInteriorPressure) {
    return "realGeometryCollision:branchInteriorPressure";
  }
  if (!inherited.wrapsOwnerRegion && opposite.wrapsOwnerRegion) {
    return "parentBoundaryViolation:ownerWrapFailure";
  }
  if (
    opposite.nestedOrderingViolationCount < inherited.nestedOrderingViolationCount &&
    inheritedCandidate.cost.totalCost - oppositeCandidate.cost.totalCost >= 48
  ) {
    return "nestedOrderingViolation";
  }
  if (
    opposite.siblingScopeConflictCount < inherited.siblingScopeConflictCount &&
    inheritedCandidate.cost.totalCost - oppositeCandidate.cost.totalCost >= 48
  ) {
    return "siblingScopeCollision";
  }
  return null;
}

function chooseGeneratedScanV2LoopFrameSide(candidateRecords = [], inheritedPreferredSide = null) {
  const acceptedCandidates = candidateRecords.filter((candidate) => candidate.accepted);
  if (acceptedCandidates.length === 0) return null;

  const byCost = [...acceptedCandidates].sort((left, right) => (
    (left.cost.totalCost - right.cost.totalCost) ||
    (left.distanceFromOwnerBox - right.distanceFromOwnerBox) ||
    (left.structuralTieBreakPenalty - right.structuralTieBreakPenalty) ||
    left.laneSide.localeCompare(right.laneSide)
  ));
  const costSelected = byCost[0];
  if (inheritedPreferredSide !== "left" && inheritedPreferredSide !== "right") {
    return {
      candidate: costSelected,
      overrideReason: null,
      selectionReason: "selectedByLoopFrameSideCost",
    };
  }

  const inheritedCandidate = byCost.find((candidate) => candidate.laneSide === inheritedPreferredSide) ?? null;
  const oppositeCandidate = byCost.find((candidate) => candidate.laneSide !== inheritedPreferredSide) ?? null;
  if (!inheritedCandidate) {
    return {
      candidate: costSelected,
      overrideReason: "unknown",
      selectionReason: "overrodeInheritedLoopFrameSide",
    };
  }
  const overrideReason = getGeneratedScanV2LoopFrameOverrideReason(inheritedCandidate, oppositeCandidate);
  if (overrideReason && oppositeCandidate) {
    return {
  ...
  return {
    ...
    overrideReason: null,
    selectionReason: "selectedInheritedLoopFrameSide",
  };
}

function applyGeneratedScanV2LoopClosureRouting(layoutPlan, routedEdges) {
  if (layoutPlan.layoutMode !== "generatedScanV2") {
    return {
      routedEdges,
      debug: {
  ...
  const scanLayoutPlan = layoutPlan.generatedScanLayoutPlanDebug ?? null;
  const scanPlacement = layoutPlan.generatedScanV2PlacementDebug ?? null;
  if (!scanLayoutPlan?.enabled || !Array.isArray(scanLayoutPlan.edges)) {
    return {
      routedEdges,
      debug: {
        enabled: false,
        skippedReason: "generatedScanLayoutPlanUnavailable",
        attempts: [],
        appliedCount: 0,
        beforeEdges: routedEdges,
        beforeAudit: null,
  ...
  if (!scanPlacement?.enabled || !Array.isArray(scanPlacement.regionBoxes)) {
    return {
      routedEdges,
      debug: {
        enabled: false,
        skippedReason: "generatedScanV2PlacementUnavailable",
        attempts: [],
        appliedCount: 0,
        beforeEdges: routedEdges,
        beforeAudit: null,
  ...
  const contentEnvelope = getBoundsEnvelope([...nodeBoundsById.values()]);
  const branchInteriorPadding = Math.max(layoutPlan.layout.sideRouteGap * 0.75, 10);
  const branchInteriorBoxes = instructionNodes
    .filter((node) => (
      node.generatedScanV2BranchLane ||
      node.generatedScanV2LocalMotifMember?.role === "fallthroughArm" ||
      node.generatedScanV2LocalMotifMember?.role === "jumpArm"
    ))
    .map((node) => {
      const bounds = nodeBoundsById.get(node.id);
      return {
        left: bounds.left - branchInteriorPadding,
        right: bounds.right + branchInteriorPadding,
        top: bounds.top - branchInteriorPadding,
        bottom: bounds.bottom + branchInteriorPadding,
      };
    });
  const protectedRegions = buildProtectedRegionDiagnostics(
    layoutPlan,
  ...
  const regionById = new Map((scanLayoutPlan.regions ?? []).map((region) => [region.regionId, region]));
  const regionBoxById = new Map(
    (scanPlacement.regionBoxes ?? [])
      .filter((regionBox) => regionBox?.materialized)
      .map((regionBox) => [regionBox.regionId, regionBox]),
  );
  const localFrameById = new Map();
  const indexLocalFrame = (frame) => {
    if (!frame?.frameId || localFrameById.has(frame.frameId)) return;
    localFrameById.set(frame.frameId, frame);
    (frame.children ?? []).forEach(indexLocalFrame);
  };
  indexLocalFrame(scanPlacement.localFrameLayout?.rootFrame ?? null);
  const closureRows = (scanLayoutPlan.edges ?? []).filter((row) => (
    row.edgeClass === "loopClosure" || row.edgeClass === "nestedLoopClosure"
  ));
  ...
  if (closureRows.length === 0) {
    return {
      ...
      debug: {
        enabled: false,
        skippedReason: "noLoopClosureEdges",
        attempts: [],
        appliedCount: 0,
        beforeEdges: routedEdges,
        beforeAudit: null,
  ...
  let baselineEdgeFindingMap = buildEdgeFindingSummaryMap(baselineBundle.audit.findings);
  const attempts = [];
  const reservedLoopCorridors = [];
  const targetKinds = [
    "longDiagonalCrossesProtectedRegion",
    "edgePassesThroughForeignProtectedRegion",
    "edgeNodeIntersection",
  ...
  const laneBase = Math.max(layoutPlan.layout.loopLaneDistance * 1.1, layoutPlan.layout.sideRouteGap * 2, 24);
  const laneStep = Math.max(layoutPlan.layout.sideRouteGap, 14);
  const includeVerboseDiagnostics = isBetaFlowAuditDebugEnabled();

  closureRows.forEach((row) => {
    const edge = workingEdges.find((candidateEdge) => candidateEdge.id === row.edgeId) ?? null;
    const sourceNode = edge ? layoutPlan.nodeMap.get(edge.from) : null;
    const targetNode = edge ? layoutPlan.nodeMap.get(edge.to) : null;
    const ownerRegionId = row.ownerRegionId ?? row.sourceRegionId ?? row.targetRegionId ?? null;
    const ownerRegion = ownerRegionId ? regionById.get(ownerRegionId) ?? null : null;
    const ownerFrameId = ownerRegionId ? `scan-frame:region:${ownerRegionId}` : null;
    const loopNestingDepth = (() => {
      let depth = 0;
      let cursor = ownerRegion;
      const seenRegionIds = new Set();
      while (cursor?.parentRegionId && !seenRegionIds.has(cursor.parentRegionId)) {
        seenRegionIds.add(cursor.parentRegionId);
        cursor = regionById.get(cursor.parentRegionId) ?? null;
    ...
    const ownerRegionBox = ownerRegionId ? regionBoxById.get(ownerRegionId) ?? null : null;
    const parentRegionBox = ownerRegion?.parentRegionId
      ? regionBoxById.get(ownerRegion.parentRegionId) ?? null
      : null;
    const childRegionBoxes = (ownerRegion?.childrenRegionIds ?? [])
      .map((childRegionId) => regionBoxById.get(childRegionId))
      .filter(Boolean);
    const childEnvelope = childRegionBoxes.length > 0
      ? {
        left: Math.min(...childRegionBoxes.map((regionBox) => regionBox.left)),
        right: Math.max(...childRegionBoxes.map((regionBox) => regionBox.right)),
        top: Math.min(...childRegionBoxes.map((regionBox) => regionBox.top)),
        bottom: Math.max(...childRegionBoxes.map((regionBox) => regionBox.bottom)),
      }
      : null;
    const intervalNodes = instructionNodes.filter((node) => (
      Number.isInteger(node.displayNode?.startIndex) &&
      Number.isInteger(row.targetIndex) &&
      Number.isInteger(row.sourceIndex) &&
      row.targetIndex <= node.displayNode.startIndex &&
      node.displayNode.startIndex <= row.sourceIndex
    ));
    const intervalEnvelope = getBoundsEnvelope(
      intervalNodes.map((node) => nodeBoundsById.get(node.id)),
    );
    const routingRegionBox = ownerRegionBox ?? intervalEnvelope;
    const ownerEnvelope = routingRegionBox
      ? {
        left: Math.min(routingRegionBox.left, childEnvelope?.left ?? routingRegionBox.left),
        right: Math.max(routingRegionBox.right, childEnvelope?.right ?? routingRegionBox.right),
        top: Math.min(routingRegionBox.top, childEnvelope?.top ?? routingRegionBox.top),
        bottom: Math.max(routingRegionBox.bottom, childEnvelope?.bottom ?? routingRegionBox.bottom),
      }
      : null;
    const isOutermostLoopClosure =
      row.edgeClass === "loopClosure" &&
      (!ownerRegion?.parentRegionId || ownerRegion.parentRegionId === null);
    const laneClass = row.edgeClass === "nestedLoopClosure"
      ? "nestedLoopReturn"
      : isOutermostLoopClosure
        ? "outerLoopReturn"
        : "loopReturn";
    const ownership = resolveGeneratedScanV2LoopFrameOwnership({
      sourceNode,
      targetNode,
      intervalNodes,
      ownerRegion,
    ...
    const returnPointScope = resolveGeneratedScanV2LoopClosureSideFromReturnPointFrameAncestry(targetNode);
    const splitReturnPort = resolveGeneratedScanV2LoopTargetReturnPort(sourceNode, targetNode);
    const targetReturnPortSide = splitReturnPort.targetReturnPort === "left" ||
      splitReturnPort.targetReturnPort === "right"
      ? splitReturnPort.targetReturnPort
      : null;
    const inheritedClosureSide = returnPointScope.inheritedClosureSide === "left" ||
      returnPointScope.inheritedClosureSide === "right"
      ? returnPointScope.inheritedClosureSide
      : null;
    const returnClassification = classifyGeneratedScanV2LoopReturn({
      edge,
      row,
      sourceNode,
      targetNode,
    ...
    const preferredClosureSide = returnClassification.preferredClosureSide;
    const closureSideSource = returnClassification.sideAuthority;

    const attempt = {
      edgeId: row.edgeId,
      sourceIndex: row.sourceIndex,
      targetIndex: row.targetIndex,
      sourceCoordinates: sourceNode ? { x: sourceNode.x, y: sourceNode.y } : null,
      targetCoordinates: targetNode ? { x: targetNode.x, y: targetNode.y } : null,
      edgeClass: row.edgeClass,
      ownerRegionId,
      ownerFrameId,
      ownerRegionInterval: ownerRegion
        ? `${formatInstructionNodeId(ownerRegion.startIndex)}-${formatInstructionNodeId(ownerRegion.endIndex)}`
        : null,
      ownerRegionDepth: ownerRegion?.depth ?? null,
      ownerRegionParentId: ownerRegion?.parentRegionId ?? null,
      ownerRegionChildIds: ownerRegion?.childrenRegionIds ?? [],
      laneClass,
      ownershipKind: ownership.ownershipKind,
      owningBranchLane: ownership.owningBranchLane,
      ownershipEvidence: ownership.evidence,
      ownershipIntervalSideCounts: ownership.intervalSideCounts ?? null,
      inheritedPreferredSide: preferredClosureSide,
      loopSourceInstruction: row.sourceIndex,
      loopReturnInstruction: row.targetIndex,
      returnPointFrameId: returnPointScope.returnPointFrameId,
      inheritedClosureSide: inheritedClosureSide ?? "none",
      inheritedFromFrameId: returnPointScope.inheritedFromFrameId,
      inheritedFromBranchDirection: returnPointScope.inheritedFromBranchDirection,
      targetReturnPort: splitReturnPort.targetReturnPort,
      targetReturnPortReason: splitReturnPort.targetReturnPortReason,
      sourceTokenSide: splitReturnPort.sourceTokenSide,
      targetPortSide: targetReturnPortSide,
      closureSideSource,
      returnClassification: returnClassification.returnClassification,
      returnClassificationReason: returnClassification.returnClassificationReason,
      sideAuthority: returnClassification.sideAuthority,
      enclosingBodyFrameId: returnClassification.enclosingBodyFrameId,
      enclosingBodyFrameSide: returnClassification.enclosingBodyFrameSide,
      compactClosureCandidate: returnClassification.compactClosureCandidate,
      compactClosureAccepted: returnClassification.compactClosureAccepted,
      targetSplitPort: splitReturnPort.targetReturnPort,
      targetSplitPortUsedFor: returnClassification.targetSplitPortUsedFor,
      tunedGroundTruthExpectation: null,
      generatedBeforeSide: returnClassification.oldRouteSideRelativeToInterval,
      generatedAfterSide: null,
      safetyOverrideReason: null,
      splitPortRejectedBySafetyGate: false,
      splitPortSafetyGateReason: null,
      closureSideFallbackEvidence: targetReturnPortSide || inheritedClosureSide
        ? null
        : ownership.evidence,
      ...(includeVerboseDiagnostics ? {
        returnPointFrameAncestry: returnPointScope.ancestry,
      } : {}),
      regionBoxUsed: ownerRegionBox
        ? {
          left: ownerRegionBox.left,
          right: ownerRegionBox.right,
          top: ownerRegionBox.top,
          bottom: ownerRegionBox.bottom,
        }
        : null,
      intervalEnvelopeUsed: intervalEnvelope,
      parentRegionBoxUsed: parentRegionBox
        ? {
          left: parentRegionBox.left,
          right: parentRegionBox.right,
          top: parentRegionBox.top,
          bottom: parentRegionBox.bottom,
        }
        : null,
      ownerEnvelopeUsed: ownerEnvelope
        ? {
          left: ownerEnvelope.left,
          right: ownerEnvelope.right,
          top: ownerEnvelope.top,
          bottom: ownerEnvelope.bottom,
        }
        : null,
      childEnvelopeUsed: childEnvelope
        ? {
          left: childEnvelope.left,
          right: childEnvelope.right,
          top: childEnvelope.top,
          bottom: childEnvelope.bottom,
        }
        : null,
      childRegionBoxes: childRegionBoxes.map((childRegionBox) => ({
        regionId: childRegionBox.regionId,
        interval: childRegionBox.interval,
        left: childRegionBox.left,
        right: childRegionBox.right,
        top: childRegionBox.top,
        bottom: childRegionBox.bottom,
      })),
      laneSide: null,
      laneX: null,
      ...(includeVerboseDiagnostics ? {
        oldRoutePoints: edge?.points ?? [],
        newRoutePoints: null,
      } : {}),
      oldFindingSummary: baselineEdgeFindingMap.get(row.edgeId) ?? {
        totalCount: 0,
        highCount: 0,
        kindCounts: {},
      },
      newFindingSummary: null,
      oldEvaluation: null,
      newEvaluation: null,
      childRegionIntersectionCountBefore: null,
      childRegionIntersectionCountAfter: null,
      ...(includeVerboseDiagnostics ? { candidateEvaluations: [] } : {}),
      leftCandidateCost: null,
      rightCandidateCost: null,
      selectedSide: null,
      finalClosureSide: null,
      inheritedSideOverridden: false,
      overrideReason: null,
      closureSideOverrideReason: null,
      finalOverrideReason: null,
      loopNestingDepth,
      corridorRankOnSide: null,
      comparedReservedCorridorCount: null,
      sameScopeConflictCount: null,
      ancestorCorridorCount: null,
      descendantCorridorCount: null,
      siblingScopeConflictCount: null,
      unrelatedAbsoluteConflictCount: null,
      hardGeometryCollisionCount: null,
      sourceX: sourceNode?.x ?? null,
      returnX: targetNode?.x ?? null,
      ownerFrameBBox: null,
      returnPointFrameBBox: null,
      edgeRouteBBox: null,
      edgeRouteCenterX: null,
      edgeRouteMinX: null,
      edgeRouteMaxX: null,
      actualRenderedSideRelativeToOwner: "unknown",
      sideGeometryMatchesFinalSide: false,
      sideGeometryMismatchReason: "routeGeometryUnavailable",
      routeSideOverrideReason: null,
      finalSideHonoredByRoute: false,
      mainCostReason: null,
      defaultedLeftWithoutScoring: false,
      applied: false,
      reason: "notEvaluated",
    };

    if (!edge || !sourceNode || !targetNode) {
      attempt.reason = "missingEdgeOrNodes";
      attempts.push(attempt);
      return;
    }
    if (!routingRegionBox || !ownerEnvelope) {
      attempt.reason = "routingEnvelopeUnavailable";
      attempt.defaultedLeftWithoutScoring = true;
      attempts.push(attempt);
      return;
    }
    if (returnClassification.returnClassification === "compactLocalClosure") {
      Object.assign(attempt, buildGeneratedScanV2LoopSideGeometryDiagnostics({
        points: edge.points ?? [],
        ownerFrameBBox: routingRegionBox,
        returnPointFrameBBox: localFrameById.get(returnPointScope.returnPointFrameId)?.box ?? null,
        finalClosureSide: null,
        sideThreshold: Math.max(layoutPlan.layout.sideRouteGap * 0.5, 4),
      }));
      attempt.selectedSide = "inside";
      attempt.finalClosureSide = "inside";
      attempt.generatedAfterSide = "inside";
      attempt.finalSideHonoredByRoute = attempt.actualRenderedSideRelativeToOwner === "inside";
      attempt.sideGeometryMatchesFinalSide = attempt.finalSideHonoredByRoute;
      attempt.sideGeometryMismatchReason = attempt.finalSideHonoredByRoute
        ? null
        : "compactRouteLeavesOwnerFrame";
      attempt.applied = true;
      attempt.reason = "preservedCompactLocalClosureRoute";
      attempts.push(attempt);
      return;
    }

    const allowedRegionIds = new Set(
      [row.ownerRegionId, row.sourceRegionId, row.targetRegionId].filter(Boolean),
    );
    const oldEvaluation = evaluateEdgeHighSeverityViolations({
      edge,
      points: edge.points ?? [],
    ...
    attempt.oldEvaluation = oldEvaluation;
    const childIntersectionBefore = countEdgeRegionBoxIntersections(edge.points ?? [], childRegionBoxes);
    attempt.childRegionIntersectionCountBefore = childIntersectionBefore;

    const depth = ownerRegion?.depth ?? 0;
    const isNestedClosure = row.edgeClass === "nestedLoopClosure";
    const laneDistance = isNestedClosure
      ? laneBase + Math.max(0, depth - 1) * laneStep * 0.75
      : laneBase + laneStep * (2 + Math.max(0, depth - 1));
    const farOuterDistance = laneDistance + Math.max(
      layoutPlan.layout.sideHaltDistance,
      Math.round(layoutPlan.layout.loopLaneDistance * 2.5),
      96,
    );
    const nearLeftX = ownerEnvelope.left - laneDistance;
    const nearRightX = ownerEnvelope.right + laneDistance;
    const farLeftX = ownerEnvelope.left - farOuterDistance;
    const farRightX = ownerEnvelope.right + farOuterDistance;

    const preferredSide = row.preferredExitSide === "left" || row.preferredExitSide === "right"
      ? row.preferredExitSide
      : null;
    const nearCandidates = preferredSide === "left"
      ? [
        { laneSide: "left", laneX: nearLeftX, candidateKind: "nearLeft" },
        { laneSide: "right", laneX: nearRightX, candidateKind: "nearRight" },
      ]
      : preferredSide === "right"
        ? [
          { laneSide: "right", laneX: nearRightX, candidateKind: "nearRight" },
    ...
    const farCandidates = isOutermostLoopClosure
      ? (preferredSide === "left"
        ? [
          { laneSide: "left", laneX: farLeftX, candidateKind: "farLeft" },
          { laneSide: "right", laneX: farRightX, candidateKind: "farRight" },
        ]
        : preferredSide === "right"
          ? [
            { laneSide: "right", laneX: farRightX, candidateKind: "farRight" },
      ...
      : [];
    const candidateSpecs = [...nearCandidates, ...farCandidates];

    const oldKindCounts = attempt.oldFindingSummary.kindCounts ?? {};
    const oldTargetKindCount = targetKinds.reduce(
      (count, kind) => count + (oldKindCounts[kind] ?? 0),
      0,
    );
    const oldHighCount = attempt.oldFindingSummary.highCount ?? 0;
    const oldNodeIntersectionCount = oldEvaluation.intersectingNodeIds.length;
    const contentCenterX = contentEnvelope
      ? (contentEnvelope.left + contentEnvelope.right) / 2
      : 0;
    const structuralTieBreakSide = ((sourceNode.x + targetNode.x) / 2) >= contentCenterX
      ? "right"
      : "left";
    attempt.structuralTieBreakSide = structuralTieBreakSide;

    let bestCandidate = null;
    const candidateRecords = [];

    candidateSpecs.forEach((candidateSpec) => {
      const laneSide = candidateSpec.laneSide;
      const laneX = candidateSpec.laneX;
      const distanceFromOwnerBox = laneSide === "left"
        ? ownerEnvelope.left - laneX
        : laneX - ownerEnvelope.right;
      const candidatePoints = buildScanV2LoopClosureRoute({
        sourceNode,
        targetNode,
        layout: layoutPlan.layout,
        laneX,
      ...
      const hasFinitePoints = candidatePoints.length >= 2 &&
        candidatePoints.every(([x, y]) => Number.isFinite(x) && Number.isFinite(y));
      const candidateDebug = {
        laneClass,
        candidateKind: candidateSpec.candidateKind,
        laneSide,
        laneX: Number(laneX.toFixed(2)),
        distanceFromOwnerBox: Number(distanceFromOwnerBox.toFixed(2)),
        ownerEnvelopeUsed: ownerEnvelope
          ? {
            left: ownerEnvelope.left,
            right: ownerEnvelope.right,
        ...
        candidatePoints,
        hasFinitePoints,
        candidateConnected: hasFinitePoints,
        accepted: false,
        rejectionReason: null,
        oldFindingSummary: attempt.oldFindingSummary,
        newFindingSummary: null,
        oldEvaluation: attempt.oldEvaluation,
        newEvaluation: null,
        childRegionIntersectionCountBefore: childIntersectionBefore,
        childRegionIntersectionCountAfter: null,
        score: null,
        cost: null,
        structuralTieBreakPenalty: laneSide === structuralTieBreakSide ? 0 : 1,
      };
      if (!hasFinitePoints) {
        candidateDebug.rejectionReason = "invalidCandidatePoints";
        if (includeVerboseDiagnostics) attempt.candidateEvaluations.push(candidateDebug);
        return;
      }

      const candidateEdges = cloneRoutedEdges(workingEdges);
      const candidateEdge = candidateEdges.find((candidate) => candidate.id === edge.id);
      if (!candidateEdge) {
        candidateDebug.rejectionReason = "candidateEdgeNotFound";
        if (includeVerboseDiagnostics) attempt.candidateEvaluations.push(candidateDebug);
        return;
      }
      candidateEdge.points = candidatePoints;
      if (candidateEdge.branch && candidatePoints.length >= 2) {
        const [p0, p1] = candidatePoints;
        candidateEdge.labelX = Number(((p0[0] + p1[0]) / 2).toFixed(2));
        candidateEdge.labelY = Number(((p0[1] + p1[1]) / 2 - 3).toFixed(2));
      }
      ...
      const candidateBundle = computeAuditBundle(candidateEdges);
      const candidateFindingMap = buildEdgeFindingSummaryMap(candidateBundle.audit.findings);
      const newFindingSummary = candidateFindingMap.get(edge.id) ?? {
        totalCount: 0,
        highCount: 0,
        kindCounts: {},
      };
      ...
      const childIntersectionAfter = countEdgeRegionBoxIntersections(candidatePoints, childRegionBoxes);
      const newKindCounts = newFindingSummary.kindCounts ?? {};
      const newTargetKindCount = targetKinds.reduce(
        (count, kind) => count + (newKindCounts[kind] ?? 0),
        0,
      );
      const highCountDelta = oldHighCount - (newFindingSummary.highCount ?? 0);
      const targetKindDelta = oldTargetKindCount - newTargetKindCount;
      const nodeIntersectionDelta = oldNodeIntersectionCount - newEvaluation.intersectingNodeIds.length;
      const childIntersectionDelta = childIntersectionBefore - childIntersectionAfter;
      const connected = true;
      const accepted = connected;
      const cost = evaluateGeneratedScanV2LoopFrameCandidateCost({
        candidatePoints,
        laneSide,
        laneX,
        ownerEnvelope,
        ...
        newFindingSummary,
        newEvaluation,
        childIntersectionCount: childIntersectionAfter,
        includeVerboseDiagnostics,
      });
      ...
      const candidateRecord = {
        ...
        newFindingSummary,
        newEvaluation,
        childIntersectionAfter,
        score: {
          highCountDelta,
          targetKindDelta,
          nodeIntersectionDelta,
          childIntersectionDelta,
        },
        cost,
        structuralTieBreakPenalty: laneSide === structuralTieBreakSide ? 0 : 1,
        accepted,
      };
      candidateDebug.newFindingSummary = newFindingSummary;
      candidateDebug.newEvaluation = newEvaluation;
      candidateDebug.childRegionIntersectionCountAfter = childIntersectionAfter;
      candidateDebug.score = candidateRecord.score;
      candidateDebug.cost = cost;
      candidateDebug.accepted = accepted;
      candidateDebug.rejectionReason = accepted ? null : "candidateNotConnected";
      if (includeVerboseDiagnostics) attempt.candidateEvaluations.push(candidateDebug);

      if (!accepted) return;
      candidateRecords.push(candidateRecord);
    });
    const sideSelection = chooseGeneratedScanV2LoopFrameSide(
      candidateRecords,
      preferredClosureSide,
    );
    bestCandidate = sideSelection?.candidate ?? null;
    attempt.leftCandidateCost = candidateRecords
      .filter((candidate) => candidate.laneSide === "left")
      .reduce((best, candidate) => Math.min(best, candidate.cost.totalCost), Number.POSITIVE_INFINITY);
    attempt.rightCandidateCost = candidateRecords
      .filter((candidate) => candidate.laneSide === "right")
      .reduce((best, candidate) => Math.min(best, candidate.cost.totalCost), Number.POSITIVE_INFINITY);
    if (!Number.isFinite(attempt.leftCandidateCost)) attempt.leftCandidateCost = null;
    if (!Number.isFinite(attempt.rightCandidateCost)) attempt.rightCandidateCost = null;

    if (!bestCandidate) {
      attempt.reason = "noAcceptableCandidate";
      attempts.push(attempt);
      return;
    }

    attempt.laneClass = bestCandidate.laneClass;
    attempt.candidateKind = bestCandidate.candidateKind;
    workingEdges = bestCandidate.candidateEdges;
    baselineBundle = bestCandidate.candidateBundle;
    baselineEdgeFindingMap = bestCandidate.candidateFindingMap;

    attempt.laneSide = bestCandidate.laneSide;
    attempt.selectedSide = bestCandidate.laneSide;
    attempt.finalClosureSide = bestCandidate.laneSide;
    attempt.generatedAfterSide = bestCandidate.laneSide;
    attempt.inheritedSideOverridden = Boolean(
      preferredClosureSide &&
      bestCandidate.laneSide !== preferredClosureSide
    );
    attempt.overrideReason = sideSelection?.overrideReason ?? null;
    attempt.closureSideOverrideReason = sideSelection?.overrideReason ?? null;
    attempt.finalOverrideReason = sideSelection?.overrideReason ?? null;
    if (sideSelection?.overrideReason) {
      attempt.sideAuthority = "safetyOverride";
      attempt.safetyOverrideReason = sideSelection.overrideReason;
    }
    attempt.splitPortRejectedBySafetyGate = Boolean(
      targetReturnPortSide &&
      preferredClosureSide === targetReturnPortSide &&
      bestCandidate.laneSide !== targetReturnPortSide
    );
    attempt.splitPortSafetyGateReason = attempt.splitPortRejectedBySafetyGate
      ? sideSelection?.overrideReason ?? "unknown"
      : null;
    attempt.closureFollowsTargetPort = Boolean(
      targetReturnPortSide &&
      bestCandidate.laneSide === targetReturnPortSide
    );
    attempt.closureFollowsSourceToken = Boolean(
      splitReturnPort.sourceTokenSide &&
      bestCandidate.laneSide === splitReturnPort.sourceTokenSide
    );
    attempt.laneX = bestCandidate.laneX;
    attempt.distanceFromOwnerBox = bestCandidate.distanceFromOwnerBox;
    if (includeVerboseDiagnostics) attempt.newRoutePoints = bestCandidate.points;
    attempt.newFindingSummary = bestCandidate.newFindingSummary;
    attempt.newEvaluation = bestCandidate.newEvaluation;
    attempt.childRegionIntersectionCountAfter = bestCandidate.childIntersectionAfter;
    attempt.mainCostReason = bestCandidate.cost.mainCostReason;
    attempt.comparedReservedCorridorCount = bestCandidate.cost.factors.comparedReservedCorridorCount;
    attempt.sameScopeConflictCount = bestCandidate.cost.factors.sameScopeConflictCount;
    attempt.ancestorCorridorCount = bestCandidate.cost.factors.ancestorCorridorCount;
    attempt.descendantCorridorCount = bestCandidate.cost.factors.descendantCorridorCount;
    attempt.siblingScopeConflictCount = bestCandidate.cost.factors.siblingScopeConflictCount;
    attempt.unrelatedAbsoluteConflictCount = bestCandidate.cost.factors.unrelatedAbsoluteConflictCount;
    attempt.hardGeometryCollisionCount = bestCandidate.cost.factors.hardGeometryCollisionCount;
    const returnPointFrameBBox = localFrameById.get(returnPointScope.returnPointFrameId)?.box ?? null;
    Object.assign(attempt, buildGeneratedScanV2LoopSideGeometryDiagnostics({
      points: bestCandidate.points,
      ownerFrameBBox: routingRegionBox,
      returnPointFrameBBox,
      finalClosureSide: bestCandidate.laneSide,
      sideThreshold: Math.max(layoutPlan.layout.sideRouteGap * 0.5, 4),
    }));
    attempt.applied = true;
    attempt.reason = sideSelection?.selectionReason ?? "selectedByLoopFrameSideCost";
    const candidateTop = Math.min(...bestCandidate.points.map(([, y]) => y));
    const candidateBottom = Math.max(...bestCandidate.points.map(([, y]) => y));
    attempt.corridorRankOnSide = reservedLoopCorridors.filter((corridor) => (
      corridor.laneSide === bestCandidate.laneSide &&
      Math.min(candidateBottom, corridor.bottom) -
        Math.max(candidateTop, corridor.top) > 8
    )).length + 1;
    reservedLoopCorridors.push({
      edgeId: row.edgeId,
      laneSide: bestCandidate.laneSide,
      laneX: bestCandidate.laneX,
      top: candidateTop,
      bottom: candidateBottom,
      ownerRegionId,
      ownerFrameId,
      loopNestingDepth,
      corridorRankOnSide: attempt.corridorRankOnSide,
    });
    attempts.push(attempt);
  });
  ...
  const rejectedReasonCounts = attempts
  ...
  const loopSideGeometryMismatchEdges = attempts
    .filter((attempt) => attempt.applied && !attempt.finalSideHonoredByRoute)
    .map((attempt) => attempt.edgeId);
  const safetyOverrideGeometryRows = includeVerboseDiagnostics
    ? attempts
      .filter((attempt) => attempt.safetyOverrideReason)
      .map((attempt) => {
        const rejectedCandidate = (attempt.candidateEvaluations ?? []).find((candidate) => (
          candidate.laneSide === attempt.inheritedPreferredSide
        )) ?? null;
        const offendingNodeIds = rejectedCandidate?.newEvaluation?.intersectingNodeIds ?? [];
        const offendingRegionIds = rejectedCandidate?.newEvaluation?.foreignRegionIds ?? [];
        return {
          edgeId: attempt.edgeId,
          sourceInstruction: attempt.loopSourceInstruction,
          targetInstruction: attempt.loopReturnInstruction,
          rejectedSide: attempt.inheritedPreferredSide,
          acceptedSide: attempt.finalClosureSide,
          rejectionReason: attempt.safetyOverrideReason,
          exactOffendingNodeIds: offendingNodeIds,
          exactOffendingRegionIds: offendingRegionIds,
          offendingNodeBBoxes: offendingNodeIds.map((nodeId) => ({
            nodeId,
            bbox: nodeBoundsById.get(nodeId) ?? null,
          })),
          offendingRegionBBoxes: offendingRegionIds.map((regionId) => {
            const region = protectedRegions.find((candidate) => candidate.id === regionId) ?? null;
            return {
              regionId,
              bbox: region?.bounds ?? null,
            };
          }),
          candidateRouteBBox: getGeneratedScanV2RouteBBox(rejectedCandidate?.candidatePoints ?? []),
          localOwnerBBox: attempt.intervalEnvelopeUsed ?? null,
          broaderParentBBox: attempt.parentRegionBoxUsed ?? null,
          ownerEnvelopeUsed: attempt.ownerEnvelopeUsed ?? null,
          childEnvelopeUsed: attempt.childEnvelopeUsed ?? null,
        };
      })
    : [];
  ...
  return {
    ...
    debug: {
      ...
      skippedReason: null,
      attempts,
      appliedCount: attempts.filter((attempt) => attempt.applied).length,
      beforeEdges: routedEdges,
      beforeAudit: {
        findingCount: beforeAuditBundle.audit.findings.length,
        severityCounts: beforeAuditBundle.audit.severityCounts,
      ...
      auditComparison,
      rejectedReasonCounts,
      defaultedLeftWithoutScoringCount: attempts.filter(
        (attempt) => attempt.defaultedLeftWithoutScoring,
      ).length,
      sideGeometryMismatchCount: loopSideGeometryMismatchEdges.length,
...
function buildGeneratedScanV2VisualReadabilityDiagnostics(layoutPlan) {
  if (layoutPlan.layoutMode !== "generatedScanV2") {
    return {
      available: false,
      skippedReason: "layoutModeNotGeneratedScanV2",
  ...
  if (invalidGeometryReason) {
    return {
      available: false,
      skippedReason: invalidGeometryReason,
    };
  }
  ...
  if (instructionNodes.length === 0) {
    return {
      available: false,
      skippedReason: "noInstructionNodes",
    };
  }

  const renderScale = computeGraphRenderScale(layoutPlan);
  const instructionBounds = instructionNodes.map((node) => {
    const bounds = getNodeBounds(node, layoutPlan.layout);
    return {
      nodeId: node.id,
      instructionIndex: node.instructionIndex,
      left: bounds.left + layoutPlan.offsetX,
      right: bounds.right + layoutPlan.offsetX,
      top: bounds.top + layoutPlan.offsetY,
      bottom: bounds.bottom + layoutPlan.offsetY,
      centerX: node.x + layoutPlan.offsetX,
      centerY: node.y + layoutPlan.offsetY,
    };
  });

  const minX = Math.min(...instructionBounds.map((row) => row.left));
  const maxX = Math.max(...instructionBounds.map((row) => row.right));
  const minY = Math.min(...instructionBounds.map((row) => row.top));
  const maxY = Math.max(...instructionBounds.map((row) => row.bottom));
  const visibleNodeWidth = maxX - minX;
  const visibleNodeHeight = maxY - minY;
  const canvasWidth = layoutPlan.width;
  const canvasHeight = layoutPlan.height;
  const topWhitespaceBeforeFirstNode = Math.max(0, minY);
  const bottomWhitespaceAfterLastNode = Math.max(0, canvasHeight - maxY);
  const leftWhitespaceBeforeFirstNode = Math.max(0, minX);
  const rightWhitespaceAfterLastNode = Math.max(0, canvasWidth - maxX);
  const visibleNodeBBoxToCanvasHeightRatio = canvasHeight > 0
    ? Number((visibleNodeHeight / canvasHeight).toFixed(4))
    : 0;
  const visibleNodeBBoxToCanvasWidthRatio = canvasWidth > 0
    ? Number((visibleNodeWidth / canvasWidth).toFixed(4))
    : 0;
  const graphCenterY = minY + visibleNodeHeight / 2;
  const graphCenterX = minX + visibleNodeWidth / 2;
  const graphCenterOffsetYFromCanvasCenter = Number((graphCenterY - canvasHeight / 2).toFixed(2));
  const graphCenterOffsetXFromCanvasCenter = Number((graphCenterX - canvasWidth / 2).toFixed(2));

  const edgeLengths = (layoutPlan.edges ?? [])
    .map((edge) => {
      const points = Array.isArray(edge?.points) ? edge.points : [];
    ...
    .sort((left, right) => left - right);
  const medianEdgeLength = edgeLengths.length > 0
    ? edgeLengths[Math.floor(edgeLengths.length / 2)]
    : 0;
  const maxEdgeLength = edgeLengths.length > 0
    ? edgeLengths[edgeLengths.length - 1]
    : 0;

  const nodeSpacingSamples = (layoutPlan.edges ?? [])
    .filter((edge) => (
      Number.isInteger(edge.sourceIndex) &&
      Number.isInteger(edge.targetIndex) &&
      edge.targetIndex === edge.sourceIndex + 1
    ...
    .map((edge) => {
      const source = instructionBounds.find((row) => row.instructionIndex === edge.sourceIndex);
      const target = instructionBounds.find((row) => row.instructionIndex === edge.targetIndex);
      if (!source || !target) return null;
      return Math.hypot(target.centerX - source.centerX, target.centerY - source.centerY);
    })
    .filter((value) => Number.isFinite(value) && value > 0)
    .sort((left, right) => left - right);
  const medianNodeToNodeSpacing = nodeSpacingSamples.length > 0
    ? nodeSpacingSamples[Math.floor(nodeSpacingSamples.length / 2)]
    : medianEdgeLength;
  const longEdgeThreshold = Math.max(1, medianNodeToNodeSpacing * 2.5);
  const longEdgeCount = edgeLengths.filter((length) => length > longEdgeThreshold).length;

  const scaledActionHeight = layoutPlan.layout.actionNodeHeight * renderScale;
  const scaledActionWidth = layoutPlan.layout.actionNodeWidth * renderScale;
  const unreadableThresholdHeight = 16;
  const unreadableThresholdWidth = 50;
  const unreadableNodeCount = (scaledActionHeight < unreadableThresholdHeight || scaledActionWidth < unreadableThresholdWidth)
    ? instructionNodes.length
    : 0;

  const excessiveTopWhitespace =
    topWhitespaceBeforeFirstNode > canvasHeight * 0.25 ||
    topWhitespaceBeforeFirstNode > visibleNodeHeight * 2;
  const graphTooSmallForCanvas = visibleNodeBBoxToCanvasHeightRatio < 0.38;
  const graphVerticallyMiscentered = Math.abs(graphCenterOffsetYFromCanvasCenter) > canvasHeight * 0.22;
  const excessiveLongEdges = longEdgeCount >= Math.max(5, Math.round((layoutPlan.edges?.length ?? 0) * 0.15));
  const generatedScanV2VisuallyReadable = !(
    excessiveTopWhitespace ||
    graphTooSmallForCanvas ||
    graphVerticallyMiscentered ||
    excessiveLongEdges ||
  ...
  return {
    ...
    visibleNodeWidth: Number(visibleNodeWidth.toFixed(2)),
    visibleNodeHeight: Number(visibleNodeHeight.toFixed(2)),
    canvasWidth: Number(canvasWidth.toFixed(2)),
    canvasHeight: Number(canvasHeight.toFixed(2)),
    topWhitespaceBeforeFirstNode: Number(topWhitespaceBeforeFirstNode.toFixed(2)),
    bottomWhitespaceAfterLastNode: Number(bottomWhitespaceAfterLastNode.toFixed(2)),
    leftWhitespaceBeforeFirstNode: Number(leftWhitespaceBeforeFirstNode.toFixed(2)),
    rightWhitespaceAfterLastNode: Number(rightWhitespaceAfterLastNode.toFixed(2)),
    visibleNodeBBoxToCanvasHeightRatio,
    visibleNodeBBoxToCanvasWidthRatio,
    graphCenterOffsetYFromCanvasCenter,
    graphCenterOffsetXFromCanvasCenter,
    medianEdgeLength: Number(medianEdgeLength.toFixed(2)),
    maxEdgeLength: Number(maxEdgeLength.toFixed(2)),
    longEdgeThreshold: Number(longEdgeThreshold.toFixed(2)),
    longEdgeCount,
    tinyNodeCount: unreadableNodeCount,
    unreadableNodeCount,
    scaledActionHeight: Number(scaledActionHeight.toFixed(2)),
    scaledActionWidth: Number(scaledActionWidth.toFixed(2)),
    excessiveTopWhitespace,
    graphTooSmallForCanvas,
    graphVerticallyMiscentered,
    excessiveLongEdges,
...
function buildGeneratedScanV2VisualGrammarDiagnostics(layoutPlan) {
  if (layoutPlan.layoutMode !== "generatedScanV2") {
    return {
      available: false,
      skippedReason: "layoutModeNotGeneratedScanV2",
  ...
  const renderScale = computeGraphRenderScale(layoutPlan);
  const toRenderedBounds = (node) => {
    const bounds = getNodeBounds(node, layoutPlan.layout);
    return {
      nodeId: node.id,
      instructionIndex: node.instructionIndex,
      left: (bounds.left + layoutPlan.offsetX) * renderScale,
      right: (bounds.right + layoutPlan.offsetX) * renderScale,
      top: (bounds.top + layoutPlan.offsetY) * renderScale,
      bottom: (bounds.bottom + layoutPlan.offsetY) * renderScale,
      centerX: (node.x + layoutPlan.offsetX) * renderScale,
      centerY: (node.y + layoutPlan.offsetY) * renderScale,
    };
  };
  const instructionBounds = instructionNodes.map(toRenderedBounds);
  const instructionBoundsByIndex = new Map(
    instructionBounds
      .filter((bounds) => Number.isInteger(bounds.instructionIndex))
      .map((bounds) => [bounds.instructionIndex, bounds]),
  );
  const renderedBoundsByNodeId = new Map(
    layoutPlan.nodes.map((node) => [node.id, toRenderedBounds(node)]),
  );
  const minX = Math.min(...instructionBounds.map((bounds) => bounds.left));
  const maxX = Math.max(...instructionBounds.map((bounds) => bounds.right));
  const minY = Math.min(...instructionBounds.map((bounds) => bounds.top));
  ...
  const visibleNodeWidth = maxX - minX;
  const visibleNodeHeight = maxY - minY;
  const canvasWidth = layoutPlan.width * renderScale;
  const canvasHeight = layoutPlan.height * renderScale;
  const topWhitespaceBeforeFirstNode = Math.max(0, minY);
  const bottomWhitespaceAfterLastNode = Math.max(0, canvasHeight - maxY);
  const visibleNodeBBoxToCanvasHeightRatio = canvasHeight > 0 ? visibleNodeHeight / canvasHeight : 0;
  const graphCenterOffsetYFromCanvasCenter = minY + visibleNodeHeight / 2 - canvasHeight / 2;

  const routedEdges = layoutPlan.edges ?? [];
  const routedEdgeRows = routedEdges
    .map((edge) => {
      if (!Array.isArray(edge.points) || edge.points.length < 2) return null;
      return {
        edgeId: edge.id,
        sourceIndex: edge.sourceIndex ?? null,
        targetIndex: Number.isInteger(edge.targetIndex) ? edge.targetIndex : null,
        length: getPolylineLength(edge.points) * renderScale,
        renderedPoints: edge.points.map(([x, y]) => [
          Number(((x + layoutPlan.offsetX) * renderScale).toFixed(2)),
          Number(((y + layoutPlan.offsetY) * renderScale).toFixed(2)),
        ]),
      };
    })
    .filter(Boolean);
  const routedEdgeRowById = new Map(routedEdgeRows.map((row) => [row.edgeId, row]));
  const edgeLengths = routedEdgeRows
    .map((row) => row.length)
    .filter((length) => Number.isFinite(length) && length > 0)
    .sort((left, right) => left - right);
  const medianEdgeLength = edgeLengths.length > 0 ? edgeLengths[Math.floor(edgeLengths.length / 2)] : 0;
  const maxEdgeLength = edgeLengths.length > 0 ? edgeLengths.at(-1) : 0;
  const nodeSpacingSamples = routedEdges
    .filter((edge) => (
      Number.isInteger(edge.sourceIndex) &&
      Number.isInteger(edge.targetIndex) &&
      edge.targetIndex === edge.sourceIndex + 1
    ...
    .map((edge) => {
      const source = instructionBoundsByIndex.get(edge.sourceIndex);
      const target = instructionBoundsByIndex.get(edge.targetIndex);
      if (!source || !target) return null;
      return Math.hypot(target.centerX - source.centerX, target.centerY - source.centerY);
    })
  ...
  const medianNodeToNodeSpacing = nodeSpacingSamples.length > 0
    ...
    : medianEdgeLength;
  const longEdgeThreshold = Math.max(1, medianNodeToNodeSpacing * 2.5);
  const longEdgeRows = routedEdgeRows.filter((row) => row.length > longEdgeThreshold);

  const scanLayoutPlan = layoutPlan.generatedScanLayoutPlanDebug ?? {};
  const scanPlacement = layoutPlan.generatedScanV2PlacementDebug ?? {};
  const scanEdgePlanById = new Map((scanLayoutPlan.edges ?? []).map((edge) => [edge.edgeId, edge]));
  const scanRegionBoxById = new Map(
    (scanPlacement.regionBoxes ?? [])
      .filter((regionBox) => regionBox?.materialized)
      .map((regionBox) => [regionBox.regionId, regionBox]),
  );
  const isTerminalExit = (edge) => Number.isInteger(edge.sourceIndex) && (
    isHaltExitEdge(edge, analysis.program?.length ?? 0) ||
    String(edge.to ?? "").startsWith("halt")
  );
  const edgeCrossesRegion = (edge, regionBox) => (
    (edge.points ?? []).some((point, index, points) => (
      index > 0 &&
      segmentIntersectsRectInterior(points[index - 1], point, regionBox, 0.01)
    ))
  );

  const branchRemoteDistanceThreshold = Math.max(medianNodeToNodeSpacing * 3.5, layoutPlan.layout.actionNodeWidth * renderScale * 3);
  const branchCrossDiagramSpanThreshold = Math.max(medianNodeToNodeSpacing * 5, layoutPlan.layout.actionNodeWidth * renderScale * 4);
  const branchLocalityRows = routedEdges
    .filter((edge) => (
      analysis.nodeRoles?.[edge.sourceIndex] === "conditionalJump" &&
      Number.isInteger(edge.targetIndex) &&
      !isBackwardEdge(edge) &&
      !isTerminalExit(edge)
    ))
    .map((edge) => {
      const source = instructionBoundsByIndex.get(edge.sourceIndex) ?? null;
      const target = instructionBoundsByIndex.get(edge.targetIndex) ?? null;
      const edgePlan = scanEdgePlanById.get(edge.id) ?? null;
      const sourceRegionId = edgePlan?.sourceRegionId ?? edgePlan?.ownerRegionId ?? null;
      const targetRegionId = edgePlan?.targetRegionId ?? null;
      const sourceRegionBox = sourceRegionId ? scanRegionBoxById.get(sourceRegionId) ?? null : null;
      const targetNode = layoutPlan.nodeMap.get(edge.to) ?? null;
      const allowedRegionIds = new Set([sourceRegionId, targetRegionId, edgePlan?.ownerRegionId].filter(Boolean));
      const crossedUnrelatedRegionIds = Array.from(scanRegionBoxById.entries())
        .filter(([regionId, regionBox]) => !allowedRegionIds.has(regionId) && edgeCrossesRegion(edge, regionBox))
        .map(([regionId]) => regionId);
      const horizontalSpan = source && target ? Math.abs(target.centerX - source.centerX) : null;
      const verticalSpan = source && target ? Math.abs(target.centerY - source.centerY) : null;
      const sourceTargetDistance = Number.isFinite(horizontalSpan) && Number.isFinite(verticalSpan)
        ? Math.hypot(horizontalSpan, verticalSpan)
        : null;
      const usesApprovedOuterCorridor = edgePlan?.proposedLaneType === "outerContinuation";
      const targetTooRemote = Boolean(!usesApprovedOuterCorridor && sourceTargetDistance > branchRemoteDistanceThreshold);
      const hasCrossDiagramEdge = Boolean(
        !usesApprovedOuterCorridor &&
        (
          (routedEdgeRowById.get(edge.id)?.length ?? 0) > branchCrossDiagramSpanThreshold ||
          horizontalSpan > canvasWidth * 0.5 ||
          verticalSpan > canvasHeight * 0.35 ||
          crossedUnrelatedRegionIds.length > 0
        )
      );
      return {
        edgeId: edge.id,
        sourceIndex: edge.sourceIndex,
        targetIndex: edge.targetIndex,
        sourceTargetDistance: Number.isFinite(sourceTargetDistance) ? Number(sourceTargetDistance.toFixed(2)) : null,
        horizontalSpan: Number.isFinite(horizontalSpan) ? Number(horizontalSpan.toFixed(2)) : null,
        verticalSpan: Number.isFinite(verticalSpan) ? Number(verticalSpan.toFixed(2)) : null,
        routeLength: Number((routedEdgeRowById.get(edge.id)?.length ?? 0).toFixed(2)),
        sourceRegionId,
        targetRegionId,
        targetInsideSourceLocalRegion: Boolean(
          sourceRegionBox &&
          targetNode &&
          sourceRegionBox.left <= targetNode.x &&
          targetNode.x <= sourceRegionBox.right &&
          sourceRegionBox.top <= targetNode.y &&
          targetNode.y <= sourceRegionBox.bottom
        ),
        crossesUnrelatedRegions: crossedUnrelatedRegionIds.length > 0,
        crossedUnrelatedRegionIds,
        usesApprovedOuterCorridor,
        targetTooRemote,
        hasCrossDiagramEdge,
      };
    });
  const remoteBranchTargetRows = branchLocalityRows.filter((row) => row.targetTooRemote);
  const ordinaryCrossDiagramBranchRows = branchLocalityRows.filter((row) => row.hasCrossDiagramEdge);
  const branchTargetsTooRemote = remoteBranchTargetRows.length > 0;
  const ordinaryBranchHasCrossDiagramEdge = ordinaryCrossDiagramBranchRows.length > 0;

  const loopReadabilityRows = routedEdges
    .filter((edge) => isBackwardEdge(edge))
    .map((edge) => {
      const loop = (analysis.loopIntervals ?? []).find((interval) => interval.edgeId === edge.id) ?? null;
      const bodyBounds = loop
        ? rangeIndexes(loop.startIndex, loop.endIndex).map((index) => instructionBoundsByIndex.get(index)).filter(Boolean)
        : [];
      const loopBodyBBox = bodyBounds.length > 0
        ? {
          left: Math.min(...bodyBounds.map((bounds) => bounds.left)),
          right: Math.max(...bodyBounds.map((bounds) => bounds.right)),
          top: Math.min(...bodyBounds.map((bounds) => bounds.top)),
          bottom: Math.max(...bodyBounds.map((bounds) => bounds.bottom)),
        }
        : null;
      const renderedPoints = routedEdgeRowById.get(edge.id)?.renderedPoints ?? [];
      const loopEdgeBBox = renderedPoints.length > 0
        ? {
          left: Math.min(...renderedPoints.map(([x]) => x)),
          right: Math.max(...renderedPoints.map(([x]) => x)),
          top: Math.min(...renderedPoints.map(([, y]) => y)),
          bottom: Math.max(...renderedPoints.map(([, y]) => y)),
        }
        : null;
      const loopBodyBBoxWidth = loopBodyBBox ? loopBodyBBox.right - loopBodyBBox.left : 0;
      const loopBodyBBoxHeight = loopBodyBBox ? loopBodyBBox.bottom - loopBodyBBox.top : 0;
      const loopBodyBBoxArea = loopBodyBBoxWidth * loopBodyBBoxHeight;
      const loopEdgeBBoxWidth = loopEdgeBBox ? loopEdgeBBox.right - loopEdgeBBox.left : 0;
      const loopEdgeBBoxHeight = loopEdgeBBox ? loopEdgeBBox.bottom - loopEdgeBBox.top : 0;
      const loopEdgeBBoxArea = loopEdgeBBoxWidth * loopEdgeBBoxHeight;
      const ratio = loopBodyBBoxArea > 0 ? loopEdgeBBoxArea / loopBodyBBoxArea : null;
      const loopEnvelopeMuchLargerThanBody = Number.isFinite(ratio) && ratio > 5;
      const loopBackTooDominant = Boolean(
        loopEnvelopeMuchLargerThanBody ||
        (
          (routedEdgeRowById.get(edge.id)?.length ?? 0) >
          2.25 * 2 * (loopBodyBBoxWidth + loopBodyBBoxHeight)
        )
      );
      return {
        edgeId: edge.id,
        sourceIndex: edge.sourceIndex,
        targetIndex: edge.targetIndex,
        loopBBoxWidth: Number(loopEdgeBBoxWidth.toFixed(2)),
        loopBBoxHeight: Number(loopEdgeBBoxHeight.toFixed(2)),
        loopBBoxArea: Number(loopEdgeBBoxArea.toFixed(2)),
        loopBodyBBox,
        loopBodyBBoxWidth: Number(loopBodyBBoxWidth.toFixed(2)),
        loopBodyBBoxHeight: Number(loopBodyBBoxHeight.toFixed(2)),
        loopBodyBBoxArea: Number(loopBodyBBoxArea.toFixed(2)),
        loopEdgeBBoxToLoopBodyBBoxAreaRatio: Number.isFinite(ratio) ? Number(ratio.toFixed(3)) : null,
        formsMeaningfulEnvelopeAroundBody: Boolean(
          loopBodyBBox &&
          loopEdgeBBox &&
          (loopEdgeBBox.left < loopBodyBBox.left || loopEdgeBBox.right > loopBodyBBox.right) &&
          loopEdgeBBox.top <= loopBodyBBox.bottom &&
          loopEdgeBBox.bottom >= loopBodyBBox.top
        ),
        loopBackTooDominant,
        loopEnvelopeMuchLargerThanBody,
      };
    });
  const dominantLoopBackRows = loopReadabilityRows.filter((row) => row.loopBackTooDominant);
  const oversizedLoopEnvelopeRows = loopReadabilityRows.filter((row) => row.loopEnvelopeMuchLargerThanBody);
  const loopBackTooDominant = dominantLoopBackRows.length > 0;
  const loopEnvelopeMuchLargerThanBody = oversizedLoopEnvelopeRows.length > 0;

  const rightmostInstructionX = Math.max(...instructionBounds.map((bounds) => bounds.right));
  const terminalExitRows = routedEdges
    .filter((edge) => isTerminalExit(edge))
    .map((edge) => {
      const target = renderedBoundsByNodeId.get(edge.to) ?? null;
      return {
        edgeId: edge.id,
        sourceIndex: edge.sourceIndex,
        targetNodeId: edge.to,
        targetCenterX: target?.centerX ?? null,
        targetCenterY: target?.centerY ?? null,
        usesRightSideExitCorridor: Boolean(
          target &&
          target.centerX >= rightmostInstructionX + layoutPlan.layout.sideRouteGap * renderScale
        ),
      };
    });
  const haltXValues = terminalExitRows.map((row) => row.targetCenterX).filter(Number.isFinite);
  const haltTargetXSpread = haltXValues.length > 0 ? Math.max(...haltXValues) - Math.min(...haltXValues) : 0;
  const haltExitConsistencyTolerance = Math.max(
    layoutPlan.layout.actionNodeWidth * renderScale * 1.5,
    layoutPlan.layout.sideRouteGap * renderScale * 2,
  );
  const haltExitsVisuallyConsistent = haltTargetXSpread <= haltExitConsistencyTolerance;
  const haltPlacedAsSinkRegion = terminalExitRows.length === 0 ||
    terminalExitRows.every((row) => row.usesRightSideExitCorridor);
  const fragmentedHaltExits = terminalExitRows.length > 1 && !haltExitsVisuallyConsistent;
  const exitCorridorNotRecognizable = !haltPlacedAsSinkRegion;

  const approvedLongEdgeIds = new Set([
    ...routedEdges.filter((edge) => isBackwardEdge(edge) || isTerminalExit(edge)).map((edge) => edge.id),
    ...(scanLayoutPlan.edges ?? [])
      .filter((edge) => edge.proposedLaneType === "outerContinuation")
      .map((edge) => edge.edgeId),
  ]);
  const unapprovedLongEdgeRows = longEdgeRows.filter((row) => !approvedLongEdgeIds.has(row.edgeId));
  const excessiveLongEdges = unapprovedLongEdgeRows.length >= Math.max(1, Math.round(routedEdges.length * 0.08));

  const renderedNodeWidth = layoutPlan.layout.actionNodeWidth * renderScale;
  const renderedNodeHeight = layoutPlan.layout.actionNodeHeight * renderScale;
  const tinyNodeCount = renderedNodeWidth < 70 || renderedNodeHeight < 24 ? instructionNodes.length : 0;
  const labelReadabilityFailure = tinyNodeCount > 0;
  const excessiveTopWhitespace = topWhitespaceBeforeFirstNode > canvasHeight * 0.25 ||
    topWhitespaceBeforeFirstNode > visibleNodeHeight * 2;
  const graphTooSmallForCanvas = visibleNodeBBoxToCanvasHeightRatio < 0.38;
  const graphVerticallyMiscentered = Math.abs(graphCenterOffsetYFromCanvasCenter) > canvasHeight * 0.22;
  const hardFailureRules = [
    excessiveTopWhitespace ? "excessiveTopWhitespace" : null,
    graphTooSmallForCanvas ? "graphTooSmallForCanvas" : null,
    graphVerticallyMiscentered ? "graphVerticallyMiscentered" : null,
    branchTargetsTooRemote ? "branchTargetsTooRemote" : null,
    ordinaryBranchHasCrossDiagramEdge ? "ordinaryBranchHasCrossDiagramEdge" : null,
    loopBackTooDominant ? "loopBackTooDominant" : null,
    loopEnvelopeMuchLargerThanBody ? "loopEnvelopeMuchLargerThanBody" : null,
    fragmentedHaltExits ? "fragmentedHaltExits" : null,
    exitCorridorNotRecognizable ? "exitCorridorNotRecognizable" : null,
    excessiveLongEdges ? "excessiveLongEdges" : null,
    labelReadabilityFailure ? "labelReadabilityFailure" : null,
  ].filter(Boolean);
  ...
  return {
    ...
    topWhitespaceBeforeFirstNode: Number(topWhitespaceBeforeFirstNode.toFixed(2)),
    bottomWhitespaceAfterLastNode: Number(bottomWhitespaceAfterLastNode.toFixed(2)),
    visibleNodeBBoxToCanvasHeightRatio: Number(visibleNodeBBoxToCanvasHeightRatio.toFixed(4)),
    graphCenterOffsetYFromCanvasCenter: Number(graphCenterOffsetYFromCanvasCenter.toFixed(2)),
    medianEdgeLength: Number(medianEdgeLength.toFixed(2)),
    maxEdgeLength: Number(maxEdgeLength.toFixed(2)),
    longEdgeThreshold: Number(longEdgeThreshold.toFixed(2)),
    longEdgeCount: longEdgeRows.length,
    unapprovedLongEdgeCount: unapprovedLongEdgeRows.length,
    unapprovedLongEdgeRows,
    branchTargetsTooRemote,
    ordinaryBranchHasCrossDiagramEdge,
    loopBackTooDominant,
    loopEnvelopeMuchLargerThanBody,
    fragmentedHaltExits,
    exitCorridorNotRecognizable,
    excessiveLongEdges,
    renderedNodeWidth: Number(renderedNodeWidth.toFixed(2)),
    renderedNodeHeight: Number(renderedNodeHeight.toFixed(2)),
    tinyNodeCount,
    labelReadabilityFailure,
    branchLocality: {
      remoteDistanceThreshold: Number(branchRemoteDistanceThreshold.toFixed(2)),
      crossDiagramSpanThreshold: Number(branchCrossDiagramSpanThreshold.toFixed(2)),
      rows: branchLocalityRows,
      remoteBranchTargetRows,
      ordinaryCrossDiagramBranchRows,
    },
    loopReadability: {
      rows: loopReadabilityRows,
      dominantLoopBackRows,
      oversizedLoopEnvelopeRows,
    },
    exitHaltCorridor: {
      rows: terminalExitRows,
      haltTargetXSpread: Number(haltTargetXSpread.toFixed(2)),
      haltExitConsistencyTolerance: Number(haltExitConsistencyTolerance.toFixed(2)),
      haltExitsVisuallyConsistent,
      haltPlacedAsSinkRegion,
      fragmentedHaltExits,
      exitCorridorNotRecognizable,
    },
    readability: {
    ...
    hardFailureRules,
    generatedScanV2VisuallyReadable: hardFailureRules.length === 0,
  };
}

function buildGeneratedScanV2OwnershipColorTrace({
  layoutPlan,
  analysis,
  edgeRoleById,
  loopIntervals,
}) {
  const instructionNodes = layoutPlan.nodes.filter((node) => Number.isInteger(node.instructionIndex));
  const nodeByInstruction = new Map(
    instructionNodes.map((node) => [node.instructionIndex, node]),
  );
  const loopEdgeIds = new Set((loopIntervals ?? []).map((interval) => interval.edgeId));
  const incomingByTarget = new Map();
  (analysis.edges ?? []).forEach((edge) => {
    if (Number.isInteger(edge.targetIndex)) {
      if (!incomingByTarget.has(edge.targetIndex)) incomingByTarget.set(edge.targetIndex, []);
  ...
  const loopAttemptByEdgeId = new Map(
    (layoutPlan.generatedScanV2LoopClosureRoutingDebug?.attempts ?? [])
      .map((attempt) => [attempt.edgeId, attempt]),
  );
  const getAncestry = (node) => (
    Array.isArray(node?.generatedScanV2LocalFrameAncestry)
      ? node.generatedScanV2LocalFrameAncestry
      : []
  );
  const getNearestDirectionalAncestor = (node) => (
    [...getAncestry(node)].reverse().find((frame) => (
      frame?.branchDirection === "left" || frame?.branchDirection === "right"
    )) ?? null
  );
  const getDirectionalColor = (node) => (
    getGeneratedScanV2NodeBranchSide(node) ??
    getNearestDirectionalAncestor(node)?.branchDirection ??
    null
  );
  const getBranchPortSide = (edge) => {
    if (edge?.branch === "no" || edge?.branch === "false") return "left";
    if (edge?.branch === "yes" || edge?.branch === "true") return "right";
    return null;
  };
  const getSourceColor = (edge) => (
    getBranchPortSide(edge) ?? getDirectionalColor(nodeByInstruction.get(edge.sourceIndex)) ?? "neutral"
  );
  const getTargetColor = (edge) => (
    getDirectionalColor(nodeByInstruction.get(edge.targetIndex)) ?? "neutral"
  );
  const getEdgeKind = (edge) => {
    if (loopEdgeIds.has(edge.id) || (
      Number.isInteger(edge.targetIndex) && edge.targetIndex <= edge.sourceIndex
    )) return "loopBack";
    const branchPortSide = getBranchPortSide(edge);
    if (branchPortSide === "left") return "branchNo";
    if (branchPortSide === "right") return "branchYes";
    if (!Number.isInteger(edge.targetIndex)) return "halt";
    if ((incomingByTarget.get(edge.targetIndex) ?? []).length > 1) return "merge";
    if (edge.targetIndex === edge.sourceIndex + 1) return "sequential";
    if (edgeRoleById[edge.id]?.role === "exit" || edgeRoleById[edge.id]?.flowRole === "exit") {
      return "exitTail";
    }
    return "unknown";
  };
  const getTransitionType = (edge, edgeKind, sourceColor, targetColor) => {
    if (edgeKind === "halt" || edgeKind === "exitTail") return "branchToExitTail";
    const targetNode = nodeByInstruction.get(edge.targetIndex);
    if (edgeKind === "loopBack" && targetNode?.kind === "conditionalJump") {
      return "splitReturn";
    }
    if (edgeKind === "loopBack" && targetColor === "neutral") return "branchToPreSplitSpine";
    if (
      edgeKind === "merge" &&
      sourceColor !== "neutral" &&
      targetColor !== "neutral" &&
      sourceColor !== targetColor
    ) return "crossColorMerge";
    if (sourceColor === targetColor && sourceColor !== "neutral") return "sameOwnership";
    if (sourceColor !== "neutral" && targetColor !== "neutral") return "crossOwnership";
    return sourceColor === "neutral" && targetColor === "neutral" ? "neutral" : "unknown";
  };
  const edgeRows = (analysis.edges ?? [])
    .filter((edge) => Number.isInteger(edge.sourceIndex))
    .map((edge) => {
    const edgeKind = getEdgeKind(edge);
    const sourceNode = nodeByInstruction.get(edge.sourceIndex);
    const targetNode = nodeByInstruction.get(edge.targetIndex);
    const sourceColor = getSourceColor(edge);
    const targetColor = getTargetColor(edge);
    const transitionType = getTransitionType(edge, edgeKind, sourceColor, targetColor);
    const loopAttempt = loopAttemptByEdgeId.get(edge.id) ?? null;
    const renderedRouteSide = loopAttempt?.actualRenderedSideRelativeToOwner ??
      loopAttempt?.finalClosureSide ?? null;
    const governedBy = edgeKind === "loopBack"
      ? (targetNode?.kind === "conditionalJump" ? "splitPortColor" : "targetColor")
      : edgeKind === "branchYes" || edgeKind === "branchNo"
        ? "splitPortColor"
        : "sourceColor";
    return {
      edgeId: edge.id,
      sourceInstruction: Number.isInteger(edge.sourceIndex) ? edge.sourceIndex : null,
      targetInstruction: Number.isInteger(edge.targetIndex) ? edge.targetIndex : null,
      edgeKind,
      sourceFrameAncestry: getAncestry(sourceNode).map((frame) => frame.frameId),
      targetFrameAncestry: getAncestry(targetNode).map((frame) => frame.frameId),
      sourceOwnershipColor: sourceColor,
      ownershipTokenBefore: sourceNode?.generatedScanV2OwnershipToken ?? null,
      sourceOwnershipPort: getBranchPortSide(edge),
      targetOwnershipColor: targetColor,
      ownershipTokenAfter: targetNode?.generatedScanV2OwnershipToken ?? null,
      tokenPropagationReason: targetNode?.generatedScanV2OwnershipToken
        ? "targetCarriesOwnershipToken"
        : null,
      tokenResetReason: sourceNode?.generatedScanV2OwnershipToken &&
        !targetNode?.generatedScanV2OwnershipToken
        ? "targetOwnershipTokenUnavailable"
        : null,
      targetOwnershipPort: loopAttempt?.targetReturnPort ?? (
        edgeKind === "loopBack" && targetNode?.kind === "conditionalJump"
          ? (sourceColor === "left" || sourceColor === "right" ? sourceColor : null)
          : null
      ),
      targetReturnPortReason: loopAttempt?.targetReturnPortReason ?? null,
      sourceTokenSide: loopAttempt?.sourceTokenSide ??
        sourceNode?.generatedScanV2OwnershipToken?.side ?? null,
      targetPortSide: loopAttempt?.targetPortSide ?? null,
      ownershipTransitionType: transitionType,
      renderedRouteSide,
      routeSideAgreesWithSourceColor: Boolean(
        renderedRouteSide && renderedRouteSide === sourceColor
      ),
      routeSideAgreesWithTargetColor: Boolean(
        renderedRouteSide && renderedRouteSide === targetColor
      ),
      governedBy,
      closureSideSource: loopAttempt?.closureSideSource ?? null,
      returnClassification: loopAttempt?.returnClassification ?? null,
      returnClassificationReason: loopAttempt?.returnClassificationReason ?? null,
      sideAuthority: loopAttempt?.sideAuthority ?? null,
      enclosingBodyFrameId: loopAttempt?.enclosingBodyFrameId ?? null,
      enclosingBodyFrameSide: loopAttempt?.enclosingBodyFrameSide ?? null,
      compactClosureCandidate: loopAttempt?.compactClosureCandidate ?? false,
      compactClosureAccepted: loopAttempt?.compactClosureAccepted ?? false,
      targetSplitPortUsedFor: loopAttempt?.targetSplitPortUsedFor ?? null,
      generatedBeforeSide: loopAttempt?.generatedBeforeSide ?? null,
      generatedAfterSide: loopAttempt?.generatedAfterSide ?? null,
      safetyOverrideReason: loopAttempt?.safetyOverrideReason ?? null,
      closureFollowsTargetPort: loopAttempt?.closureFollowsTargetPort ?? false,
      closureFollowsSourceToken: loopAttempt?.closureFollowsSourceToken ?? false,
      splitPortRejectedBySafetyGate: loopAttempt?.splitPortRejectedBySafetyGate ?? false,
      splitPortSafetyGateReason: loopAttempt?.splitPortSafetyGateReason ?? null,
    };
    });
  const meaningfulEdgeColor = (edge) => (
    edge.sourceOwnershipPort ?? edge.sourceOwnershipColor
  );
  const nodeRows = instructionNodes.map((node) => {
    const incomingEdges = edgeRows.filter((edge) => edge.targetInstruction === node.instructionIndex);
    const outgoingEdges = edgeRows.filter((edge) => edge.sourceInstruction === node.instructionIndex);
    const incomingOwnershipColors = Array.from(new Set(incomingEdges.map(meaningfulEdgeColor)));
    const outgoingOwnershipColors = Array.from(new Set(outgoingEdges.map(meaningfulEdgeColor)));
    const directionalColor = getDirectionalColor(node);
    const nearestDirectionalAncestor = getNearestDirectionalAncestor(node);
    const isSplit = node.kind === "conditionalJump";
    const isMerge = incomingEdges.length > 1;
    return {
      instructionId: formatInstructionNodeId(node.instructionIndex),
      instructionIndex: node.instructionIndex,
      frameOwnerId: node.generatedScanV2LocalFrameId ?? null,
      branchAncestryChain: getAncestry(node)
        .filter((frame) => frame.branchDirection === "left" || frame.branchDirection === "right")
        .map((frame) => ({ frameId: frame.frameId, branchDirection: frame.branchDirection })),
      ownershipColor: isSplit
        ? "split"
        : isMerge
          ? (
            incomingOwnershipColors.length === 1
              ? "sameColorMerge"
              : incomingOwnershipColors.length > 1
                ? "crossColorMerge"
                : "neutralMerge"
          )
          : directionalColor ?? "neutral",
      ownershipLane: directionalColor ?? "neutral",
      ownershipToken: node.generatedScanV2OwnershipToken ?? null,
      nearestDirectionalAncestor: nearestDirectionalAncestor?.frameId ?? null,
      hasSplitPortOwnership: isSplit,
      incomingOwnershipColors,
      outgoingOwnershipColors,
    };
  });
  const loopRows = edgeRows
    .filter((edge) => edge.edgeKind === "loopBack")
    .map((edge) => {
      const attempt = loopAttemptByEdgeId.get(edge.edgeId) ?? null;
      const shouldClosureFollowReturnColor = edge.targetOwnershipColor === "left" ||
        edge.targetOwnershipColor === "right";
      return {
        loopSourceInstruction: edge.sourceInstruction,
        loopReturnInstruction: edge.targetInstruction,
        sourceOwnershipColor: edge.sourceOwnershipColor,
        returnOwnershipColor: edge.targetOwnershipColor,
        returnPointSplitPortSide: edge.targetOwnershipPort,
        inheritedClosureSide: attempt?.inheritedClosureSide ?? "none",
        finalClosureSide: attempt?.finalClosureSide ?? null,
        actualRenderedSideRelativeToOwner: edge.renderedRouteSide,
        ownershipTransitionType: edge.ownershipTransitionType,
        closureSideSource: edge.closureSideSource,
        returnClassification: edge.returnClassification,
        returnClassificationReason: edge.returnClassificationReason,
        sideAuthority: edge.sideAuthority,
        enclosingBodyFrameId: edge.enclosingBodyFrameId,
        enclosingBodyFrameSide: edge.enclosingBodyFrameSide,
        compactClosureCandidate: edge.compactClosureCandidate,
        compactClosureAccepted: edge.compactClosureAccepted,
        targetSplitPortUsedFor: edge.targetSplitPortUsedFor,
        generatedBeforeSide: edge.generatedBeforeSide,
        generatedAfterSide: edge.generatedAfterSide,
        safetyOverrideReason: edge.safetyOverrideReason,
        closureFollowsTargetPort: edge.closureFollowsTargetPort,
        closureFollowsSourceToken: edge.closureFollowsSourceToken,
        splitPortRejectedBySafetyGate: edge.splitPortRejectedBySafetyGate,
        splitPortSafetyGateReason: edge.splitPortSafetyGateReason,
        shouldClosureFollowReturnColor,
        closureFollowsReturnColor: Boolean(
          edge.renderedRouteSide && edge.renderedRouteSide === edge.targetOwnershipColor
        ),
        closureFollowsSourceColor: Boolean(
          edge.renderedRouteSide && edge.renderedRouteSide === edge.sourceOwnershipColor
        ),
        ownershipColorMismatchReason: shouldClosureFollowReturnColor &&
          edge.renderedRouteSide !== edge.targetOwnershipColor
          ? "closureSideDiffersFromReturnColor"
          : null,
      };
    });
  const diamondRows = nodeRows
    .filter((node) => node.hasSplitPortOwnership)
    .map((node) => {
      const outgoingEdges = edgeRows.filter((edge) => edge.sourceInstruction === node.instructionIndex);
      const leftEdge = outgoingEdges.find((edge) => edge.sourceOwnershipPort === "left") ?? null;
      const rightEdge = outgoingEdges.find((edge) => edge.sourceOwnershipPort === "right") ?? null;
      return {
        diamondInstruction: node.instructionIndex,
        leftNoTarget: leftEdge?.targetInstruction ?? null,
        rightYesTarget: rightEdge?.targetInstruction ?? null,
        leftOwnershipColorId: leftEdge ? `I${node.instructionIndex}:left` : null,
        rightOwnershipColorId: rightEdge ? `I${node.instructionIndex}:right` : null,
        leftDescendantPreservesColor: leftEdge?.targetOwnershipColor === "left",
        rightDescendantPreservesColor: rightEdge?.targetOwnershipColor === "right",
        descendantCrossColorEdgeIds: edgeRows
          .filter((edge) => (
            edge.sourceFrameAncestry.includes(node.frameOwnerId) &&
            ["crossOwnership", "crossColorMerge"].includes(edge.ownershipTransitionType)
          ))
          .map((edge) => edge.edgeId),
      };
    });
  return {
    ...
    loopRows,
    diamondRows,
    crossColorEdgeIds: edgeRows
      .filter((edge) => ["crossOwnership", "crossColorMerge"].includes(edge.ownershipTransitionType))
      .map((edge) => edge.edgeId),
    splitPortRequiredEdgeIds: edgeRows
      .filter((edge) => edge.governedBy === "splitPortColor")
      .map((edge) => edge.edgeId),
  };
}
...
function classifyVisualGeometry(layoutPlan, selectedFunctionId = "") {
  ...
  if (isBetaFlowAuditDebugEnabled()) {
    if ((layoutPlan.layoutMode ?? analysis.layoutMode) === "generatedScanV2") {
      motifs.push({
        kind: "generatedScanV2OwnershipColorTrace",
        ...buildGeneratedScanV2OwnershipColorTrace({
          layoutPlan,
          analysis,
          edgeRoleById,
          loopIntervals,
  ...
  const includeVerboseScanDiagnostics =
    layoutPlan.layoutMode !== "generatedScanV2" ||
    scanV2AuditEnabled;
  const generatedScanV2VisualReadabilityDebug = scanV2AuditEnabled
    ? buildGeneratedScanV2VisualReadabilityDiagnostics(layoutPlan)
    : { available: false, skippedReason: "auditModeDisabled" };
  const generatedScanV2VisualGrammarDebug = scanV2AuditEnabled
    ? buildGeneratedScanV2VisualGrammarDiagnostics(layoutPlan)
    : { available: false, skippedReason: "auditModeDisabled", hardFailureRules: [] };
  ...
  if (generatedScanV2PlacementDebug) {
    motifs.push({
      kind: "generatedScanV2Placement",
      enabled: generatedScanV2PlacementDebug.enabled ?? false,
      skippedReason: generatedScanV2PlacementDebug.skippedReason ?? null,
      placementDriver: generatedScanV2PlacementDebug.placementDriver ?? "legacyRunStacking",
      cellBoxesDrivingPlacement: generatedScanV2PlacementDebug.cellBoxesDrivingPlacement ?? false,
      nodesMissingCell: generatedScanV2PlacementDebug.nodesMissingCell ?? 0,
      fallbackReason: generatedScanV2PlacementDebug.fallbackReason ?? null,
      percentNodesOnMainSpine: generatedScanV2PlacementDebug.percentNodesOnMainSpine ?? 0,
      exitOutsideOuter: generatedScanV2PlacementDebug.exitOutsideOuter ?? false,
      exitCorridorGapFromOuter:
        generatedScanV2PlacementDebug.exitCorridorGapFromOuter ?? null,
      totalDiagramHeight:
        generatedScanV2PlacementDebug.totalDiagramHeight ?? 0,
      totalDiagramWidth:
        generatedScanV2PlacementDebug.totalDiagramWidth ?? 0,
      levelConsistencyAdjustmentsApplied:
        generatedScanV2PlacementDebug.levelConsistencyAdjustmentsApplied ?? 0,
      localContinuationLaneAdjustmentsApplied:
        generatedScanV2PlacementDebug.localContinuationLaneAdjustmentsApplied ?? 0,
      localFrameLayoutAvailable:
        generatedScanV2PlacementDebug.localFrameLayoutAvailable ?? false,
      localFrameCount:
        generatedScanV2PlacementDebug.localFrameCount ?? 0,
      localFrameCommitCount:
        generatedScanV2PlacementDebug.localFrameCommitCount ?? 0,
      branchChildFrameCandidateCount:
        generatedScanV2PlacementDebug.branchChildFrameCandidateCount ?? 0,
      branchChildFrameMaterializedCount:
        generatedScanV2PlacementDebug.branchChildFrameMaterializedCount ?? 0,
      branchChildFrameSkippedCount:
        generatedScanV2PlacementDebug.branchChildFrameSkippedCount ?? 0,
      mergeChildFrameCandidateCount:
        generatedScanV2PlacementDebug.mergeChildFrameCandidateCount ?? 0,
      mergeChildFrameMaterializedCount:
        generatedScanV2PlacementDebug.mergeChildFrameMaterializedCount ?? 0,
      exitTailFrameCandidateCount:
        generatedScanV2PlacementDebug.exitTailFrameCandidateCount ?? 0,
      exitTailFrameMaterializedCount:
        generatedScanV2PlacementDebug.exitTailFrameMaterializedCount ?? 0,
      centerSpinePlacementCount:
        generatedScanV2PlacementDebug.centerSpinePlacementCount ?? 0,
      centerFallbackCount:
        generatedScanV2PlacementDebug.centerFallbackCount ?? 0,
      unknownCenterFallbackCount:
        generatedScanV2PlacementDebug.unknownCenterFallbackCount ?? 0,
      cellPackingApplied:
        generatedScanV2PlacementDebug.cellPackingApplied ?? false,
      averageVerticalGapBetweenSequentialCells:
        generatedScanV2PlacementDebug.averageVerticalGapBetweenSequentialCells ?? 0,
      maxVerticalGapBetweenSequentialCells:
        generatedScanV2PlacementDebug.maxVerticalGapBetweenSequentialCells ?? 0,
      failedContainmentCheckCount:
        generatedScanV2PlacementDebug.failedContainmentCheckCount ?? 0,
      nullRegionBoxCount: generatedScanV2PlacementDebug.nullRegionBoxCount ?? 0,
      hierarchyCoherent: generatedScanV2PlacementDebug.hierarchyCoherent ?? false,
      ...(scanV2AuditEnabled ? {
        transferCellCollapsedOntoMainSpine:
          generatedScanV2PlacementDebug.transferCellCollapsedOntoMainSpine ?? false,
        transferCellCenterX:
          generatedScanV2PlacementDebug.transferCellCenterX ?? null,
        transferCellWidth:
          generatedScanV2PlacementDebug.transferCellWidth ?? null,
        transferCellOffsetFromMainSpine:
          generatedScanV2PlacementDebug.transferCellOffsetFromMainSpine ?? null,
        transferCellBridgeBetweenSiblingIslands:
          generatedScanV2PlacementDebug.transferCellBridgeBetweenSiblingIslands ?? false,
        transferCellWarnings:
          generatedScanV2PlacementDebug.transferCellWarnings ?? [],
        percentTransferNodesOnMainSpine:
          generatedScanV2PlacementDebug.percentTransferNodesOnMainSpine ?? 0,
        siblingLoopIslandsShareSameGridColumn:
          generatedScanV2PlacementDebug.siblingLoopIslandsShareSameGridColumn ?? false,
        compactnessRatio:
          generatedScanV2PlacementDebug.compactnessRatio ?? 0,
        cellsExpandedOnlyForPaddingCount:
          generatedScanV2PlacementDebug.cellsExpandedOnlyForPaddingCount ?? 0,
        cellsWithExcessiveWhitespace:
          generatedScanV2PlacementDebug.cellsWithExcessiveWhitespace ?? [],
        runSegmentationSummary: generatedScanV2PlacementDebug.runSegmentationSummary ?? {},
        runsSplitCount: generatedScanV2PlacementDebug.runsSplitCount ?? 0,
        coarseRunWarningCount: generatedScanV2PlacementDebug.coarseRunWarningCount ?? 0,
        failedContainmentChecks:
          generatedScanV2PlacementDebug.failedContainmentChecks ?? [],
        regionBoxSourceCounts:
          generatedScanV2PlacementDebug.regionBoxSourceCounts ?? {},
        parentBoxExpandedForChildren:
          generatedScanV2PlacementDebug.parentBoxExpandedForChildren ?? false,
        parentRegionExpansionStats:
          generatedScanV2PlacementDebug.parentRegionExpansionStats ?? [],
        regionBoxes: generatedScanV2PlacementDebug.regionBoxes ?? [],
        regionContainmentChecks: generatedScanV2PlacementDebug.regionContainmentChecks ?? [],
        runBoxes: generatedScanV2PlacementDebug.runBoxes ?? [],
        nodeCoordinates: generatedScanV2PlacementDebug.nodeCoordinates ?? [],
        edgesDeferredOrUnresolved:
          generatedScanV2PlacementDebug.edgesDeferredOrUnresolved ?? [],
        suggestedMajorLayoutSkeleton:
          generatedScanV2PlacementDebug.suggestedMajorLayoutSkeleton ?? null,
        verticalNormalizationApplied:
          generatedScanV2PlacementDebug.verticalNormalizationApplied ?? false,
        verticalNormalizationShiftY:
          generatedScanV2PlacementDebug.verticalNormalizationShiftY ?? 0,
        levelConsistencyAdjustments:
          generatedScanV2PlacementDebug.levelConsistencyAdjustments ?? [],
        levelConsistencyEvaluations:
          generatedScanV2PlacementDebug.levelConsistencyEvaluations ?? [],
        localContinuationLaneAdjustments:
          generatedScanV2PlacementDebug.localContinuationLaneAdjustments ?? [],
        localContinuationCenterPlacementRows:
          generatedScanV2PlacementDebug.localContinuationCenterPlacementRows ?? [],
        ownershipTokenPropagationRows:
          generatedScanV2PlacementDebug.ownershipTokenPropagationRows ?? [],
        instructionIndexVerticalBiasCount:
          generatedScanV2PlacementDebug.instructionIndexVerticalBiasCount ?? 0,
        neutralizedInstructionIndexVerticalBiasCount:
          generatedScanV2PlacementDebug.neutralizedInstructionIndexVerticalBiasCount ?? 0,
        neutralizedInstructionIndexVerticalBiasRows:
          generatedScanV2PlacementDebug.neutralizedInstructionIndexVerticalBiasRows ?? [],
        framesOrderedByInstructionIndexCount:
          generatedScanV2PlacementDebug.framesOrderedByInstructionIndexCount ?? 0,
        framesOrderedByDiscoveryCount:
          generatedScanV2PlacementDebug.framesOrderedByDiscoveryCount ?? 0,
        nodesWithYFromInstructionIndex:
          generatedScanV2PlacementDebug.nodesWithYFromInstructionIndex ?? [],
        nodesWithYFromControlFlowAdjacency:
          generatedScanV2PlacementDebug.nodesWithYFromControlFlowAdjacency ?? [],
        suspiciousLateInstructionDownshiftEdges:
          generatedScanV2PlacementDebug.suspiciousLateInstructionDownshiftEdges ?? [],
        suspiciousLongEdgesCausedByIndexOrder:
          generatedScanV2PlacementDebug.suspiciousLongEdgesCausedByIndexOrder ?? [],
        verticalStretchGapCount:
          generatedScanV2PlacementDebug.verticalStretchGapCount ?? 0,
        suspiciousLongEdgeCount:
          generatedScanV2PlacementDebug.suspiciousLongEdgeCount ?? 0,
        overInflatedFrameCount:
          generatedScanV2PlacementDebug.overInflatedFrameCount ?? 0,
        maxReservedToNodeHeightRatio:
          generatedScanV2PlacementDebug.maxReservedToNodeHeightRatio ?? 0,
        maxReservedToNodeWidthRatio:
          generatedScanV2PlacementDebug.maxReservedToNodeWidthRatio ?? 0,
        maxLoopEnvelopeToBodyHeightRatio:
          generatedScanV2PlacementDebug.maxLoopEnvelopeToBodyHeightRatio ?? 0,
        frameEnvelopeInflationRows:
          generatedScanV2PlacementDebug.frameEnvelopeInflationRows ?? [],
        verticalGapConstraintRows:
          generatedScanV2PlacementDebug.verticalGapConstraintRows ?? [],
        rowPackingRows:
          generatedScanV2PlacementDebug.rowPackingRows ?? [],
        localFrameLayout:
          generatedScanV2PlacementDebug.localFrameLayout ?? null,
        branchChildFrameCandidates:
          generatedScanV2PlacementDebug.branchChildFrameCandidates ?? [],
        centerSpinePlacementRows:
          generatedScanV2PlacementDebug.centerSpinePlacementRows ?? [],
        visualReadability: generatedScanV2VisualReadabilityDebug,
        visualGrammar: generatedScanV2VisualGrammarDebug,
      } : {}),
    });
  }
