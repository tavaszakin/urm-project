...
} from "./endpoint_transition_intrusion_ownership.mjs";
...
const CASES = [
  {
    id: "bounded-i22-d",
    title: "1A · bounded_sub i-22 default · departure-owned conflict",
    fixture: "minimization:bounded_sub",
    focusFork: "i-22",
    candidate: "D",
    edgeId: "i-24-jump",
    nodeIds: ["i-26"],
    note: "Departure side NO; i-26 is in YES. The segment remains locally owned.",
  },
  {
    id: "bounded-i22-f",
    title: "1B · bounded_sub i-22 flipped · intrusion absent",
    fixture: "minimization:bounded_sub",
    focusFork: "i-22",
    candidate: "F",
    edgeId: "i-24-jump",
    nodeIds: ["i-26"],
    note: "The same edge and node are highlighted, but no segment intersects i-26.",
  },
  {
    id: "eq-i9-body",
    title: "2 · eq i-9 flipped · NO → NO body ownership",
    fixture: "characteristic:eq",
    focusFork: "i-9",
    candidate: "F",
    edgeId: "i-16-jump",
    nodeIds: ["i-23", "i-24", "i-25", "i-26"],
    note: "Both endpoints are NO, so body segment 1 is NO-owned and recovers four YES-node conflicts.",
  },
  {
    id: "divides-i3-arrival",
    title: "3 · divides i-3 flipped · YES → NO arrival withheld",
    fixture: "characteristic:divides",
    focusFork: "i-3",
    candidate: "F",
    edgeId: "i-53-yes",
    nodeIds: ["i-54"],
    note: "The final segment crosses source-side i-54 before reaching the NO target; target ownership is withheld.",
  },
  {
    id: "predecessor-i2-arrival",
    title: "4 · predecessor i-2 flipped · OUTSIDE → YES arrival",
    fixture: "predecessor",
    focusFork: "i-2",
    candidate: "F",
    edgeId: "i-0-yes",
    nodeIds: ["i-4"],
    note: "The edge enters i-2/YES from outside and its arrival crosses sibling node i-4 in i-2/NO.",
  },
];
...
function segmentLabelPoint(start, end, phase) {
  ...
  const phaseOffsetY = phase === "body" ? 16 : phase === "arrival" ? -11 : 0;
  return [
    (start[0] + end[0]) / 2 - (dy / length) * 15,
    (start[1] + end[1]) / 2 + (dx / length) * 15 + phaseOffsetY,
  ];
}
...
function svgFor(view, route, events, spec, scale) {
  ...
  const markerId = `transition-arrow-${spec.id}`;
  ...
  for (let segmentIndex = 0; segmentIndex < routeSegmentCount; segmentIndex += 1) {
    ...
    const [labelX, labelY] = segmentLabelPoint(start, end, transition.phase);
    const ownership = transition.segmentSide == null
      ? transition.ownershipReason === "opposite-side-arrival-withheld" ? "withheld" : "unowned"
      : `${transition.segmentSide}-owned`;
    body += `<line x1="${start[0]}" y1="${start[1]}" x2="${end[0]}" y2="${end[1]}" stroke="${color}" stroke-width="7" stroke-linecap="round"${transition.segmentSide == null ? ' stroke-dasharray="12 7" opacity="0.65"' : ""}${segmentIndex === routeSegmentCount - 1 ? ` marker-end="url(#${markerId})"` : ""}/>`;
    body += `<text class="phase-label" x="${round(labelX)}" y="${round(labelY)}" fill="${color}" text-anchor="middle">segment ${segmentIndex} · ${transition.phase} · ${ownership}</text>`;
  }
...
function metadataFor(view, route, events, spec) {
  const representative = events[0] ?? transitionForSegment(view, route, spec, 0);
  const nodePaths = [...new Set(spec.nodeIds.map((nodeId) => pathText(view.ownership.branchPath(nodeId))))];
  const transition = representative.transition;
  const result = events.length === 0 ? "intrusion absent" : representative.classification;
  const eventPhase = events.length === 0 ? "none" : `${representative.phase} · segment ${representative.segmentIndex}/${representative.routeSegmentCount}`;
  return `<dl><dt>edge × node</dt><dd>${escapeHtml(`${spec.edgeId} × ${spec.nodeIds.join(", ")}`)}</dd>`
    + `<dt>intruding phase</dt><dd>${escapeHtml(eventPhase)}</dd>`
    + `<dt>departure path</dt><dd><code>${escapeHtml(pathText(representative.departurePath))}</code></dd>`
    + `<dt>target path</dt><dd><code>${escapeHtml(pathText(representative.targetPath))}</code></dd>`
    + `<dt>node path${nodePaths.length > 1 ? "s" : ""}</dt><dd><code>${escapeHtml(nodePaths.join(" · "))}</code></dd>`
    + `<dt>transition</dt><dd><strong>${escapeHtml(transition)}</strong></dd>`
    + `<dt>segment ownership</dt><dd>${escapeHtml(representative.segmentSide ?? "withheld / unowned")}</dd>`
    + `<dt>reason</dt><dd>${escapeHtml(representative.ownershipReason)}</dd>`
    + `<dt>result</dt><dd class="${events.length === 0 ? "good" : ""}">${escapeHtml(result)}</dd></dl>`;
}
...
function cardFor(view, spec, scale) {
  ...
  article.innerHTML = `<h2>${escapeHtml(spec.title)}</h2>`
    ...
    + metadataFor(view, route, events, spec)
...
async function startViewer() {
  ...
  const draw = () => {
    ...
    window.__SKETCHV4_ENDPOINT_TRANSITION_VISUAL_STATE__ = {
  ...
  draw();
  status.textContent = `${CASES.length} isolated transition-inspection cards · ${buildMs.toFixed(1)} ms candidate construction · no orientation scoring.`;
}
